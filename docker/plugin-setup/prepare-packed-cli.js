...
const EXPECTED_NAME = 'forge-universal';
const EXPECTED_BIN = 'scripts/forge.js';
...
function validatePackedPackage(packageRoot) {
  ...
  if (!manifest.bin || manifest.bin.forge !== EXPECTED_BIN) {
    fail(`Packed package bin.forge must map to ${EXPECTED_BIN}.`);
  }
  ...
  const binTarget = path.resolve(resolvedRoot, manifest.bin.forge);
  if (!isWithin(resolvedRoot, binTarget)) {
    fail('Packed package bin.forge escapes the extracted package root.');
  }
...
function preparePackedCli(sourceRoot, outputRoot) {
  ...
  if (resolvedSource !== '/forge') {
    fail('Package source must be the read-only /forge checkout.');
  }
  ...
  const publicBin = path.join(binRoot, 'forge');
