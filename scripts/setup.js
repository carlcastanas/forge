...
Install scopes:
  user      Global for this user; FORGE is available in every project.
  project   Shared project configuration; the repository can enable FORGE for collaborators.
...
Re-running setup updates an existing forge@forge installation at its detected scope.
...
async function collectInteractiveOptions(options, defaults = {}, providedTerminal) {
  ...
  try {
    ...
    const scope = options.scope || await askChoice(
      terminal,
      'Where should Claude enable forge@forge?',
      scopeChoices,
      scopeDefaultIndex
    );
    const hookChoices = [
