...
Pull the latest FORGE repo changes and reinstall the current context's managed targets
...
function deriveRepoRootFromState(state) {
  ...
  throw new Error('Unable to infer FORGE repo root from install-state operations');
}
...
// Recognized FORGE package names. A repo root is only trusted to run its
// install-apply.js if its package.json identifies it as FORGE — otherwise a
...
const FORGE_PACKAGE_NAMES = new Set(['forge-universal', 'forge']);
...
function validateRepoRoot(repoRoot) {
  ...
  if (!fs.existsSync(packageJsonPath)) {
    throw new Error(`Invalid FORGE repo root: missing package.json at ${packageJsonPath}`);
  }

  if (!fs.existsSync(installApplyPath)) {
    throw new Error(`Invalid FORGE repo root: missing install script at ${installApplyPath}`);
  }
  ...
  try {
    ...
    throw new Error(`Invalid FORGE repo root: unreadable package.json at ${packageJsonPath}`);
  }
  if (!FORGE_PACKAGE_NAMES.has(pkgName)) {
    throw new Error(`Refusing to run install from untrusted repo root ${normalized}: package.json name '${pkgName}' is not an official FORGE package.`);
  }
...
function runAutoUpdate(options = {}, dependencies = {}) {
  ...
  if (!requestedRepoRoot) {
    ...
    if (uniqueRepoRoots.length > 1) {
      throw new Error(`Multiple FORGE repo roots detected: ${uniqueRepoRoots.join(', ')}`);
    }
  }
...
function printHuman(result) {
  if (result.results.length === 0) {
    ...
    console.log(hasWarnings
      ? 'No active FORGE install-state files found for the current home/project context.'
      : 'No FORGE install-state files found for the current home/project context.');
