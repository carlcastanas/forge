 ...
 *
 * Reads bridge file from forge-metrics-bridge.js and injects agent-facing
...
// forge-metrics-bridge.js), so 5 means ALL of the last 5 calls must be the
...
function costWarningsEnabled(env = process.env) {
  return isEnabledEnv(env.FORGE_CONTEXT_MONITOR_COST_WARNINGS, true);
}
...
function getWarnPath(sessionId) {
  return path.join(os.tmpdir(), `forge-ctx-warn-${sessionId}.json`);
}
 ...
 * and `writeCostWarningIfChanged` in `scripts/hooks/forge-metrics-bridge.js`.
 *
 * @param {string} sessionId
 * @param {object} state
 */
