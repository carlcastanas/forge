 ...
 *
 * Enable: Set FORGE_GOVERNANCE_CAPTURE=1
 * Configure session: Set FORGE_SESSION_ID for session correlation
 */
...
function run(rawInput, options = {}) {
  ...
  if (String(process.env.FORGE_GOVERNANCE_CAPTURE || '').toLowerCase() !== '1') {
    return rawInput;
  }
...
if (require.main === module) {
  ...
  process.stdin.on('end', () => {
    const result = run(raw, {
      truncated,
      maxStdin: Number(process.env.FORGE_HOOK_INPUT_MAX_BYTES) || MAX_STDIN,
    });
