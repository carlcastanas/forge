...
const { run: runMetricsBridge } = require('./forge-metrics-bridge');
const { run: runContextMonitor } = require('./forge-context-monitor');
...
const SYNC_HOOKS = [
  ...
  { id: 'post:forge-metrics-bridge', matcher: '*', profiles: 'minimal,standard,strict', script: 'scripts/hooks/forge-metrics-bridge.js', run: runMetricsBridge },
  { id: 'post:forge-context-monitor', matcher: '*', profiles: 'standard,strict', script: 'scripts/hooks/forge-context-monitor.js', run: runContextMonitor }
];
...
function getPluginRoot(env = process.env) {
  return env.CLAUDE_PLUGIN_ROOT || env.FORGE_PLUGIN_ROOT || path.resolve(__dirname, '..', '..');
}
...
async function main() {
  ...
  const stdout = resolveMainStdout(raw, result, {
    passthrough: process.env.FORGE_POSTTOOLUSE_PASSTHROUGH === '1',
    truncated
  });
