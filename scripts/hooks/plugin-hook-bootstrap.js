...
function passthrough(result) {
  const stdout =
    typeof result?.stdout === 'string' || Buffer.isBuffer(result?.stdout)
      ...
      : Buffer.alloc(0);
  if (stdout.length > 0) {
    // Most FORGE hook scripts follow a `run(rawInput) -> rawInput` passthrough
    ...
    // transcript as a hook_success attachment -- ~89% of every FORGE session's
...
function main() {
  ...
  const rootDir = normalizePluginRootForPlatform(
    process.env.CLAUDE_PLUGIN_ROOT || process.env.FORGE_PLUGIN_ROOT
  );
