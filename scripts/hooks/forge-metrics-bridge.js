#!/usr/bin/env node
/**
 * FORGE Metrics Bridge — PostToolUse hook
 *
 * Maintains a running session aggregate in /tmp/forge-metrics-{session}.json.
 * This bridge file is read by forge-statusline.js and forge-context-monitor.js,
...
const WARNING_CACHE_PREFIX = 'forge-metrics-cost-warnings-';
  ...
  try {
    ...
    if (malformed > 0) {
      writeCostWarningIfChanged(
        ...
        `[forge-metrics-bridge] skipped ${malformed} malformed line(s) in ${costsPath}\n`
      );
    }
    ...
    if (err && err.code !== 'ENOENT') {
      writeCostWarningIfChanged(
        ...
        `[forge-metrics-bridge] failing open after ${err.name || 'error'} reading ${costsPath}: ${err.message || String(err)}\n`
      );
    }
...
function run(rawInput) {
  try {
    ...
    const sessionId = sanitizeSessionId(input.session_id) || sanitizeSessionId(process.env.FORGE_SESSION_ID) || sanitizeSessionId(process.env.CLAUDE_SESSION_ID);
