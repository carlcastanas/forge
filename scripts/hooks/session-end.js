...
const SUMMARY_START_MARKER = '<!-- FORGE:SUMMARY:START -->';
const SUMMARY_END_MARKER = '<!-- FORGE:SUMMARY:END -->';
...
async function main() {
  ...
  // FORGE's LLM summary helper launches a one-shot Claude subprocess whose Stop
  ...
  if (process.env.FORGE_LLM_SUMMARY_SUBPROCESS === '1') {
    log('[SessionEnd] Skipped FORGE LLM summary subprocess');
    return;
  }
  ...
  if (transcriptPath && summary && transcriptExists) {
    ...
    const interval = parseInt(process.env.FORGE_LLM_SUMMARY_INTERVAL || '50', 10);
