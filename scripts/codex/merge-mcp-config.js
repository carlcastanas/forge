...
/**
 * Merge FORGE-recommended MCP servers into a Codex config.toml.
 *
 ...
 *   - Log warnings when an existing server's config differs from the FORGE recommendation.
 *   - With --update-mcp, also replace existing FORGE-managed servers.
 *
...
try {
  ...
  console.error('[forge-mcp] Missing dependency: @iarna/toml');
  console.error('[forge-mcp] Run: npm install   (from the FORGE repo root)');
  process.exit(1);
}
...
// FORGE <= 2.0.0 emitted [mcp_servers.exa] with a `url` key. Codex rejects
// `url` for stdio servers, which makes the *entire* config.toml fail to
// load (#2224). Repair exactly that FORGE-emitted form on every merge so
...
// Legacy section names that should be treated as an existing FORGE server.
...
function log(msg) {
  console.log(`[forge-mcp] ${msg}`);
}

function warn(msg) {
  console.warn(`[forge-mcp] WARNING: ${msg}`);
}
...
function main() {
  ...
  const disabledServers = new Set(parseDisabledMcpServers(process.env.FORGE_DISABLED_MCPS));
  ...
  if (!fs.existsSync(configPath)) {
    console.error(`[forge-mcp] Config file not found: ${configPath}`);
    process.exit(1);
  }

  log(`Package manager: ${PM_NAME} (exec: ${PM_EXEC})`);
  if (disabledServers.size > 0) {
    log(`Disabled via FORGE_DISABLED_MCPS: ${[...disabledServers].join(', ')}`);
  }
  ...
  try {
    ...
    console.error(`[forge-mcp] Failed to parse ${configPath}: ${err.message}`);
    process.exit(1);
  }
  ...
  for (const [name, spec] of Object.entries(FORGE_SERVERS)) {
    const entry = existing[name];
    ...
    // (Codex itself rejects url-form stdio servers; FORGE only ever emits
    ...
    if (finalEntry) {
      if (updateMcp) {
        ...
        if (legacyName && !hasCanonical) {
          ...
          warn(`mcp_servers.${name} differs from FORGE recommendation (run with --update-mcp to refresh)`);
        } else {
  ...
  if (toAppend.length === 0 && !hasRemovals) {
    log('All FORGE MCP servers already present. Nothing to do.');
    return;
  }
