...
# Install FORGE git safety hooks globally via core.hooksPath.
# Usage:
...
DEST_DIR="${FORGE_GLOBAL_HOOKS_DIR:-$HOME/.codex/git-hooks}"
STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP_DIR="$HOME/.codex/backups/git-hooks-$STAMP"

log() {
  printf '[forge-hooks] %s\n' "$*"
}
...
log "Installed FORGE global git hooks."
log "Disable per repo by creating .forge-hooks-disable in project root."
log "Temporary bypass: FORGE_SKIP_PRECOMMIT=1 or FORGE_SKIP_PREPUSH=1"
