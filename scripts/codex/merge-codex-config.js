...
try {
  ...
  console.error('[forge-codex] Missing dependency: @iarna/toml');
  console.error('[forge-codex] Run: npm install   (from the FORGE repo root)');
  process.exit(1);
}
...
function log(message) {
  console.log(`[forge-codex] ${message}`);
}

function warn(message) {
  console.warn(`[forge-codex] WARNING: ${message}`);
}
...
function main() {
  ...
  if (!fs.existsSync(referencePath)) {
    console.error(`[forge-codex] Reference config not found: ${referencePath}`);
    process.exit(1);
  }

  if (!fs.existsSync(configPath)) {
    console.error(`[forge-codex] Config file not found: ${configPath}`);
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(`[forge-codex] Failed to parse TOML: ${error.message}`);
    process.exit(1);
  }
