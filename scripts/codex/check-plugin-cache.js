...
function usage() {
  console.log([
    ...
    '  --marketplace <name> Marketplace cache name (default: forge)',
    '  --plugin <name>      Plugin cache name (default: forge)',
...
function checkCache(options) {
  ...
  if (failures > 0) {
    ...
    const syncScript = path.join(REPO_ROOT, 'scripts', 'sync-forge-to-codex.sh');
    if (fs.existsSync(syncScript)) {
      log('Use the supported sync path until the cache contains the referenced files:');
      log('npm install && bash scripts/sync-forge-to-codex.sh');
    } else {
      log('Use the supported manual sync workflow from your FORGE installation.');
    }
    return 1;
  }
...
function main() {
  ...
  try {
    ...
    console.error(`[forge-codex] ${error.message}`);
    usage();
    process.exit(1);
  }
