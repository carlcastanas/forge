...
Track Linear, GitHub, handoff, and manual roadmap items in the FORGE SQLite state
store so "forge status" can include linked work and blocked operator follow-up.

Options:
  --id <id>                 Stable local work-item id for upsert
  --source <source>         Source system, e.g. linear, github, handoff, manual
  --source-id <id>          Source-local identifier, e.g. FORGE-20 or PR number
  ...
  --session-id <id>         Optional FORGE session id
...
function runGhJson(args) {
  const shimPath = process.env.FORGE_GH_SHIM;
...
function buildGithubPrWorkItem(repo, pr, options = {}) {
  return {
    ...
    metadata: {
      ...
      syncedBy: 'forge-work-items-sync-github'
    }
  };
}
...
function closeStaleGithubItems(store, repo, activeIds, options = {}) {
  ...
  for (const item of payload.items) {
    if (!item.metadata || item.metadata.syncedBy !== 'forge-work-items-sync-github') {
      continue;
    }
