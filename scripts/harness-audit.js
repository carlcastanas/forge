...
const FORGE_PLUGIN_KEY_PATTERNS = [
  /^forge@/i,
  /^forge@/i,
];

const FORGE_LEGACY_PLUGIN_DIRS = [
  'forge',
  'forge@forge',
  'forge',
  'forge@forge',
...
const FORGE_CACHE_MARKETPLACES = ['forge', 'forge'];
const FORGE_CACHE_PLUGIN_NAMES = ['forge', 'forge'];
...
function findPluginInstallFromManifest(installedPluginsPaths) {
  for (const installedPath of installedPluginsPaths) {
    ...
    for (const [key, value] of Object.entries(manifest.plugins)) {
      if (!FORGE_PLUGIN_KEY_PATTERNS.some(pattern => pattern.test(key))) {
        continue;
      }
...
function findPluginInstallFlatLayout(candidateRoots) {
  for (const pluginsDir of candidateRoots) {
    for (const pluginDir of FORGE_LEGACY_PLUGIN_DIRS) {
...
function findPluginInstallMarketplaceCache(candidateRoots) {
  for (const pluginsDir of candidateRoots) {
    for (const marketplace of FORGE_CACHE_MARKETPLACES) {
      for (const pluginName of FORGE_CACHE_PLUGIN_NAMES) {
  ...
  return [
    {
      ...
      points: 4,
      scopes: ['repo'],
      path: '~/.claude/plugins/forge/ (legacy forge paths also supported)',
      description: 'FORGE is installed for the active user or project',
      pass: Boolean(pluginInstall),
      fix: 'Install the FORGE plugin for this user or project before auditing project-specific harness quality.',
    },
    {
      ...
      fix: 'Add project-local .claude hooks, commands, skills, or settings that tailor FORGE to this repo.',
    },
    {
...
Audits the current working directory by default and auto-detects FORGE repo mode vs consumer-project mode.
`);
