...
function showHelp(output = process.stdout) {
  output.write(`
FORGE guided multi-harness install

Usage:
  forge install --guided
  forge install --guided --harness claude --harness codex --harness kimi [options]

Guided harnesses:
  claude  Native Claude Code plugin; choose user, project, or local scope and an FORGE hook profile.
  codex   Native Codex plugin and Codex-owned hook review/trust.
  kimi    Managed project install under ./.kimi-code; FORGE hooks are not configured.
...
Advanced managed adapters remain available through explicit forge install --target commands:
  ${ADVANCED_HARNESSES}

This command configures FORGE. It does not install or authenticate provider CLIs.
`);
}
...
async function askHarnesses(terminal, output) {
  ...
  output.write('\nWhich coding agents should FORGE configure?\n');
  ...
  output.write(`\nAdvanced adapters (use forge install --target): ${ADVANCED_HARNESSES}.\n\n`);
  while (true) {
...
async function collectInteractiveOptions(options, dependencies = {}) {
  ...
  const claudeScope = includesClaude && !options.claudeScope
    ? await askChoice(terminal, output, 'Where should Claude enable forge@forge?', [...VALID_CLAUDE_SCOPES], 'user')
    : options.claudeScope;
  const claudeHooks = includesClaude && !options.claudeHooks
    ? await askChoice(terminal, output, 'How should FORGE hooks run in Claude?', [...VALID_CLAUDE_HOOKS], 'standard')
    : options.claudeHooks;
  const profile = includesKimi && !options.profile
    ? await askChoice(terminal, output, 'Which FORGE content profile should Kimi receive?', [...VALID_PROFILES], 'core')
    : options.profile;
  return {
    ...options,
...
function printPlan(plan, output) {
  ...
  if (plan.request.harnesses.includes('kimi')) {
    output.write('\nKimi note: FORGE hooks are not configured; model, provider, and authentication settings are unchanged.\n');
  }
...
async function confirmPlan(terminal, output) {
  output.write('\n');
  const answer = await terminal.question('Apply FORGE to these harnesses? [y/N]: ');
...
async function main(argv = process.argv.slice(2), injected = {}) {
  ...
  try {
    ...
    const spinner = interactive && !options.json
      ? makeSpinner('Applying FORGE to selected harnesses...')
      : undefined;
    let result;
    try {
    ...
    if (options.json) {
      ...
      errorOutput.write(
        `FORGE stopped at ${sanitizeTerminalText(result.failure.id)}: `
        + `${sanitizeTerminalText(result.failure.message)}\n`
        + `Retry with: forge-universal install --guided ${retry}\n`
      );
    }
