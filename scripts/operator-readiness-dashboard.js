...
function forgeShieldEnterpriseGap(roadmap) {
  if (roadmap.includes('operator-visible promotion output values')) {
    return 'deepen live operator approval and readback after the release gates';
  }
...
function linearProgressEvidence(context) {
  if (hasCurrentLinearProgressSync(context)) {
    if (context.roadmap.includes('467d148a-712a-4777-aad9-95593e9f1739')
      ...
      return 'Linear live sync is current with the May 20 release-gate comments on ITO-61 and the FORGE platform roadmap; progress-sync contract defines the file-backed work-items/status path';
    }
...
function buildRequirements(rootDir, platformReport) {
  ...
  const ownerApprovalPacketReady = includesAll(ownerApprovalPacket, [
    ...
    'Final URL Fill-In',
    'Do Not Approve If',
    'No outbound email, personal-account post, package publish, or plugin tag is authorized by this packet alone.'
  ...
  return [
    ...
    buildRequirement(
      ...
      includesAll(releaseUrlLedger, ['Live Now', 'Approval-Gated URLs', 'Codex marketplace CLI docs'])
        ? 'final live release/npm/plugin URLs and publish approval still pending'
    ...
    buildRequirement(
      ...
      ownerApprovalPacketReady
        ? 'owner approval packet covers release, package, plugin, video, social, and outbound decisions'
...
function renderMarkdown(report) {
  const lines = [
    ...
    `| Publication | ${report.publicationReady ? 'Ready' : 'Not complete'} | release, npm, plugin, and announcement gates are tracked below |`,
    '',
