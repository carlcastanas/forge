...
# Sync FORGE assets into a local Codex CLI setup.
# - Backs up ~/.codex config and AGENTS.md
# - Merges FORGE AGENTS.md into existing AGENTS.md (marker-based, preserves user content)
...
# - Merges FORGE MCP servers into config.toml (add-only via Node TOML parser)
...
BACKUP_DIR="$CODEX_HOME/backups/forge-$STAMP"

log() { printf '[forge-sync] %s\n' "$*"; }
...
generate_prompt_file() {
  ...
  local cmd_name="$3"
  {
    printf '# FORGE Command Prompt: /%s\n\n' "$cmd_name"
    printf 'Source: %s\n\n' "$src"
    printf 'Use this prompt to run the FORGE `%s` workflow.\n\n' "$cmd_name"
    awk '
      NR == 1 && $0 == "---" { fm = 1; next }
      fm == 1 && $0 == "---" { fm = 0; next }
      fm == 1 { next }
...
require_path "$REPO_ROOT/AGENTS.md" "FORGE AGENTS.md"
require_path "$AGENTS_CODEX_SUPP_SRC" "FORGE Codex AGENTS supplement"
require_path "$CODEX_AGENTS_SRC" "FORGE Codex agent roles"
require_path "$CODEX_NAV_GUIDE_SRC" "FORGE Codex navigation guide"
require_path "$CODEX_COMMAND_AGENT_MAP_SRC" "FORGE command-agent map"
require_path "$CODEX_COMMANDS_QUICK_REF_SRC" "FORGE commands quick reference"
require_path "$CODEX_CONTRIBUTING_SRC" "FORGE contributing guide"
require_path "$CODEX_PR_TEMPLATE_SRC" "FORGE PR template"
require_path "$PROMPTS_SRC" "FORGE commands directory"
require_path "$BASELINE_MERGE_SCRIPT" "FORGE Codex baseline merge script"
require_path "$HOOKS_INSTALLER" "FORGE global git hooks installer"
require_path "$SANITY_CHECKER" "FORGE global sanity checker"
require_path "$LEGACY_STATE_HELPER" "FORGE legacy sync state helper"
require_path "$CURSOR_RULES_DIR" "FORGE Cursor rules directory"
require_path "$CONFIG_FILE" "Codex config.toml"
require_path "$MCP_MERGE_SCRIPT" "FORGE MCP merge script"
...
if [[ "$MODE" == "apply" ]]; then
  ...
  LEGACY_STATE_PATH="$(
    node "$LEGACY_STATE_HELPER" begin \
      ...
      --installed-hooks-path "${FORGE_GLOBAL_HOOKS_DIR:-$CODEX_HOME/git-hooks}"
  )"
...
FORGE_BEGIN_MARKER="<!-- BEGIN FORGE -->"
FORGE_END_MARKER="<!-- END FORGE -->"

compose_forge_block() {
  printf '%s\n' "$FORGE_BEGIN_MARKER"
  cat "$AGENTS_ROOT_SRC"
  printf '\n\n---\n\n'
  printf '# Codex Supplement (From FORGE .codex/AGENTS.md)\n\n'
  cat "$AGENTS_CODEX_SUPP_SRC"
  printf '\n%s\n' "$FORGE_END_MARKER"
}

log "Merging FORGE AGENTS into $AGENTS_FILE (preserving user content)"
if [[ "$MODE" == "dry-run" ]]; then
  printf '[dry-run] merge FORGE block into %s from %s + %s\n' "$AGENTS_FILE" "$AGENTS_ROOT_SRC" "$AGENTS_CODEX_SUPP_SRC"
else
  replace_forge_section() {
    # Replace the FORGE block between markers in $AGENTS_FILE with fresh content.
    # Uses awk to correctly handle all positions including line 1.
    local tmp
    tmp="$(mktemp)"
    local forge_tmp
    forge_tmp="$(mktemp)"
    compose_forge_block > "$forge_tmp"
    awk -v begin="$FORGE_BEGIN_MARKER" -v end="$FORGE_END_MARKER" -v forge="$forge_tmp" '
      { gsub(/\r$/, "") }
      $0 == begin { skip = 1; while ((getline line < forge) > 0) print line; close(forge); next }
      $0 == end   { skip = 0; next }
      !skip        { print }
    ' "$AGENTS_FILE" > "$tmp"
  ...
  if [[ ! -f "$AGENTS_FILE" ]]; then
    ...
    compose_forge_block > "$AGENTS_FILE"
  elif awk -v b="$FORGE_BEGIN_MARKER" -v e="$FORGE_END_MARKER" '
        { gsub(/\r$/, "") }
        $0 == b { bc++; if (!fb) fb = NR }
        $0 == e { ec++; if (!fe) fe = NR }
        END { exit !(bc == 1 && ec == 1 && fb < fe) }
      ' "$AGENTS_FILE"; then
    # Exactly one BEGIN/END pair in correct order — replace only the FORGE section
    replace_forge_section
  elif awk -v b="$FORGE_BEGIN_MARKER" -v e="$FORGE_END_MARKER" '
        { gsub(/\r$/, "") }
        $0 == b { bc++ } $0 == e { ec++ }
    ...
    log "WARNING: FORGE markers found but not a clean pair — stripping markers and re-appending"
    _fix_tmp="$(mktemp)"
    awk -v b="$FORGE_BEGIN_MARKER" -v e="$FORGE_END_MARKER" '
      { gsub(/\r$/, "") }
      $0 == b { skip = 1; next }
      $0 == e { skip = 0; next }
      !skip   { print }
    ...
    # Existing file without markers — append FORGE block, preserving existing content.
    # Legacy FORGE-only files will have duplicate content after this first run, but
    ...
    log "No FORGE markers found — appending managed block (backup saved)"
    {
      printf '\n\n'
      compose_forge_block
    } >> "$AGENTS_FILE"
...
log "Merging FORGE Codex baseline into $CONFIG_FILE (add-only, preserving user config)"
if [[ "$MODE" == "dry-run" ]]; then
...
# ~/.agents/skills/ (installed by FORGE installer / npx skills).
# Copying into ~/.codex/skills/ was unnecessary.

log "Generating prompt files from FORGE commands"
run_or_echo mkdir -p "$PROMPTS_DEST"
manifest="$PROMPTS_DEST/forge-prompts-manifest.txt"
...
while IFS= read -r -d '' command_file; do
  ...
  out="$PROMPTS_DEST/forge-$name.md"
  if [[ "$MODE" == "dry-run" ]]; then
...
extension_manifest="$PROMPTS_DEST/forge-extension-prompts-manifest.txt"
...
write_extension_prompt "forge-tool-run-tests.md" <<EOF
# FORGE Tool Prompt: run-tests
...
write_extension_prompt "forge-tool-check-coverage.md" <<EOF
# FORGE Tool Prompt: check-coverage
...
write_extension_prompt "forge-tool-security-audit.md" <<EOF
# FORGE Tool Prompt: security-audit
...
write_extension_prompt "forge-rules-pack-common.md" <<EOF
# FORGE Rule Pack: common (optional)

Apply FORGE common engineering rules for this session. Use these files as the source of truth:
...
write_extension_prompt "forge-rules-pack-typescript.md" <<EOF
# FORGE Rule Pack: typescript (optional)

Apply FORGE common rules plus TypeScript-specific rules for this session.

## Common
Use \`$PROMPTS_DEST/forge-rules-pack-common.md\`.
...
write_extension_prompt "forge-rules-pack-python.md" <<EOF
# FORGE Rule Pack: python (optional)

Apply FORGE common rules plus Python-specific rules for this session.
...
write_extension_prompt "forge-rules-pack-golang.md" <<EOF
# FORGE Rule Pack: golang (optional)

Apply FORGE common rules plus Go-specific rules for this session.
...
write_extension_prompt "forge-rules-pack-swift.md" <<EOF
# FORGE Rule Pack: swift (optional)

Apply FORGE common rules plus Swift-specific rules for this session.
...
log "Merging FORGE MCP servers into $CONFIG_FILE (add-only, preserving user config)"
if [[ "$MODE" == "dry-run" ]]; then
...
if [[ "$MODE" == "dry-run" ]]; then
  ...
  FORGE_GLOBAL_HOOKS_DIR="${FORGE_GLOBAL_HOOKS_DIR:-$CODEX_HOME/git-hooks}" \
    "$HOOKS_INSTALLER" --dry-run
else
  record_managed_path "${FORGE_GLOBAL_HOOKS_DIR:-$CODEX_HOME/git-hooks}/pre-commit"
  record_managed_path "${FORGE_GLOBAL_HOOKS_DIR:-$CODEX_HOME/git-hooks}/pre-push"
  HOME="$HOME" \
  CODEX_HOME="$CODEX_HOME" \
  AGENTS_HOME="${AGENTS_HOME:-$HOME/.agents}" \
  FORGE_GLOBAL_HOOKS_DIR="${FORGE_GLOBAL_HOOKS_DIR:-$CODEX_HOME/git-hooks}" \
