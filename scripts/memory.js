...
Write options:
  ...
  --source-harness <name>    Originating harness (default: FORGE_MEMORY_HARNESS or unknown)
...
MCP:
  forge-memory-mcp             Start the opt-in local stdio MCP server
...
function printInit(result, json) {
  if (json) return writeJson({ schemaVersion: 'forge.memory.init.v1', ...result });
  process.stdout.write([
    `Initialized FORGE memory scopes: ${sanitizeTerminalText(result.scopes.join(', '))}`,
...
function printWrite(result, json) {
  ...
  const payload = {
    schemaVersion: 'forge.memory.write.v1',
    memory,
...
function printSearch(query, result, json) {
  const payload = { schemaVersion: 'forge.memory.search.v1', query, ...result };
  if (json) return writeJson(payload);
...
function printRead(result, json) {
  const payload = { schemaVersion: 'forge.memory.read.v1', ...result };
  if (json) return writeJson(payload);
  process.stdout.write([
...
function printDoctor(report, json) {
  ...
  process.stdout.write([
    `FORGE memory doctor: ${report.ok ? 'PASS' : 'ISSUES FOUND'}`,
...
function saveInput(options, kindOverride = null) {
  const sourceHarness = options.from
    ...
    || process.env.FORGE_MEMORY_HARNESS
    || 'unknown';
  return {
    title: options.title,
