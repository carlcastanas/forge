...
Remove FORGE-managed files recorded in install-state for the current context.
When no install-state is found, the uninstaller also detects and removes
legacy sync-forge-to-codex.sh artifacts, but only when a legacy ownership
...
function printHuman(result) {
  if (result.results.length === 0) {
    console.log('No FORGE install-state files found for the current home/project context.');
    return;
  }
 ...
 * global `forge --dry-run <command>` prefix, which sets FORGE_DRY_RUN=1 (#2952).
 * Destructive subcommands must honor both forms rather than silently ignoring
 * the global flag.
 */
function isDryRun(options) {
  const dryRunEnv = process.env.FORGE_DRY_RUN;
  if (dryRunEnv !== undefined && dryRunEnv !== '0' && dryRunEnv !== '1') {
    throw new Error('FORGE_DRY_RUN must be "1" or "0" when set');
  }
