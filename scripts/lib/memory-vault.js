...
function resolveVaultRoots(options = {}) {
  ...
  const projectVault = env.FORGE_MEMORY_PROJECT_ROOT
    ? resolveOverride(env.FORGE_MEMORY_PROJECT_ROOT, cwd)
    : path.join(projectRoot, '.forge', 'memory');
  const userVault = env.FORGE_MEMORY_USER_ROOT
    ? resolveOverride(env.FORGE_MEMORY_USER_ROOT, cwd)
    : path.join(homeDir, '.forge', 'memory');
  ...
  Object.defineProperty(roots, VAULT_ROOT_BOUNDARIES, {
    value: Object.freeze({
      project: env.FORGE_MEMORY_PROJECT_ROOT
        ? realpathNearestExisting(projectVault)
        : projectRoot,
      team: env.FORGE_MEMORY_PROJECT_ROOT
        ? realpathNearestExisting(projectVault)
        : projectRoot,
      user: env.FORGE_MEMORY_USER_ROOT
...
function writeCreateOnlyTextFile(filePath, content, trustedRoot) {
  ...
  const temporaryPath = path.join(
    ...
    `.forge-memory-${process.pid}-${crypto.randomUUID()}.tmp`
  );
...
function assertMemoryMatchesLocation(memory, scope, root, filePath) {
  ...
  if (memory.scope !== scope || kindDirectory !== `${memory.kind}s`) {
    ...
    error.code = 'FORGE_MEMORY_LOCATION_MISMATCH';
    throw error;
  }
}

function publicMemoryFileError(error) {
  if (error?.code === 'FORGE_MEMORY_SECRET') {
    return { code: 'suspected-secret', message: 'Memory document was quarantined.' };
  }
  if (error?.code === 'FORGE_MEMORY_LOCATION_MISMATCH') {
    return {
    ...
    for (const filePath of walked.paths) {
      ...
      try {
        ...
        if (findPotentialSecrets(JSON.stringify(memory)).length > 0) {
          ...
          error.code = 'FORGE_MEMORY_SECRET';
          throw error;
        }
        entries.push({
          memory,
...
function doctorMemoryVault(options = {}) {
  ...
  return {
    schemaVersion: 'forge.memory.doctor.v1',
    ok,
