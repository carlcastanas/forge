  ...
  for (const plugin of plugins) {
    ...
    if (
      ...
      fail(
        ...
        'Claude plugin inventory contains an invalid FORGE plugin entry'
      );
    }
  }
  return plugins;
  ...
  for (const marketplace of marketplaces) {
    ...
    if (
      ...
      fail(
        ...
        'Claude marketplace inventory contains an invalid `forge` entry'
      );
    }
  }
  return marketplaces;
...
function assertGitAvailable(options = {}, dependencies = {}) {
  ...
  if (result.error?.code === 'ENOENT') {
    fail(
      'GIT_NOT_FOUND',
      'Git is required for Claude marketplace setup but `git` is not on PATH. Install Git, ensure `git` is on PATH, then rerun FORGE setup.',
      {
        phase: 'preflight',
        recovery: [
  ...
  if (result.error || result.status !== 0) {
    ...
    fail(
      ...
      {
        phase: 'preflight',
        recovery: ['Repair Git, ensure `git --version` succeeds, then rerun FORGE setup.'],
      }
    );
  }
}
...
function runClaude(args, options = {}, dependencies = {}) {
  ...
  if (result.error) {
    if (result.error.code === 'ENOENT') {
      fail(
        'CLAUDE_NOT_FOUND',
        'Claude Code is not installed or `claude` is not on PATH. Install Claude Code, then rerun FORGE setup.',
...
function readSettings(settingsPath) {
  ...
  const forgeConfig = pluginConfigs?.[CURRENT_PLUGIN_ID];
  if (forgeConfig !== undefined && (
    !forgeConfig
    || typeof forgeConfig !== 'object'
    || Array.isArray(forgeConfig)
  )) {
    fail(
      'INVALID_CLAUDE_SETTINGS',
  ...
  if (forgeConfig?.options !== undefined && (
    !forgeConfig.options
    || typeof forgeConfig.options !== 'object'
    || Array.isArray(forgeConfig.options)
  )) {
    fail(
      'INVALID_CLAUDE_SETTINGS',
...
function writeClaudePluginOptions(settingsPath, hooks) {
  ...
  const pluginConfigs = settings.pluginConfigs || {};
  const forgeConfig = pluginConfigs[CURRENT_PLUGIN_ID] || {};
  const options = forgeConfig.options || {};
  ...
  if (installed.length === 1 && installed[0].scope !== scope) {
    fail(
      ...
      `${CURRENT_PLUGIN_ID} is already installed at ${installed[0].scope} scope. Use the scope migration workflow to move it to ${scope}.`,
      {
        observedScopes,
        recovery: [
          `forge setup --mode claude-plugin --scope ${scope} --move-scope --yes`,
        ],
      }
    );
  }
...
function assertSafeLocalInventory(options) {
  ...
  if (manual) {
    fail(
      ...
      `A manual FORGE plugin layout exists at ${manual.manifestPath}. Remove or migrate the manual install before setup.`
    );
  }
  let managedInstalls;
  try {
  ...
  if (overlap) {
    fail(
      ...
      `Managed FORGE content at ${overlap.statePath} overlaps the Claude plugin. Remove that managed overlap before setup.`
    );
  }
...
function ensureOfficialMarketplace(options) {
  ...
  if (!verified || !isOfficialMarketplace(verified)) {
    fail(
      ...
      'Could not verify the official FORGE marketplace after the marketplace change.',
