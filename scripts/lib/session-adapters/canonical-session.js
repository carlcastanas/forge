...
const SESSION_SCHEMA_VERSION = 'forge.session.v1';
const SESSION_RECORDING_SCHEMA_VERSION = 'forge.session.recording.v1';
const DEFAULT_RECORDING_DIR = path.join(os.tmpdir(), 'forge-session-recordings');
...
function resolveRecordingDir(options = {}) {
  ...
  if (typeof process.env.FORGE_SESSION_RECORDING_DIR === 'string' && process.env.FORGE_SESSION_RECORDING_DIR.length > 0) {
    return path.resolve(process.env.FORGE_SESSION_RECORDING_DIR);
  }
