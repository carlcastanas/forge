 ...
 *
 * Controls:
 * - FORGE_HOOKS_ENABLED=true|false (default: true)
 * - FORGE_HOOK_PROFILE=minimal|standard|strict (default: standard)
 * - FORGE_DISABLED_HOOKS=comma,separated,hook,ids
 *
 * Claude plugin options are used when their corresponding FORGE variable is
 * absent. A managed install can provide forge/setup.json as the final fallback.
 */
...
function readManagedHookConfig(env = process.env) {
  const pluginRoot = String(
    env.CLAUDE_PLUGIN_ROOT || env.FORGE_PLUGIN_ROOT || ''
  ).trim();
  const configPath = String(env.FORGE_HOOK_CONFIG || '').trim()
    || (pluginRoot ? path.join(pluginRoot, 'forge', 'setup.json') : '');
  ...
  try {
    ...
    process.stderr.write(`${sanitizeDiagnostic(
      `Warning: unable to read managed FORGE hook config at ${configPath}: ${error.message}`
    )}\n`);
    return {};
  }
}
...
function areHooksEnabled(env = process.env, managed = readManagedHookConfig(env)) {
  const raw = env.FORGE_HOOKS_ENABLED !== undefined
    ? env.FORGE_HOOKS_ENABLED
    : (
...
function getHookProfile(env = process.env, managed = readManagedHookConfig(env)) {
  const selected = env.FORGE_HOOK_PROFILE !== undefined
    ? env.FORGE_HOOK_PROFILE
    : (
...
function getDisabledHookIds(env = process.env) {
  const raw = String(env.FORGE_DISABLED_HOOKS || '');
