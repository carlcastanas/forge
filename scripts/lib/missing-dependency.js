...
// Runtime dependencies that can be missing when FORGE is installed via the
...
function describeMissingDependencyError(error) {
  ...
  return (
    `Missing dependency '${moduleName}'. FORGE's production dependencies aren't installed ` +
    '(this happens when FORGE was installed via the Claude Code plugin marketplace, which ' +
    'clones the repo but never runs npm install). Run "npm install" from the FORGE repo ' +
