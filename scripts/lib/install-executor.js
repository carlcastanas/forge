...
const CLAUDE_FORGE_NAMESPACE = 'forge';
...
function addCursorAgentDataScaffoldOperations(operations, options) {
  ...
  addFileCopyOperation(operations, {
    ...
    sourceRelativePath: path.join('scaffolds', 'cursor', 'forge-agent-data.json'),
    destinationPath: path.join(options.targetRoot, 'forge-agent-data.json'),
  ...
  addFileCopyOperation(operations, {
    moduleId: options.moduleId,
    sourceRoot: options.sourceRoot,
    sourceRelativePath: path.join('scaffolds', 'cursor', 'rules', 'forge-agent-data-home.mdc'),
    destinationPath: path.join(options.targetRoot, 'rules', 'forge-agent-data-home.mdc'),
    strategy: 'preserve-relative-path'
  });
...
function planClaudeStyleLegacyInstall(context, { adapterId, adapterRootInput, rulesDir: rulesDirOverride }) {
  ...
  const rulesDir = rulesDirOverride || path.join(targetRoot, 'rules', CLAUDE_FORGE_NAMESPACE);
