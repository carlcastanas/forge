...
const ADAPTER_RECORDS = Object.freeze([
  ...
  },
  {
    ...
    supported_assets: [
      'Pi package manifest',
      'canonical FORGE skills (skills/)',
      'canonical FORGE commands as prompt templates (commands/)',
      'canonical FORGE engineering rules (rules/common/) injected into the system prompt',
      'session lifecycle hook adapter',
      '/forge-doctor diagnostics command',
    ],
    unsupported_surfaces: [
      'Subagents, chains, approval prompts, and persistent todos require companion Pi packages and are not part of this adapter',
      'Pi core has no MCP surface, though FORGE MCP configs load verbatim through the community pi-mcp-adapter package, which FORGE neither installs nor depends on',
    ],
    install_or_onramp: ['`pi install git:github.com/carlcastanas/forge`', '`pi install /path/to/FORGE` from a local checkout'],
    verification_commands: [
  ...
  },
  {
    ...
    unsupported_surfaces: ['No full FORGE hook parity; ecosystem ports must document drift from upstream FORGE'],
  ...
  },
  {
    ...
    unsupported_surfaces: ['No FORGE installer or direct adapter today'],
    ...
    risk_notes: ['Do not import product-specific assumptions; convert lessons into FORGE event fields.'],
    last_verified_at: '2026-05-12',
    owner: 'FORGE maintainers',
  ...
  },
  {
    ...
    verification_commands: ['`npm run observability:ready`'],
    risk_notes: ['Keep FORGE portable; do not require a desktop workspace to get basic value.'],
    last_verified_at: '2026-05-12',
    owner: 'FORGE maintainers',
    source_docs: ['docs/architecture/cross-harness.md'],
  },
