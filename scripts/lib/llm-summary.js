 ...
 * own authentication.
 *
 * Recursion guard: sets FORGE_SKIP_LLM_SUMMARY=1 in subprocess env so any Stop
...
function getLLMModel() {
  return process.env.FORGE_LLM_SUMMARY_MODEL || 'haiku';
}

function getContextThreshold() {
  const raw = parseInt(process.env.FORGE_LLM_SUMMARY_CONTEXT_THRESHOLD || '20', 10);
...
function generateSessionSummary(transcriptPath) {
  if (process.env.FORGE_SKIP_LLM_SUMMARY) return null;
  ...
  try {
    const result = spawnSync('claude', ['--model', getLLMModel(), '-p'], {
      ...
      env: {
        ...
        FORGE_LLM_SUMMARY_SUBPROCESS: '1'
      },
