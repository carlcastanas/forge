...
const SCHEMA = 'forge.codex-legacy-sync.v1';
const BEGIN_MARKER = '<!-- BEGIN FORGE -->';
const END_MARKER = '<!-- END FORGE -->';

function getStatePath(codexHome) {
  return path.join(codexHome, 'forge', 'legacy-sync-state.json');
}
...
function removeOpenedRegularFile(filePath, opened) {
  const quarantineDir = fs.mkdtempSync(path.join(path.dirname(filePath), '.forge-remove-'));
...
function listLegacyCandidates(codexHome) {
  ...
  if (fs.existsSync(promptsDir)) {
    for (const entry of fs.readdirSync(promptsDir)) {
      if (entry.startsWith('forge-') || entry.startsWith('forge_') || entry.includes('forge-rules-pack')) {
  ...
  for (const relativePath of [
    ...
    'forge-prompts-manifest.txt',
    'forge-extension-prompts-manifest.txt',
  ]) {
  ...
  if (!state) {
    ...
    try {
      ...
      if (openedAgents) {
        ...
        if (stripped !== content) {
          plannedRemovals.push(`${agentsPath}#forge-marker-block`);
          if (!dryRun) {
