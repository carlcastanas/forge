/**
 * Resolve FORGE agent data home (memory persistence root) across harnesses.
 *
 ...
 * purpose, consistent with FORGE script modules elsewhere. Automated PR reviewers
 ...
 *
 * @see https://github.com/carlcastanas/forge/issues/2065
 */
...
const AGENT_DATA_HOME_ENV = 'FORGE_AGENT_DATA_HOME';
const DEFAULT_CLAUDE_DIR_NAME = '.claude';
const DEFAULT_CURSOR_FORGE_DIR_SEGMENTS = ['.cursor', 'forge'];
const PROJECT_CONFIG_RELATIVE = path.join('.cursor', 'forge-agent-data.json');
 ...
 * dependency and risk divergent defaults for `~/.cursor/forge` vs `~/.claude`.
 *
...
/**
 * Project root for a config file under .cursor/forge-agent-data.json.
 */
...
function getDefaultCursorAgentDataHome() {
  return path.join(getHomeDirFromEnv(), ...DEFAULT_CURSOR_FORGE_DIR_SEGMENTS);
}
...
function warnUnsafeProjectConfig() {
  console.error(
    '[FORGE] Ignoring unsafe agent data project config: agentDataHome must stay ' +
    'within the default Cursor or Claude data directories. Use ' +
    'FORGE_AGENT_DATA_HOME for an explicit trusted override.'
  );
}
...
function readProjectConfigAt(configPath) {
  ...
  try {
    ...
    const candidate = parsed.agentDataHome || parsed.FORGE_AGENT_DATA_HOME;
    ...
    console.error(
      `[FORGE] Failed to read or parse agent data config at ${configPath}: ${error.message}`
    );
    return null;
  }
}
...
/**
 * Set FORGE_AGENT_DATA_HOME on the current process when unset (hook subprocess safety net).
...
function getCursorSessionEnvPayload(options = {}) {
  ...
  return {
    FORGE_AGENT_DATA_HOME: agentDataHome,
  };
}
...
module.exports = {
  ...
  DEFAULT_CURSOR_FORGE_DIR_SEGMENTS,
