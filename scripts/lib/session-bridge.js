...
/**
 * Shared session bridge utilities for FORGE hooks.
 *
...
function getBridgePath(sessionId) {
  return path.join(os.tmpdir(), `forge-metrics-${sessionId}.json`);
}
 ...
 * writers (e.g. PostToolUse `forge-metrics-bridge` and the background
 * `forge-statusline`, both writing to the same session bridge) do not
 ...
 * `scripts/hooks/forge-metrics-bridge.js` (commit 9b1d8918) for the
...
function resolveSessionId() {
  const raw = process.env.FORGE_SESSION_ID || process.env.CLAUDE_SESSION_ID || '';
