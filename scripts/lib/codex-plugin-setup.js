...
function parseMarketplaceInventory(stdout, phase) {
  ...
  const forgeEntries = inventory.marketplaces.filter(
    marketplace => marketplace.name === OFFICIAL_MARKETPLACE_NAME
  );
  if (forgeEntries.length > 1) {
    fail(
      'INVALID_MARKETPLACE_INVENTORY',
      'Codex marketplace inventory contains duplicate `forge` entries',
      { phase }
    );
  }
...
function parsePluginInventory(stdout, phase) {
  ...
  const forgeEntries = inventory.installed.filter(
...
async function runCodexCommand(args, options = {}, dependencies = {}) {
  ...
  try {
    ...
    if (error?.code === 'ENOENT') {
      fail(
        'CODEX_NOT_FOUND',
        'Codex CLI is not installed or `codex` is not on PATH. Install Codex, then rerun FORGE setup.',
        { argv, phase: options.phase }
      );
    }
...
async function resolveMarketplaceRepository(marketplace, options = {}, dependencies = {}) {
  ...
  try {
    ...
    fail(
      ...
      `Refusing the existing \`forge\` marketplace because its Git provenance could not be verified${detail ? `: ${detail}` : ''}.`,
      { phase: options.phase || 'marketplace-provenance' }
    );
  }
...
async function assertOfficialMarketplace(
  ...
  try {
    ...
    fail(
      'MARKETPLACE_COLLISION',
      `Refusing the existing \`forge\` marketplace because its provenance could not be verified${detail ? `: ${detail}` : ''}.`,
      { phase }
    );
  }
...
function parseMarketplaceUpgradeResult(stdout, marketplace) {
  ...
  if (
    ...
    fail(
      ...
      'Codex did not confirm that the official FORGE marketplace was refreshed.',
      { phase, argv }
    );
  }
  return result;
