 ...
 *
 * Visual language mirrors the FORGE web dashboard (scripts/dashboard-web.js):
...
// air-gapped mirror) via FORGE_PLAN_CANVAS_MERMAID_URL. If the fetch fails, the
...
function mermaidUrl(env = process.env) {
  const override = env.FORGE_PLAN_CANVAS_MERMAID_URL;
...
// the FORGE canvas. Kept import-only so a CDN failure degrades gracefully.
...
(() => {
  ...
  const QKEY = 'forge-plan-canvas:queue:' + key;
  let queue = [];
  ...
  // --- theme ---------------------------------------------------------
  const themeKey = 'forge-plan-canvas:theme';
  ...
  // above and below; match it. See carlcastanas/FORGE#2702.
  function readTheme() {
...
// FORGE-styled document template for rendered markdown plan artifacts.
