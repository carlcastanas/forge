...
const CURRENT_PLUGIN_SLUG = 'forge';
const LEGACY_PLUGIN_SLUG = 'forge';
...
// Artifacts that identify a COMPLETE FORGE root when the caller gives no explicit
// probe. A real FORGE root ships both the script tree AND FORGE's skills; a partial
...
// be present with none of FORGE's skills — so we probe for a sentinel skill that
// ships in every FORGE root and is exactly what the failing skill commands need.
...
/**
 * Resolve the FORGE source root directory.
 *
 * Tries, in order:
 ...
 *   4. Plugin cache auto-detection — scans ~/.claude/plugins/cache/{forge,forge}/
 ...
 *                                    FORGE script tree and a sentinel FORGE skill,
...
function resolveEccRoot(options = {}) {
  ...
  // Decide whether a candidate directory is a usable FORGE root. An explicit
  ...
  // candidate must contain both FORGE's scripts and a sentinel FORGE skill —
