...
function readSettingsSnapshot(settingsPath) {
  ...
  try {
    ...
    try {
      ...
      if (error && error.code === 'ENOENT') {
        error.code = 'FORGE_SETTINGS_CHANGED';
      }
      throw error;
    }
    if (
...
function assertSettingsSnapshotUnchanged(settingsPath, snapshot) {
  ...
  try {
    ...
    error.code = error.code || 'FORGE_SETTINGS_CHANGED';
    throw error;
  }
  ...
  const update = () => {
    ...
    const validateParent = () => {
      ...
      if (
        ...
        error.code = 'FORGE_SETTINGS_PARENT_CHANGED';
        throw error;
      }
    };
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      try {
        ...
        if (error.code !== 'FORGE_SETTINGS_CHANGED' || attempt === maxAttempts) {
          throw error;
        }
      }
    }
