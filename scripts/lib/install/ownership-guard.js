 ...
 * as an FORGE-managed operation in the previous install-state, the operation is
...
function prepareUserOwnedFileGuard(plan, migration) {
  ...
  for (const operation of (migration && migration.appliedOperations) || []) {
    if (
      ...
      warnings.push(
        `Skipped user-owned file ${operation.destinationPath}: the existing file is not recorded in FORGE install-state.`
      );
      continue;
    }
  ...
  // would claim ownership of a file FORGE did not create and make uninstall
  // delete it (#2964).
