...
function assertSafeInstallOperation(plan, operation) {
  ...
  assertWithinTrustedRoot(operation.destinationPath, targetRoot, 'install FORGE file');
  ...
  for (const segmentIndex of Array.from({ length: segments.length + 1 }, (_value, index) => index)) {
    ...
    try {
      ...
      if (stats.isSymbolicLink()) {
        throw new Error(
          `Refusing to install FORGE file through symlinked path: '${currentPath}'.`
        );
      }
    } catch (error) {
    ...
    if (operation.kind === 'remove-claude-settings-hooks') {
      ...
      if (removal.retained.length > 0) {
        throw new Error(
          ...
          + 'run the FORGE uninstaller to review retained entries.'
        );
      }
    } else {
...
function applyInstallPlanLocked(plan, dependencies = {}, settingsLockHeld = false) {
  ...
  const disabledServers = parseDisabledMcpServers(process.env.FORGE_DISABLED_MCPS);
  ...
  try {
    ...
    if (antigravityMigration.detected && !antigravityMigration.complete) {
      antigravityMigrationWarnings = [
        'Legacy Antigravity migration is incomplete. FORGE preserved modified, unverifiable, or unmanaged content under .agent; review and move anything you want to keep, then rerun the Antigravity install.',
  ...
  try {
    ...
    if (opencodeMigration.detected && !opencodeMigration.complete) {
      opencodeMigrationWarnings = [
        'Legacy OpenCode migration is incomplete. FORGE preserved modified or unverifiable managed content under ~/.opencode; review it and rerun the OpenCode install.',
