...
const SNAPSHOT_SCHEMA_VERSION = 'forge.control-pane.snapshot.v1';
const DEFAULT_STATE_STORE_RELATIVE_PATH = path.join('.claude', 'forge', 'state.db');
...
function defaultDbPath(env = process.env) {
  return path.join(homeDir(env), '.claude', 'forge2.db');
}
...
function defaultConfigPaths(cwd = process.cwd(), env = process.env) {
  ...
  const paths = [path.join(home, 'Library', 'Application Support', 'forge2', 'config.toml'), path.join(home, '.config', 'forge2', 'config.toml'), path.join(home, '.claude', 'forge2.toml')];

  let current = path.resolve(cwd);
  while (current && current !== path.dirname(current)) {
    paths.push(path.join(current, '.claude', 'forge2.toml'));
    paths.push(path.join(current, 'forge2.toml'));
...
function readTomlConfig(configPath) {
  ...
  // file (e.g. `--help`, or a first run before any forge2.toml exists) don't
...
function resolveControlPaneConfig(options = {}) {
  ...
  return {
    ...normalizeConfig(merged, {
      env,
      dbPath: options.dbPath || env.FORGE2_DB_PATH || null,
      stateDbPath: options.stateDbPath || env.FORGE_STATE_DB_PATH || null
    }),
...
async function openSqlDatabase(dbPath) {
  ...
  // forge2.db (e.g. `--help`, or a first run before any db exists) don't need
