...
// Host/Origin gating lives in scripts/lib/loopback-guard.js so every FORGE
...
function usage() {
  return [
    'Usage:',
    '  node scripts/control-pane.js [--host 127.0.0.1] [--port 8765] [--db <forge2.db>] [--state-db <state.db>] [--config <forge2.toml>] [--query <text>]',
    '',
    'Options:',
    '  --state-db <path>  Read agent work items from an FORGE state-store database',
...
function createControlPaneServer(options = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === 'GET' && requestUrl.pathname === '/assets/forge-icon.svg') {
        const iconPath = path.join(repoRoot, 'assets', 'forge-icon.svg');
