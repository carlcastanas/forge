...
<head>
  ...
  <title>FORGE Control Pane</title>
  <style>
    :root {
      color-scheme: dark;
      --bg: #101312;
...
<body>
  <div class="shell">
    <header>
      <div class="brand">
        <img src="/assets/forge-icon.svg" alt="">
        <h1>FORGE Control Pane</h1>
      </div>
  ...
  <script>
    ...
    function renderSessions(sessions) {
      if (!sessions.length) {
        $('#sessions').innerHTML = '<div class="empty">No FORGE2 sessions found.</div>';
        return;
      }
      ...
      document.querySelectorAll('#work-items [data-wi-action]').forEach(button => {
        button.addEventListener('click', () => {
          ...
          if (button.getAttribute('data-wi-action') === 'claim') {
            ...
            forgeMoveItem(id, button.getAttribute('data-wi-lane'));
          }
        });
      });
    }
    ...
    window.forgeClaimItem = function (id) {
    ...
    window.forgeMoveItem = function (id, lane) {
      if (!state.allowActions) return;
