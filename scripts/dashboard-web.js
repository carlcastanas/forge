#!/usr/bin/env node
/**
 * FORGE Capabilities Dashboard — agents, skills, commands, MCPs, rules & hooks
...
function resolveDashboardHost(env = process.env) {
  const configured = String(env.FORGE_DASHBOARD_HOST || '').trim().toLowerCase();
  if (!configured) return DEFAULT_HOST;
  if (!LOOPBACK_HOSTNAMES.has(configured)) {
    throw new Error(
      '[FORGE] FORGE_DASHBOARD_HOST must be loopback-only ' +
...
function parsePort(v) {
  ...
  if (isNaN(n) || n < 1 || n > 65535) { console.error('[FORGE] Invalid port: ' + v + ' — using 3456'); return 3456; }
  return n;
}
const PORT = parsePort(process.argv[2] || process.env.FORGE_DASHBOARD_PORT || '3456');
...
function loadPostToolUseChildren(root) {
  ...
  try {
    ...
    console.error('[FORGE] Failed to load PostToolUse dispatcher registry:', error.message);
    return [];
  }
}
...
<title>FORGE Capabilities</title>
<style>
  :root {
...
<div class="header">
  <div class="brand" id="brand-link">
    ...
    <h1><span id="t-title">FORGE Capabilities</span> <span class="ver">v2.0.0-rc.1</span></h1>
  </div>
...
<div class="footer">
  ...
  <span class="dt"></span>
  <span id="t-contribution">Contribution to FORGE</span>
  <span class="dt"></span>
...
// Recently viewed
function recents() { try { return JSON.parse(localStorage.getItem('forge-recent') || '[]'); } catch { return []; } }
function addRecent(type, name) {
  ...
  localStorage.setItem('forge-recent', JSON.stringify(r));
}
function clearRecents() { localStorage.removeItem('forge-recent'); location.hash=''; location.reload(); }
...
// Language
function setLang(l) {
  lang = l; localStorage.setItem('forge-lang', l);
...
function toggleTheme() {
  ...
  localStorage.setItem('forge-theme', h.dataset.theme);
}
if (localStorage.getItem('forge-theme')) document.documentElement.dataset.theme = localStorage.getItem('forge-theme');
...
function createDashboardServer({
  ...
  const resolvedHost = resolveDashboardHost({ FORGE_DASHBOARD_HOST: host });
  ...
  return http.createServer((req, res) => {
    ...
    if (url.pathname === '/api/data') {
      ...
      try {
        ...
        reportDashboardFailure(
          reportError,
          '[FORGE] Failed to load dashboard data:',
          error
        );
    ...
    try {
      ...
      reportDashboardFailure(
        reportError,
        '[FORGE] Failed to render dashboard:',
        error
      );
      return sendHtml(
        res,
...
if (require.main === module) {
  listenDashboardServer(server, { port: PORT, host: HOST, onListening: () => {
    ...
    console.log(`\n    FORGE Capabilities  →  ${dashboardUrl}\n`);
