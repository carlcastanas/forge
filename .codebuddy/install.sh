#!/bin/bash
#
# FORGE CodeBuddy Installer
# Installs FORGE workflows into a CodeBuddy project.
#
# Usage:
...
# Locate the FORGE repo root by walking up from SCRIPT_DIR to find the marker
...
if [ -z "$REPO_ROOT" ]; then
    echo "Error: Cannot locate the FORGE repository root."
    echo "This script must be run from within the FORGE repository's .codebuddy/ directory."
    exit 1
fi
...
do_install() {
    ...
    echo "FORGE CodeBuddy Installer"
    echo "======================="
    echo ""
    ...
    MANIFEST="$codebuddy_full_path/.forge-manifest"
    touch "$MANIFEST"
    ...
    ensure_manifest_entry "$MANIFEST" ".forge-manifest"
    ...
    echo "  3. Enjoy the FORGE workflows!"
    echo ""
