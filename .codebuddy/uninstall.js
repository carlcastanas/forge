#!/usr/bin/env node
/**
 * FORGE CodeBuddy Uninstaller (Cross-platform Node.js version)
 * Uninstalls FORGE workflows from a CodeBuddy project.
 *
 * Usage:
...
async function doUninstall() {
  ...
  console.log('FORGE CodeBuddy Uninstaller');
  console.log('==========================');
  console.log('');
  ...
  const manifest = path.join(codebuddyFullPath, '.forge-manifest');

  // Handle missing manifest
  if (!fs.existsSync(manifest)) {
    console.log('Warning: No manifest file found (.forge-manifest)');
    console.log('');
    console.log('This could mean:');
    console.log('  1. FORGE was installed with an older version without manifest support');
  ...
  console.log('Found manifest file - will only remove files installed by FORGE');
  console.log('');

  const confirmed = await promptConfirm(`Are you sure you want to uninstall FORGE from ${codebuddyDirName}? (y/N) `);
  if (!confirmed) {
    console.log('Uninstall cancelled.');
    process.exit(0);
  }
