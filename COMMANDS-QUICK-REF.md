...
These are `forge` CLI commands, not slash commands. They use one inspectable
Markdown vault across Claude, Codex, Hermes, OpenClaw, Kimi, and other
harnesses.

| Command | What it does |
|---------|-------------|
| `forge memory init` | Create project, team, or user vault directories |
| `forge memory save` | Create an unreviewed context, decision, fact, lesson, note, preference, or runbook |
| `forge memory handoff` | Transfer bounded work state from one harness to another |
| `forge memory search` | Search memories by text, scope, kind, or target harness |
| `forge memory read` | Read a memory and its backlinks by stable ID |
| `forge memory doctor` | Report malformed files, duplicate IDs, broken links, and skipped symlinks |
| `forge-memory-mcp` | Start the optional local stdio MCP server |
...
These lifecycle commands are also available through the `forge` CLI.

| Command | What it does |
|---------|-------------|
| `forge list-installed` | Show installs recorded in FORGE's managed state |
| `forge doctor` | Diagnose missing or drifted managed files and point failures to the short problem form |
| `forge repair` | Restore missing or drifted managed files |
| `forge uninstall` | Remove only install-state-managed files and optionally show the 20-second exit-feedback route |
| `forge feedback` | Show the public problem, quick-feedback, and feature routes without reading files or uploading diagnostics |
...
| Command | What it does |
|---------|-------------|
| `/forge-guide` | Navigate FORGE's current agents, skills, commands, hooks, install profiles, and docs from the live repository surface |
...
| `/project-init` | Detect a project's stack and produce a dry-run FORGE onboarding plan |
...
| `/auto-update` | Pull the latest FORGE repo changes and reinstall the current managed targets |
| `/cost-report` | Generate a local Claude Code cost report from a cost-tracker SQLite database |
| `/security-scan` | Run Forge Shield against agent, hook, MCP, permission, and secret surfaces |
