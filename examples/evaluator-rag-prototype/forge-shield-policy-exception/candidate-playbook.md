# Forge Shield Policy Exception Playbook

Candidate id: `sarif-backed-timeboxed-exception-review`

Use this playbook when Forge Shield organization-policy output produces a
finding that may need remediation, a time-boxed exception, or explicit
enforcement.

## Accepted Path

1. Identify the Forge Shield finding id, category, severity, affected file or
   MCP/hook surface, and policy pack or organization baseline.
2. Retrieve scanner evidence before judgment:
   - SARIF/code-scanning result, especially `forge-shield-policy/*`
...
5. Decide whether immediate remediation is possible. If not, only promote a
   ...
   expiry.
6. Keep Forge Shield code, policy packs, enforcement settings, release state,
...
- Forge Shield SARIF/code-scanning artifact or report evidence
- `npx forge-shield scan --format html` when executive review evidence is
  needed
