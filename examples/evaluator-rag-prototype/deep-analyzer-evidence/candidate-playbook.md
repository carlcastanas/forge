...
2. Retrieve the Deep Analyzer Evidence contract from `README.md`
   and the follow-up logic in `src/lib/analyzer.ts`.
3. Match the change to maintained corpus or reference evidence:
   - `src/analyzers/fixtures/deep-analyzer-corpus.ts`
   - `src/analyzers/deep-analyzer-corpus.test.ts`
   - `src/lib/analyzer.compare.test.ts`
...
5. Add or update analyzer corpus, expected-output snapshots, fixtures,
   ...
   surface.
6. Run the relevant validation gate in the analyzer repository:
