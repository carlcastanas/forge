/**
 * Nine skills, chosen at build time from the real catalog — a full three-by-three
 * grid, so the last row is not two cards and a hole.
 *
...
const SLOTS: Slot[] = [
  ...
  {
    domain: 'Data',
    candidates: ['postgres-patterns', 'data-quality-validation', 'analytics-instrumentation'],
    fallback: ['postgres', 'data'],
  },
  {
    domain: 'Operations',
...
export function SkillsSampler() {
  ...
  return (
    // Same card shape as the skills page: a skill looks like a skill wherever it
    // is shown, so the two lists are not two components.
    <div className="grid grid--3">
      {picks.map(({ slot, skill }) => (
        <Link className="card" href={`/skills/${skill.name}`} key={skill.name}>
          ...
          <p className="card__body u-clamp-3">{truncate(skill.description, 210)}</p>
          <span className="badge card__badge">{slot.domain}</span>
        </Link>
      ))}
    </div>
  );
