...
export function InstallBuilder({ data }: { data: InstallData }) {
  ...
  /**
   * The page opens on the profile the manifests describe as the one for most
   * users, not on an empty selection. Landing on nothing selected put a command
   * that installs nothing behind a Copy button in a fixed bar, which reads as a
   * broken page rather than as a blank slate. "no profile" is still one click
   * away and Clear still goes there.
   */
  const seedProfile = useMemo(
    () =>
      data.profiles.find((profile) => profile.id === 'developer')?.id ??
      data.profiles.find((profile) => profile.id === 'core')?.id ??
      data.profiles[0]?.id ??
      null,
    [data.profiles],
  );
  ...
  const [selection, setSelection] = useState<Selection>(() => {
    ...
    return {
      profileId: profileId && profileById.has(profileId) ? profileId : seedProfile,
      ...
      <section className="ib-step" aria-labelledby="ib-step-refine">
        ...
        <div className="ib-controls">
          ...
          <div className="filter-bar" role="group" aria-label="Filter by family">
            ...
            >
              All <span className="u-muted">{data.totals.components}</span>
            </button>
            {data.families.map((family) => (
              ...
              >
                {family.label} <span className="u-muted">{family.count}</span>
              </button>
            ))}
            <button
              type="button"
            ...
            >
              Selected only <span className="u-muted">{selectedIds.size}</span>
            </button>
          </div>
