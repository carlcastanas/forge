...
/**
 * The repository intro and the generated index both carry a section called
 * Security. Two identical entries in a contents list read as a duplicate rather
 * than as two destinations, so the first one wins.
 */
...
export default async function DocsIndexPage() {
  ...
  /**
   * The rail has to describe the whole page, not only the part of it that came
   * out of a markdown file. Without the group headings the reader sees a
   * contents list that stops halfway down and no way to reach the index below.
   */
  const headings = dedupeByText([
    ...(intro ? extractHeadings(intro) : []),
    ...groups.map((group) => ({ depth: 2 as const, text: group.label, id: groupId(group.label) })),
  ]);
                ...
                {group.items.map((item) => (
                  <Link className="card" href={item.href} key={item.href}>
                    ...
                    {item.description ? (
                      ...
                      <p className="card__body u-muted">No summary yet.</p>
                    )}
