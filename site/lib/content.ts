...
function stripLeadingH1(content: string): string {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    // A horizontal rule or a shields.io badge row sits between the README's
    // header furniture and its H1. Neither is content; step over them so the
    // H1 is still found and removed, rather than rendering under the page title.
    if (/^\s{0,3}(-{3,}|_{3,}|\*{3,})\s*$/.test(line)) continue;
...
/**
 * Removes a `##` section, heading and body, up to the next heading of the same
 * level or higher. Fenced code is respected so a `## ` inside a block does not
 * end the section early.
 */
function dropSection(content: string, heading: RegExp): string {
  const lines = content.split('\n');
  const out: string[] = [];
  let dropping = false;
  let inFence = false;
  ...
  for (const line of lines) {
    ...
    if (!inFence) {
      ...
      if (match) {
        ...
        if (!dropping && level === 2 && heading.test(match[2].trim())) {
          dropping = true;
          continue;
        }
      }
...
/**
 * The guides index renders its own list of guides, counted and timed from the
 * files themselves. The repository README also carries a hand-maintained table
 * of the same eight, and the two disagree about read times — the file says 20
 * minutes where the word count says 12. One list per page, and it is the one
 * that cannot go stale.
 */
export function getGuidesIndexIntro(): string | null {
  const raw = safeReadFile(path.join(REPO_ROOT, 'guides', 'README.md'));
  if (raw === null) return null;
  const stripped = stripLeadingH1(stripLeadingHtmlBlocks(parse(raw).content));
  const body = dropSection(stripped, /^the\s+(\w+\s+)?guides$/i).trim();
  return body.length > 0 ? body : null;
}
