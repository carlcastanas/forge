# FORGE 2.0 Session Adapter Discovery

## Purpose

This document turns the March 11 FORGE 2.0 control-plane direction into a
...
- `scripts/lib/session-adapters/canonical-session.js`
  defines the canonical `forge.session.v1` normalization layer
...
In practice, FORGE can already answer:
...
general FORGE 2.0 control plane.
...
## Gap Between FORGE 1.x And FORGE 2.0

FORGE 1.x currently has two different "session" surfaces:
...
The missing FORGE 2.0 layer is a harness-neutral session adapter boundary that
can normalize:
...
FORGE 2.0 should introduce a canonical session adapter contract.
...
This provides a non-orchestrated baseline for FORGE 2.0.
...
3. Should event streaming be part of v1, or should FORGE 2.0 ship snapshot-only
   first?
...
5. Should the adapter registry live inside this repo long-term, or move into the
   eventual FORGE 2.0 control-plane app once the interface stabilizes?
...
The shortest path to FORGE 2.0 is:
