...
| Evolved | `${XDG_DATA_HOME:-~/.local/share}/forge-homunculus/evolved/skills/` (global) or `${XDG_DATA_HOME:-~/.local/share}/forge-homunculus/projects/<hash>/evolved/skills/` (per-project) | No | Inherits from instinct source |
...
- No provenance file. Use `origin` in SKILL.md frontmatter (FORGE, community) for attribution.
...
Location: `${XDG_DATA_HOME:-~/.local/share}/forge-homunculus/evolved/skills/` (global) or `${XDG_DATA_HOME:-~/.local/share}/forge-homunculus/projects/<hash>/evolved/skills/` (per-project).

The data root resolves in this order: an explicit override, `$XDG_DATA_HOME/forge-homunculus`, then `$HOME/.local/share/forge-homunculus`. Installs that predate the move still have data at `~/.claude/homunculus` and can migrate once with `bash skills/continuous-learning-v2/scripts/migrate-homunculus.sh`.
...
| | `${XDG_DATA_HOME:-~/.local/share}/forge-homunculus/**/evolved/**` |
| | `~/.claude/homunculus/**/evolved/**` (pre-migration installs) |
