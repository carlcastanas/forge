...
an original, FORGE-native implementation of that idea, not a port.
...
- The artifact opens in the browser inside FORGE-styled chrome (same design tokens as
...
|---|---|---|
| CLI entry | `scripts/plan-canvas.js` (+ npm bin `forge-plan-canvas`) | `scripts/control-pane.js` |
...
| Mermaid diagrams | `scripts/lib/plan-canvas/ui.js` | ` ```mermaid ` blocks render in the browser, themed to FORGE; pinned CDN with offline fallback (`FORGE_PLAN_CANVAS_MERMAID_URL` for a local mirror) |
| Session state | `scripts/lib/plan-canvas/sessions.js` | file-path-keyed sessions, state under `~/.claude/plan-canvas/` (`FORGE_PLAN_CANVAS_STATE_DIR` override) |
...
from a bare `forge-plan-canvas` invocation).

Surfaces mirror how peer workflow-quality skills ship across FORGE's harnesses:
...
- The CLI resolves from any project via the `forge-plan-canvas` bin (global/plugin install) or
...
after 30 min, `FORGE_PLAN_CANVAS_IDLE_MS`). Feedback is deliver-and-drain: queued items are
handed to exactly one `await` call and persisted to disk until then, so nothing is lost if
the poll is interrupted.

- `GET /health` — `{ok, app: "forge-plan-canvas", version}` (CLI/server version handshake)
- `GET /` — session list (FORGE chrome)
...
- `GET /canvas/<key>` — editor chrome; `GET /artifact/<key>/` — rendered artifact
  (markdown → FORGE plan template, HTML passthrough) with the annotation SDK injected;
...
- Verdict actions wired to FORGE's plan-confirmation workflow.
- FORGE design tokens and chrome; JSON (not TOON) agent output.
- Mermaid renders themed to FORGE, but without lavish's pan/zoom or node-id capture —
  whole-element annotation covers pointing at a diagram or node.
- No export/share hosting, no layout-audit gate, no bundled playbooks — FORGE's existing
...
`FORGE_PLAN_CANVAS_MERMAID_URL`. The server itself still makes no network calls.
