...
harnesses can share. Project and team memories live under `.forge/memory/`; user
memories live under `~/.forge/memory/`. The same `forge.memory.v1` documents are
available through the `forge memory` CLI and an opt-in local stdio MCP server, so
...
- **FORGE CLI:** deterministic local create/read/search/doctor interface.
- **FORGE Memory MCP:** stdio adapter exposing the same create/read/search/doctor
  operations. It has no review or promotion tool.
- **FORGE2 context graph:** optional projection populated from the Markdown
...
`FORGE_MEMORY_PROJECT_ROOT` and `FORGE_MEMORY_USER_ROOT` may override the two vault
...
```text
forge memory init [--scope project|team|user]
forge memory save --title <text> [--body-file <path>|--stdin] [metadata flags]
forge memory handoff --from <harness> --target <harness> --title <text> ...
forge memory search <query> [--scope ...] [--target-harness ...] [--json]
forge memory read <id> [--scope ...] [--json]
...
The MCP server requires a lowercase `FORGE_MEMORY_HARNESS` identity at launch.
...
the operator also sets `FORGE_MEMORY_ALLOW_USER_SCOPE=1`, after which the client
...
the full body. A later event-sourced FORGE2 projection may record content hashes
...
- Replacing FORGE2 sessions, the context graph, GitHub/Linear work items, or
...
- Whether the team scope should gain a signed promotion manifest that points
  to governed artifacts after the FORGE2 append-only event substrate lands.
...
cross-harness tests. FORGE2 graph sync, automatic session capture, semantic
