> **翻译版本。** 本页落后于英文版。以 [README.md](../../README.md) 为准。
...
<td align="center"><b>安全指南</b><br/>攻击向量、沙盒化、净化、CVE、Forge Shield。</td>
</tr>
</table>
...
2.0 主线稳定版：261 个技能、control-pane 基底（会话适配器 + MCP 清单）、worktree 生命周期服务，以及 FORGE Discord 社区。

### v2.0.0-rc.1 — 表面同步、运营工作流与 FORGE 2.0 Alpha（2026年4月）
...
* **FORGE 2.0 alpha 已进入仓库** —— `forge2/` 下的 Rust 控制层现已可在本地构建，并提供 `dashboard`、`start`、`sessions`、`status`、`stop`、`resume` 与 `daemon` 命令。
* **生态加固持续推进** —— Forge Shield、FORGE Tools 成本控制、计费门户工作与网站刷新仍围绕核心插件持续交付。
...
* **平台优先发布** — FORGE 现在被明确构建为一个智能体平台性能系统，而不仅仅是一个配置包。
* **钩子可靠性大修** — SessionStart 根回退、Stop 阶段会话摘要，以及用基于脚本的钩子替换脆弱的单行内联钩子。
* **钩子运行时控制** — `FORGE_HOOK_PROFILE=minimal|standard|strict` 和 `FORGE_DISABLED_HOOKS=...` 用于运行时门控，无需编辑钩子文件。
...
### v1.6.0 — Codex CLI、Forge Shield 与市场（2026年2月）
...
* **Forge Shield 集成** — `/security-scan` 技能直接从 Claude Code 运行 Forge Shield；1282 项测试，102 条规则
* **GitHub 市场** — FORGE Tools GitHub 应用已在 github.com/marketplace/forge-tools 上线，提供免费/专业/企业版
...
* **交互式安装向导** — 新的 `configure-forge` 技能提供了带有合并/覆盖检测的引导式设置
...
`forge memory` 使用可检查的 `forge.memory.v1` Markdown 文档，在 Claude、
...
可选的 `forge-memory-mcp` 服务必须由操作者设置小写
`FORGE_MEMORY_HARNESS` 身份；工具调用方不能覆盖该身份。只有操作者另外设置
`FORGE_MEMORY_ALLOW_USER_SCOPE=1` 后，MCP 调用才能显式请求 `user` 范围。
该服务默认不会启用。

仅安装 skill、最小配置、手动复制或 Claude 插件不会把记忆库运行时加入
`PATH`。请先单独安装 FORGE npm 运行时：
...
`forge-memory-vault` 配置到对应 harness，并为每个 harness 分别启动一个服务
进程，例如 `FORGE_MEMORY_HARNESS=codex forge-memory-mcp`。不同 harness 可以共享
...
>
> 如果你已经通过 `/plugin install` 安装了 FORGE，**不要再运行 `./install.sh --profile full`、`.\install.ps1 --profile full` 或 `npx forge-universal install --profile full`**。插件已经会自动加载 FORGE 的技能、命令和 hooks；此时再执行完整安装，会把同一批内容再次复制到用户目录，导致技能重复以及运行时行为重复。
>
> 对于插件安装路径，请只手动复制你需要的 `rules/` 目录。只有在你完全不走插件安装、而是选择“纯手动安装 FORGE”时，才应该使用完整安装器。
...
# Fully manual FORGE install path (do this instead of /plugin install)
...
**搞定！** 你现在可以使用 68 个智能体、318 项技能和 94 个命令了。
...
|   |-- configure-forge/              # 交互式安装向导（新增）
|   |-- security-scan/              # Forge Shield 安全审计集成（新增）
...
[安装 GitHub 应用](https://github.com/apps/skill-creator) | forge.example.com
...
> cp -r forge/rules/typescript ~/.claude/rules/typescript   # 选择您的技术栈
> cp -r forge/rules/python ~/.claude/rules/python
> cp -r forge/rules/golang ~/.claude/rules/golang
> cp -r forge/rules/php ~/.claude/rules/php
>
> # 选项 B：项目级规则（仅适用于当前项目）
> mkdir -p .claude/rules
> cp -r forge/rules/common .claude/rules/common
...
cp -r forge/rules/common ~/.claude/rules/common
cp -r forge/rules/typescript ~/.claude/rules/typescript   # pick your stack
cp -r forge/rules/python ~/.claude/rules/python
cp -r forge/rules/golang ~/.claude/rules/golang
cp -r forge/rules/php ~/.claude/rules/php
...
| 我想要... | 使用此表面 | 使用的智能体 |
|--------------|-----------------|------------|
| 规划新功能 | `/forge:plan "Add auth"` | planner |
| 设计系统架构 | `/forge:plan` + architect agent | architect |
...
<details>
<summary><b>我能否在自定义API端点或模型网关上使用FORGE与Claude Code？</b></summary>

是的。FORGE 不会硬编码 Anthropic 托管的传输设置。它通过 Claude Code 正常的 CLI/插件接口在本地运行，因此可以与以下系统配合工作：
...
如果您的网关重新映射模型名称，请在 Claude Code 中配置，而不是在 FORGE 中。一旦 `claude` CLI 已经正常工作，FORGE 的钩子、技能、命令和规则就与模型提供商无关。
...
FORGE 提供**完整的 Cursor IDE 支持**，包括为 Cursor 原生格式适配的钩子、规则、代理、技能、命令和 MCP 配置。
...
FORGE 为 macOS 应用和 CLI 提供 **一流的 Codex 支持**，包括参考配置、Codex 特定的 AGENTS.md 补充文档以及共享技能。
...
Codex **尚未提供与 Claude 风格同等的钩子执行功能**。FORGE 在该平台上的强制执行是通过 `AGENTS.md`、可选的 `model_instructions_file` 覆盖以及沙箱/批准设置以指令方式实现的。
...
FORGE 提供 **完整的 OpenCode 支持**，包括插件和钩子。
...
| 技能 | PASS: 318 项   | PASS: 37 项 | **Claude Code 领先** |
...
该 npm 插件条目启用了 FORGE 发布的 OpenCode 插件模块（钩子/事件和插件工具）。
它**不会**自动将 FORGE 的完整命令/代理/指令目录添加到您的项目配置中。

要获得完整的 FORGE OpenCode 设置，您可以：
...
FORGE 是**第一个最大化利用每个主要 AI 编码工具的插件**。以下是每个平台的比较：

| 功能特性 | Claude Code           | Cursor IDE | Codex CLI | OpenCode |
|---------|-----------------------|------------|-----------|----------|
| **智能体** | 68                    | 共享 (AGENTS.md) | 共享 (AGENTS.md) | 12 |
| **命令** | 94                    | 共享 | 基于指令 | 35 |
| **技能** | 318                   | 共享 | 10 (原生格式) | 37 |
...
* Codex 通过原生 `SessionStart` 引导钩子初始化 FORGE；其余行为由 `AGENTS.md`、可选的 `model_instructions_file` 覆盖以及沙箱权限提供
