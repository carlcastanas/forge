...
### v1.0.0 — Instalação Guiada para Múltiplos Harnesses
...
Graduação estável da linha 2.0: 261 skills, substrato de control-pane, inventário MCP, serviço de ciclo de vida de worktrees e a comunidade no Discord.

### v2.0.0-rc.1 — Sincronização de Superfície, Fluxos Operacionais e FORGE 2.0 Alpha (Abr 2026)
...
- **FORGE 2.0 alpha já está no repositório** — o plano de controle em Rust dentro de `forge2/` já compila localmente e expõe `dashboard`, `start`, `sessions`, `status`, `stop`, `resume` e `daemon`.
- **Fortalecimento do ecossistema** — Forge Shield, controles de custo do FORGE Tools, trabalho no portal de billing e a renovação do site continuam sendo entregues ao redor do plugin principal.
...
- **Lançamento focado em harness** — O FORGE agora é explicitamente enquadrado como um sistema de desempenho de harness de agentes, não apenas um pacote de configurações.
...
- **Controles de runtime de hooks** — `FORGE_HOOK_PROFILE=minimal|standard|strict` e `FORGE_DISABLED_HOOKS=...` para controle em tempo de execução sem editar arquivos de hook.
