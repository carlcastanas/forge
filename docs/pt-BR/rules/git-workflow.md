...
Nota: As instalações gerenciadas pelo FORGE definem `"includeCoAuthoredBy": false` em `~/.claude/settings.json`, portanto os commits não incluem `Co-Authored-By` por padrão. Para manter a atribuição do Claude, defina `"includeCoAuthoredBy": true` ou configure `attribution`; o FORGE nunca sobrescreve uma escolha explícita.
