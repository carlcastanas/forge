...
<td align="center"><b>Güvenlik Rehberi</b><br/>Saldırı vektörleri, sandboxing, sanitizasyon, CVE'ler, Forge Shield.</td>
</tr>
</table>
...
### v1.0.0 — Rehberli Çoklu Harness Kurulumu
...
2.0 hattının kararlı sürümü: 261 skill, control-pane altyapısı, MCP envanteri, worktree yaşam döngüsü servisi ve Discord topluluğu.

### v2.0.0-rc.1 — Surface Sync, Operatör İş Akışları ve FORGE 2.0 Alpha (Nis 2026)
...
- **FORGE 2.0 alpha repoda** — `forge2/` altındaki Rust kontrol katmanı artık yerelde derleniyor ve `dashboard`, `start`, `sessions`, `status`, `stop`, `resume` ve `daemon` komutlarını sunuyor.
- **Ekosistem sağlamlaştırma** — Forge Shield, FORGE Tools maliyet kontrolleri, billing portal işleri ve web yüzeyi çekirdek plugin etrafında birlikte gelişmeye devam ediyor.
...
- **Harness-first release** — FORGE artık açıkça bir agent harness performans sistemi olarak çerçevelendi, sadece bir config paketi değil.
...
- **Hook runtime kontrolleri** — `FORGE_HOOK_PROFILE=minimal|standard|strict` ve `FORGE_DISABLED_HOOKS=...` hook dosyalarını düzenlemeden runtime gating için.
...
/forge:plan "Kullanıcı kimlik doğrulaması ekle"
...
|---------------------|---------------------|------------------|
| Yeni bir feature planla | `/forge:plan "Auth ekle"` | planner |
| Sistem mimarisi tasarla | `/forge:plan` + architect agent | architect |
...
```
/forge:plan "OAuth ile kullanıcı kimlik doğrulaması ekle"
