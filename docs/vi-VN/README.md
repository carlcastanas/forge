> **Bản dịch.** Trang này đi sau bản tiếng Anh. Nguồn chuẩn là [README.md](../../README.md).
...
> **140K+ sao** | **21K+ fork** | **170+ contributor** | **12+ hệ sinh thái ngôn ngữ**
...
**FORGE là hệ thống tối ưu hiệu năng cho AI agent harness.**
...
- **Không chồng nhiều cách cài lên nhau.** Cấu hình dễ hỏng nhất là `/plugin install` trước, rồi chạy tiếp `install.sh --profile full` hoặc `npx forge-universal install --profile full`.

Nếu bạn đã cài chồng nhiều lần và thấy skill/hook bị trùng, xem [Reset / Gỡ FORGE](#reset--gỡ-forge).
...
Các tên này cố ý khác nhau. Plugin Claude Code dùng `forge@forge`; npm vẫn dùng `forge-universal`.
...
Nếu FORGE bị trùng, quá xâm lấn, hoặc hoạt động sai, đừng tiếp tục cài đè lên chính nó.

- **Đường plugin:** gỡ plugin trong Claude Code, rồi xoá các rule folder bạn đã copy thủ công dưới `~/.claude/rules/forge/`.
...
Sau đó gỡ các file do FORGE quản lý:
...
FORGE chỉ xoá file có trong install-state của nó. Nó không xoá file không liên quan.
...
/forge:plan "Thêm xác thực người dùng"
...
FORGE hiện cung cấp hàng chục agent, hơn 200 skill và legacy command shim cho các workflow agent khác nhau. Kiểm tra README tiếng Anh để xem danh sách và hướng dẫn chi tiết nhất.
