...
Use this guide when you want FORGE behavior inside a harness that does not natively load `.claude/`, `.codex/`, `.opencode/`, `.cursor/`, or `.agent/` layouts.
...
Prefer a first-class FORGE target whenever one exists:
...
Use this guide only when you need FORGE behavior in a non-native harness.

## What You Are Reproducing

When you adapt FORGE manually, you are trying to preserve four things:
...
sed -n '1,220p' skills/tdd-workflow/SKILL.md > /tmp/forge-context.md
printf '\n\n---\n\n' >> /tmp/forge-context.md
sed -n '1,220p' skills/backend-patterns/SKILL.md >> /tmp/forge-context.md
printf '\n\n---\n\n' >> /tmp/forge-context.md
sed -n '1,220p' skills/security-review/SKILL.md >> /tmp/forge-context.md
```
...
Optional: if you already use a repo packer like `repomix`, it can help compress selected files into one handoff document. It is a convenience tool, not the canonical FORGE path.

## Compression Rules

When manually packing FORGE for another harness:
...
You are not implementing real commands. You are giving the harness explicit invocation handles that map to FORGE behavior.
...
That does not recreate true automation, but it captures the operational discipline of FORGE.

## Harness Capability Matrix

| Capability | First-Class FORGE Targets | Manual-Adaptation Targets |
| --- | --- | --- |
...
2. Pack the selected FORGE skill files into one upload or paste block.
...
```text
You are operating with a manually adapted FORGE bundle.
...
- use manual adaptation to carry FORGE behavior into non-native harnesses
- use native FORGE targets whenever you want the full system
