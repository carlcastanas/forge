...
This document defines the canonical FORGE session snapshot contract for
`forge.session.v1`.
...
FORGE has multiple session sources:
...
| Field | Type | Notes |
| --- | --- | --- |
| `schemaVersion` | string | MUST be exactly `forge.session.v1` for this contract |
...
Every FORGE session adapter MUST:

1. Emit `schemaVersion: "forge.session.v1"` exactly.
...
- rely only on documented fields for `forge.session.v1`
