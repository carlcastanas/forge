...
├──────────────────────────────────────────────────────────────┤
│ Security And Review Layer                                    │
│ Forge Shield policies/SARIF, policy packs, evidence bundles, │
│ Linear/GitHub sync, governance reports                       │
└──────────────────────────────────────────────────────────────┘
```
...
- Keep `docs/architecture/observability-readiness.md` as the operator-facing
  readiness gate.
- Define a versioned HUD/status JSON contract that FORGE2 and any external
  consumer can read.
...
5. Inventory legacy folders and closed-stale PRs into the salvage ledger.
6. Port useful stale work in small attributed maintainer PRs.

## Non-Goals

- Remote telemetry before the local event model is useful and testable.
...
- Release announcements before command and package evidence is fresh.
