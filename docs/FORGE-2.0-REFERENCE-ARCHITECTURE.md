...
| Reference | Primary pressure on FORGE 2.0 | Concrete FORGE delta |
| --- | --- | --- |
...
| [`jarrodwatts/claude-hud`](https://github.com/jarrodwatts/claude-hud) | Always-visible Claude Code statusline for context, tools, agents, todos, and transcript-backed activity. | Formalize the FORGE HUD/status payload for context, cost, tool calls, active agents, todos, queue state, checks, and risk. |
| [`stanford-iris-lab/meta-harness`](https://github.com/stanford-iris-lab/meta-harness) | Automated search over task-specific harness design: what to store, retrieve, and show. | Split FORGE improvement loops into scenario spec, proposer trace, verifier result, and promoted playbook. |
...
| [`NousResearch/hermes-agent`](https://github.com/NousResearch/hermes-agent) | Self-improving operator shell with memories, skills, scheduler, gateways, subagents, terminal backends, and migration tooling. | Keep FORGE portable across local, SSH, container, and hosted terminal backends without hiding the underlying commands. |
...
FORGE 2.0 should be a harness operating system, not only a catalog of commands,
agents, and skills.
...
│ Forge Shield policies/SARIF, FORGE Tools checks, billing,       │
...
- `forge`: extend the adapter compliance matrix and public
  scorecard onramp.
- `forge2`: surface session/worktree state through a stable local payload before
  adding hosted telemetry.
- `FORGE-Tools`: consume the same lifecycle events for PR checks, issue routing,
  and Linear sync.
...
- Keep `docs/architecture/observability-readiness.md` as the operator-facing
  readiness gate.
- Define a versioned HUD/status JSON contract that both FORGE2 and FORGE Tools can
  consume.
...
- Use RAG/reference sets for vetted FORGE patterns, team history, CI failures,
  review outcomes, harness config quality, and security decisions.

Repo work:

- `forge`: document scenario specs, verifier contracts, and
  playbook promotion rules.
- `FORGE-Tools`: map analyzer findings to PR comments, check runs, and Linear
  tasks without flooding the workspace.
- `forge-shield`: feed prompt-injection and config-risk findings into regression
  suites.
...
### Forge Shield Enterprise Security Platform

Forge Shield should move from useful scanner to enterprise security platform.
...
### FORGE Tools Commercial And Review Platform

FORGE Tools should become the GitHub-native layer for billing, deep analysis,
...
- Deep analyzer comparable in scope to the useful parts of GitGuardian,
  ...
  and harness drift.
- RAG/reference set over vetted FORGE patterns, historical PR outcomes,
...
4. Start Forge Shield policy schema plus SARIF fixtures.
5. Audit FORGE Tools billing and check-run surfaces.
