...
This playbook is the FORGE operator runbook for npm, GitHub Actions, and
...
supply-chain compromise and broader Mini Shai-Hulud campaign. FORGE keeps the
...
- Socket's 2026-05-14 `node-ipc` report describes a separate active npm
  ...
  with historical malicious `node-ipc` versions also blocked by FORGE because
...
FORGE also runs `.github/workflows/supply-chain-watch.yml` every six hours and on
...
If FORGE or a maintainer machine installed a known-bad package version:
...
FORGE enforces these rules through `scripts/ci/validate-workflow-security.js`:
...
Before tagging or publishing FORGE:
...
- Forge Shield, GitGuardian, Dependabot, npm audit, or registry-signature checks
  disagree.
