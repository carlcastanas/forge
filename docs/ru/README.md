...
>
> Если ваша локальная установка Claude была очищена или сброшена, начните с `node scripts/forge.js list-installed`, затем запустите `node scripts/forge.js doctor` и `node scripts/forge.js repair` перед любой переустановкой. Обычно это восстанавливает файлы, управляемые FORGE, без пересборки всей настройки.
...
Используйте команду `/skill-create` для локального анализа репозитория без внешних сервисов:
...
Это локально анализирует вашу историю Git и создаёт:
