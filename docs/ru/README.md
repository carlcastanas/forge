> **Перевод.** Эта страница отстаёт от английской версии. Первоисточник — [README.md](../../README.md).
...
FORGE v2.0.0-rc.1 добавляет публичную историю оператора Hermes поверх этого переиспользуемого слоя: начните с [руководства по настройке Hermes](../HERMES-SETUP.md), затем прочитайте [примечания к выпуску rc.1](../releases/2.0.0-rc.1/release-notes.md) и [архитектуру для разных сред](../architecture/cross-harness.md).
...
<img src="../../assets/images/guides/shorthand-guide.png" alt="Краткое руководство по FORGE" />
</a>
</td>
<td width="33%">
<a href="/status/2014040193557471352">
<img src="../../assets/images/guides/longform-guide.png" alt="Подробное руководство по FORGE" />
</a>
</td>
<td width="33%">
...
<td align="center"><b>Руководство по безопасности</b><br/>Векторы атак, песочницы, санитизация, CVE, Forge Shield.</td>
</tr>
</table>
...
### v2.0.0-rc.1 — Обновление публичного контура, операторские рабочие процессы и FORGE 2.0 Alpha (апрель 2026)

- **Dashboard GUI** — новое настольное приложение на Tkinter (`forge_dashboard.py` или `npm run dashboard`) с переключателем тёмной/светлой темы, настройкой шрифта и логотипом проекта в заголовке и панели задач.
...
- **Расширение операторских и outbound-рабочих процессов** — `brand-voice`, `social-graph-ranker`, `connections-optimizer`, `customer-billing-ops`, `forge-tools-cost-audit`, `google-workspace-ops`, `project-flow-ops` и `workspace-surface-audit` закрывают операторское направление.
...
- **FORGE 2.0 alpha находится в дереве репозитория** — прототип control plane на Rust в `forge2/` теперь собирается локально и предоставляет команды `dashboard`, `start`, `sessions`, `status`, `stop`, `resume` и `daemon`. Это пригодная к использованию alpha-версия, но ещё не общий релиз.
- **Укрепление экосистемы** — Forge Shield, контроль затрат FORGE Tools, работа над billing portal и обновления сайта продолжают поставляться вокруг основного плагина, а не расползаются по отдельным направлениям.
...
- **Релиз с фокусом на средах агентного ИИ** — FORGE теперь явно позиционируется как система повышения эффективности таких сред, а не просто набор конфигов.
...
- **Управление хуками во время выполнения** — `FORGE_HOOK_PROFILE=minimal|standard|strict` и `FORGE_DISABLED_HOOKS=...` для runtime-ограничений без редактирования файлов хуков.
...
### v1.6.0 — Codex CLI, Forge Shield и Marketplace (февраль 2026)
...
- **Интеграция Forge Shield** — навык `/security-scan` запускает Forge Shield прямо из Claude Code; 1282 теста, 102 правила
- **GitHub Marketplace** — GitHub App FORGE Tools доступен на github.com/marketplace/forge-tools с тарифами free/pro/enterprise
...
- **Интерактивный мастер установки** — новый навык `configure-forge` предоставляет пошаговую настройку с обнаружением merge/overwrite
...
- **Не накладывайте методы установки друг на друга.** Самая частая сломанная конфигурация: сначала `/plugin install`, затем `install.sh --profile full` или `npx forge-universal install --profile full`.

Если вы уже наложили несколько установок и видите дублирование, сразу переходите к разделу [Сброс / удаление FORGE](#сброс--удаление-forge).

### Путь с малым контекстом / без хуков

Если хуки кажутся слишком глобальными или вам нужны только правила, агенты, команды и основные навыки рабочих процессов FORGE, пропустите плагин и используйте минимальный ручной профиль:
...
Если вы не уверены, какой профиль FORGE или компонент установить, спросите упакованный advisor из любого проекта:
...
Это сделано намеренно. Установки Anthropic marketplace/plugin ключуются каноническим идентификатором плагина, поэтому FORGE использует `forge@forge`, чтобы имена инструментов и пространства имен slash-команд оставались достаточно короткими для строгих валидаторов Desktop/API. Старые публикации могут всё ещё показывать прежний длинный marketplace-идентификатор; считайте его только устаревшим alias. Отдельно npm-пакет остался `forge-universal`, поэтому npm-установки и marketplace-установки намеренно используют разные имена.

### Шаг 2: Установите правила (обязательно)

> ПРЕДУПРЕЖДЕНИЕ: **Важно:** плагины Claude Code не могут автоматически распространять `rules`.
>
> Если вы уже установили FORGE через `/plugin install`, **не запускайте после этого `./install.sh --profile full`, `.\install.ps1 --profile full` или `npx forge-universal install --profile full`**. Плагин уже загружает навыки, команды и хуки FORGE. Запуск полного установщика после установки плагина скопирует те же компоненты в пользовательские директории и может создать дублирующиеся навыки и дублирующееся runtime-поведение.
>
> Для установки через плагин вручную скопируйте только нужные директории `rules/` в `~/.claude/rules/forge/`. Начните с `rules/common` плюс один языковой или framework-пакет, который вы действительно используете. Не копируйте все директории правил, если явно не хотите весь этот контекст в Claude.
>
> Используйте полный установщик только если делаете полностью ручную установку FORGE вместо пути через плагин.
>
> Если ваша локальная установка Claude была очищена или сброшена, это не значит, что нужно повторно покупать FORGE. Начните с `node scripts/forge.js list-installed`, затем запустите `node scripts/forge.js doctor` и `node scripts/forge.js repair` перед любой переустановкой. Обычно это восстанавливает файлы, управляемые FORGE, без пересборки всей настройки. Если проблема связана с аккаунтом или marketplace-доступом к FORGE Tools, восстановление billing/account нужно делать отдельно.
...
# Путь установки через плагин: скопируйте только правила FORGE в пространство имён FORGE
mkdir -p ~/.claude/rules/forge
cp -R rules/common ~/.claude/rules/forge/
cp -R rules/typescript ~/.claude/rules/forge/

# Полностью ручной путь установки FORGE (используйте вместо /plugin install)
...
### Сброс / удаление FORGE

Если FORGE кажется дублированным, навязчивым или сломанным, не переустанавливайте его снова поверх самого себя.

- **Путь через плагин:** удалите плагин из Claude Code, затем удалите конкретные папки правил, которые вы вручную скопировали в `~/.claude/rules/forge/`.
...
Затем удалите файлы, управляемые FORGE:
...
FORGE удаляет только файлы, записанные в его install-state. Он не удалит посторонние файлы, которые сам не устанавливал.

Если вы смешали методы, очищайте в таком порядке:

1. Удалите установку плагина Claude Code.
2. Запустите команду удаления FORGE из корня репозитория, чтобы удалить файлы, управляемые install-state.
...
# Существующие slash-style имена команд продолжают работать, пока FORGE мигрирует с commands/.

# Установка через плагин использует каноническую форму с namespace
/forge:plan "Добавить аутентификацию пользователей"
...
Запустите настольную панель управления, чтобы визуально изучить компоненты FORGE:
...
|   |-- configure-forge/              # Интерактивный мастер установки (НОВОЕ)
|   |-- security-scan/              # Интеграция аудитора безопасности Forge Shield (НОВОЕ)
...
|
|-- rules/            # Always-follow guidelines (копируйте в ~/.claude/rules/forge/)
...
|
|-- forge_dashboard.py  # Настольная GUI-панель управления (Tkinter)
|
...
[Установить GitHub App](https://github.com/apps/skill-creator) | forge.example.com
...
### Forge Shield — аудитор безопасности
...
> mkdir -p ~/.claude/rules/forge
> cp -r forge/rules/common ~/.claude/rules/forge/
> cp -r forge/rules/typescript ~/.claude/rules/forge/   # выберите свой стек
> cp -r forge/rules/python ~/.claude/rules/forge/
> cp -r forge/rules/golang ~/.claude/rules/forge/
> cp -r forge/rules/php ~/.claude/rules/forge/
>
...
Не копируйте сырой repo-файл `hooks/hooks.json` в `~/.claude/settings.json` или `~/.claude/hooks/hooks.json`. Этот файл ориентирован на плагин/репозиторий и должен устанавливаться через установщик FORGE или загружаться как плагин, поэтому прямое копирование не является поддерживаемым ручным способом установки.
...
Если вы установили FORGE через `/plugin install`, не копируйте эти хуки в `settings.json`. Claude Code v2.1+ уже автоматически загружает plugin `hooks/hooks.json`, а дублирование в `settings.json` вызывает двойное выполнение и кроссплатформенные конфликты хуков.

Примечание для Windows: директория конфигурации Claude — `%USERPROFILE%\\.claude`, а не `~/claude`.

#### Настроить MCP

Установки Claude plugin намеренно не включают автоматически bundled MCP server definitions FORGE. Это предотвращает слишком длинные имена plugin MCP tools на строгих сторонних gateway, но оставляет доступной ручную настройку MCP.
...
Если у вас уже запущены собственные копии MCP, bundled в FORGE, задайте:

```bash
export FORGE_DISABLED_MCPS="github,context7,exa,playwright,sequential-thinking,memory"
```

FORGE-managed install и Codex sync flows будут пропускать или удалять эти bundled servers вместо повторного добавления дубликатов. `FORGE_DISABLED_MCPS` — это фильтр установки/синхронизации FORGE, а не live-переключатель Claude Code.
...
|-----------|---------------|-------|
| Спланировать новую функцию | `/forge:plan "Добавить auth"` | planner |
| Спроектировать архитектуру системы | `/forge:plan` + агент architect | architect |
...
```
/forge:plan "Добавить OAuth-аутентификацию пользователей"
...
<details>
<summary><b>Можно ли использовать FORGE с Claude Code на custom API endpoint или model gateway?</b></summary>

Да. FORGE не хардкодит транспортные настройки Anthropic-hosted окружения. Он запускается локально через обычный CLI/plugin-интерфейс Claude Code, поэтому работает с:
...
Если ваш gateway переименовывает модели, настраивайте это в Claude Code, а не в FORGE. Хуки, навыки, команды и правила FORGE не зависят от model provider, если CLI `claude` уже работает.
...
Слишком много MCP-серверов съедают контекст. Каждое описание MCP tool потребляет токены из вашего окна 200k, потенциально сокращая его до ~70k. Контекст SessionStart по умолчанию ограничен 8000 символами; уменьшите его через `FORGE_SESSION_START_MAX_CHARS=4000` или отключите через `FORGE_SESSION_START_CONTEXT=off` для local-model или low-context setups.
...
Они не поставляются вместе с FORGE и не аудируются этим репозиторием, но о них стоит знать, если вы изучаете более широкую экосистему Claude Code skills:
...
FORGE предоставляет поддержку Cursor IDE с хуками, правилами, агентами, навыками, командами и MCP-конфигами, адаптированными под layout проекта Cursor.
...
| Agents | 50 | `.cursor/agents/forge-*.md` при установке; с префиксом, чтобы избежать конфликтов с user или marketplace agents |
...
FORGE не устанавливает root `AGENTS.md` в `.cursor/`. Cursor воспринимает вложенные `AGENTS.md` как directory context, поэтому копирование identity FORGE-репозитория в host project загрязняло бы этот проект.

Cursor-native loading behavior может различаться между сборками Cursor. FORGE устанавливает агентов как `.cursor/agents/forge-*.md`; если ваша сборка Cursor не показывает project agents, эти файлы всё равно работают как явные reference definitions, а не скрытый global prompt context.
...
FORGE предоставляет **первоклассную поддержку Codex** как для macOS app, так и для CLI: reference configuration, Codex-specific supplement `AGENTS.md` и общие skills.
...
# Автоматическая настройка: синхронизировать assets FORGE (AGENTS.md, skills, MCP servers) в ~/.codex
npm install && bash scripts/sync-forge-to-codex.sh
# или: pnpm install && bash scripts/sync-forge-to-codex.sh
# или: yarn install && bash scripts/sync-forge-to-codex.sh
# или: bun install && bash scripts/sync-forge-to-codex.sh

# Или вручную: скопировать reference config в домашнюю директорию
cp .codex/config.toml ~/.codex/config.toml
```

Sync script безопасно сливает MCP-серверы FORGE в существующий `~/.codex/config.toml` через стратегию **add-only**: он никогда не удаляет и не изменяет ваши существующие серверы. Запускайте с `--dry-run`, чтобы посмотреть изменения, или с `--update-mcp`, чтобы принудительно обновить FORGE-серверы до последнего рекомендуемого конфига.

Для Context7 FORGE использует каноническое имя секции Codex `[mcp_servers.context7]`, но всё ещё запускает пакет `@upstash/context7-mcp`. Если у вас уже есть legacy-запись `[mcp_servers.context7-mcp]`, `--update-mcp` мигрирует её на каноническое имя секции.
...
| forge | Development conventions и patterns для проекта |
...
Codex **пока не предоставляет parity с Claude-style hook execution**. Принуждение FORGE там instruction-based через `AGENTS.md`, опциональные overrides `model_instructions_file` и настройки sandbox/approval.
...
FORGE поставляет три sample role configs:
...
FORGE предоставляет **полную поддержку OpenCode**, включая плагины и хуки.
...
Эта npm plugin entry включает опубликованный OpenCode plugin module FORGE (hooks/events и plugin tools).
Она **не** добавляет автоматически полный catalog команд/агентов/instructions FORGE в конфиг вашего проекта.

Для полной настройки FORGE OpenCode:
...
FORGE — **первый плагин, который помогает максимально использовать каждый крупный инструмент AI-кодинга**. Вот как сравниваются среды:
...
Я использую Claude Code с экспериментального rollout. В сентябре 2025 выиграл Anthropic x Forum Ventures hackathon вместе с  — мы построили a hackathon project полностью с помощью Claude Code.
...
- Используйте `FORGE_DISABLED_MCPS` только для фильтрации MCP-конфигов, генерируемых FORGE, во время install/sync flows
...
Проекты, построенные на FORGE или вдохновлённые им:
...
Построили что-то с FORGE? Откройте PR, чтобы добавить это сюда.
...
**Стать спонсором** | [Уровни спонсорства](../../SPONSORS.md) | [Программа спонсорства](../../SPONSORING.md)
...
- **Краткое руководство (начните здесь):** [The Shorthand Guide to FORGE](/status/2012378465664745795)
- **Подробное руководство (продвинутый уровень):** [The Longform Guide to FORGE](/status/2014040193557471352)
- **Руководство по безопасности:** [Security Guide](../../the-security-guide.md) | [Тред](/status/2033263813387223421)
- **Подписаться:**
