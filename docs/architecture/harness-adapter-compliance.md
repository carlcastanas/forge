...
This matrix is the public onramp for teams that want to use FORGE across more
...
FORGE's durable units stay in shared sources:
...
| State | Meaning |
| --- | --- |
| Native | FORGE can install or verify the surface directly for this harness. |
| Adapter-backed | FORGE has a thin adapter, plugin, or package surface, but parity differs by harness. |
| Instruction-backed | FORGE can provide the guidance and files, but the harness does not expose the runtime hook/session surface FORGE needs for enforcement. |
| Reference-only | The tool is useful as a design pressure or external runtime, but FORGE does not yet ship a direct installer or adapter for it. |
...
| Pi | Adapter-backed | Pi package manifest; canonical FORGE skills (skills/); canonical FORGE commands as prompt templates (commands/); canonical FORGE engineering rules (rules/common/) injected into the system prompt; session lifecycle hook adapter; /forge-doctor diagnostics command | Subagents, chains, approval prompts, and persistent todos require companion Pi packages and are not part of this adapter; Pi core has no MCP surface, though FORGE MCP configs load verbatim through the community pi-mcp-adapter package, which FORGE neither installs nor depends on | `pi install git:github.com/carlcastanas/forge`; `pi install /path/to/FORGE` from a local checkout | `node tests/pi/pi-package-manifest.test.js`; `node tests/pi/pi-extension-adapter.test.js`; `npm run harness:adapters -- --check` | Pi extensions execute with full user permissions, and hooks run without a shell and resolve from the installed package rather than the user project; Keep canonical skills and commands as the single source of truth, and never generate copies under .pi/ |
...
| Gemini | Instruction-backed | Gemini project-local instructions; shared skills; rules; compatibility docs | No full FORGE hook parity; ecosystem ports must document drift from upstream FORGE | `./install.sh --profile minimal --target gemini` | `node tests/lib/install-targets.test.js` | Treat Gemini ports as ecosystem adapters until validated end to end inside Gemini CLI. |
...
| Orca | Reference-only | worktree lifecycle; review state; notification; provider-identity design pressure | No FORGE installer or direct adapter today | Use as a comparison target for worktree/session state requirements | `npm run observability:ready` | Do not import product-specific assumptions; convert lessons into FORGE event fields. |
| Superset | Reference-only | workspace presets; parallel-agent review loops; worktree isolation design pressure | No FORGE installer or direct adapter today | Use as a comparison target for workspace preset taxonomy | `npm run observability:ready` | Keep FORGE portable; do not require a desktop workspace to get basic value. |
| Ghast | Reference-only | terminal-native pane grouping; cwd grouping; search; notifications | No FORGE installer or direct adapter today | Use as a comparison target for terminal-first session grouping | `node scripts/session-inspect.js --list-adapters` | Preserve terminal ergonomics before adding visual UI assumptions. |
...
Use this sequence before asking FORGE to make a team or repo setup more
autonomous:
...
node scripts/loop-status.js --json --write-dir .forge/loop-status
```
...
- Treat reference-only tools as design pressure until FORGE has a direct adapter.
