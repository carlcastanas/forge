...
FORGE is the reusable workflow layer. Harnesses are execution surfaces.
...
[FORGE Platform Value Loop](platform-value-loop.md).
...
| Memory | `.forge/memory/`, `~/.forge/memory/` | `forge memory` CLI or opt-in `forge-memory-mcp` stdio server | Supported with explicit recall and unreviewed writes |
| Sessions | `forge2/`, session adapters, orchestration scripts | TUI/daemon, tmux/worktree orchestration, harness-specific runners | Alpha |

## What Travels Unchanged

`SKILL.md` is the most portable unit.

A good FORGE skill should:
...
- OpenCode has a plugin/event system that can reuse FORGE hook logic through an adapter layer.
- Cursor uses its own rule and hook layout, so FORGE maintains translated surfaces under `.cursor/`.
...
FORGE Memory Vault is the common knowledge-transfer surface for Claude, Codex,
Hermes, Cursor, OpenCode, and other agents. It stores portable
`forge.memory.v1` Markdown documents in three scopes:

- project: `<repo>/.forge/memory/project/`
- team: `<repo>/.forge/memory/team/`
- user: `~/.forge/memory/`

Every harness must use the same repository working directory or the same
`FORGE_MEMORY_PROJECT_ROOT` and `FORGE_MEMORY_USER_ROOT` overrides. The deterministic
`forge memory` CLI is the baseline interface. Harnesses with MCP support may
instead launch `forge-memory-mcp` and use `memory_save`, `memory_search`,
...
`FORGE_MEMORY_HARNESS`; this server-bound identity supplies the source harness
...
`FORGE_MEMORY_ALLOW_USER_SCOPE=1`.
...
Hermes is not the public FORGE runtime.

Hermes is an operator shell that can consume FORGE assets:

- import selected FORGE skills into a Hermes skills directory
- use FORGE MCP conventions for tool access
- route chat, CLI, cron, and handoff workflows through reusable FORGE patterns
- distill repeated local operator work back into sanitized FORGE skills
...
OpenCode gets the skill through the OpenCode package/plugin surface. Event handling can reuse FORGE hook logic through the adapter layer, while the skill text stays unchanged.
...
- the full platform loop where external products contribute skill packs,
  gated APIs, evals, and case studies back into FORGE

## Rule For New Work

When adding a workflow, put the durable behavior in FORGE first.
