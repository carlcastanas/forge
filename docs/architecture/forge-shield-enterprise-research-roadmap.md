# Forge Shield Enterprise Research Roadmap

Generated: 2026-05-12; refreshed with May 18 Forge Shield fleet-ticket and
Mini Shai-Hulud IOC evidence.

This is a planning artifact for the next Forge Shield enterprise iteration. It
does not modify Forge Shield code. The goal is to turn the current scanner,
...
Current Forge Shield repository state:

- Forge Shield checkout on clean `main`.
- `README.md`, `API.md`, `package.json`, `.github/workflows/*`, and
  `src/`/`tests/` module layout.
- Current supported user surfaces: `forge-shield scan`, `forge-shield init`,
  `forge-shield miniclaw start`, scanner JSON, MiniClaw API, GitHub Action,
...
- No code was copied. The takeaway is that Forge Shield should track permissions,
...
## Current Forge Shield Position

Forge Shield is already more than a static lint tool:
...
- Evidence-pack consumption is now first-class enough for downstream tools:
  `forge-shield evidence-pack inspect` verifies a bundle and emits compact
...
- Fleet-level evidence-pack consumption now has a local routing primitive:
  `forge-shield evidence-pack fleet <dirs...> [--json]` aggregates multiple
  inspected bundles into ready, security-blocker, policy-review,
  baseline-regression, supply-chain-review, and invalid routes.
- FORGE-Tools now consumes that fleet primitive in hosted security review:
  `forge-shield-evidence/fleet-summary.json` routes invalid packs, security
  blockers, policy reviews, baseline regressions, and supply-chain reviews into
  hosted findings.

May 16 update: Forge Shield PR #87 merged as
...
Forge Shield PR #88 merged as
`65ed6e2a87545dc99d962b58413f49096a4d70ec`. It adds
`forge-shield evidence-pack inspect <dir> [--json]`, validates the bundle before
...
Forge Shield PR #89 merged as
`521ada9091bb6d818511ab8589ae675b920c106a`. It adds
`forge-shield evidence-pack fleet <dirs...> [--json]`, verifies each pack through
...
Forge Shield commit `840952a7a07f820f24081c43df656d7f7295f23b` adds
...
default. FORGE-Tools follow-up routing now consumes fleet summaries and surfaces
source evidence paths in hosted findings, and the first cross-harness policy
slice now links Forge Shield fleet route target paths to harness-owner review.
Forge Shield fleet output now also emits `reviewItems` with source evidence paths
...
safer or riskier over time. Forge Shield has scan logs and baseline comparison
...
baseline snapshot creation through `forge-shield baseline write`. The remaining
product surface should make CLI drift summaries, evidence packs, and
owner-ready deltas explicit.

Target capability:

- `forge-shield baseline write --path .claude --output forge-shield-baseline.json`
- `forge-shield scan --baseline forge-shield-baseline.json`
...
- `forge-shield scan --evidence-pack out/forge-shield-evidence`
...
scenarios, traces, and playbooks. Forge Shield already has a corpus benchmark,
...
Forge Shield is already connected conceptually to the FORGE Tools GitHub App.
...
- Billing readiness checks shared with FORGE-Tools so payment state never changes
...
- It aligns Forge Shield with FORGE's harness-agnostic positioning.
...
Connect Forge Shield findings to FORGE-Tools follow-up routing.

Artifacts:

- Finding fingerprints compatible with FORGE-Tools issue caps.
...
The Forge Shield enterprise iteration is not complete until these are true:

- Local `npm run typecheck`, `npm run lint`, `npm test`, and `npm run build`
  pass from the Forge Shield repository root.
...
- FORGE-Tools can consume the finding fingerprints or backlog export without
  exceeding GitHub/Linear object caps.
- The GA roadmap and Linear project status link to the merged Forge Shield PRs.
