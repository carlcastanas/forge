# FORGE Platform Value Loop

FORGE 2.0 is moving from a portable harness layer toward a full operator
...
2. Dedicated FORGE agent: an agent that directly operates over FORGE assets instead
...
```text
FORGE can be used full-stack as a meta-harness + agent + control pane, or
...
- product-specific integrations that can become reusable FORGE skills.
...
External products can build on FORGE without becoming FORGE-branded products. The
contract is:

| Layer | Product contributes | FORGE receives |
| --- | --- | --- |
...
| Case study | A real product workflow that works through FORGE | Distribution, sponsors, Pro interest, consulting demand |
...
1. A product team builds a useful workflow as an FORGE skill pack.
...
5. Sanitized patterns become better FORGE skills, evals, gates, or docs.
6. FORGE gains distribution, maintainers, sponsors, Pro interest, and consulting leads.
...
integration becomes another reason to install FORGE, and every serious FORGE user
