...
FORGE 2.0 tracks execution state across GitHub, Linear, local handoffs, and the
...
| Repo roadmap | Auditable planning mirror | Keep `docs/FORGE-2.0-GA-ROADMAP.md` aligned to merged PR evidence and unresolved gates. |
...
The repo mirror uses these flow lanes so FORGE work does not collapse into one
...
- Forge Shield security scanning and policy packs
...
- Linear remains the external status surface; the repo does not require remote
  telemetry to be release-ready.

Remote telemetry can be added later, but it must consume the
