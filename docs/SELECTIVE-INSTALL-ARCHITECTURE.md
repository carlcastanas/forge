...
- what FORGE owns and may safely remove or repair later

That is the missing contract between FORGE 1.x installation and an FORGE 2.0
control plane.
...
- unified `forge` CLI routing install, planning, and lifecycle commands
...
- legacy `forge-install` compatibility still points at `install.sh`
...
- the repo already has a unified Node entrypoint through `forge` and
  `install-apply.js`
...
FORGE 2.0 should make operation planning a standalone phase so that:
...
- `uninstall`: remove only FORGE-owned outputs
...
- `scripts/forge.js` as the unified CLI
...
If the goal is FORGE 2.0 and not just “working enough,” the next modularization
steps should be:
...
Today FORGE still behaves like a broad payload copier:
...
That means FORGE 2.0 needs two contracts, not one:
...
1. Keep `forge` as the canonical source repo.
...
Fields still needed for FORGE 2.0:

- `installStrategy`
  for example `copy`, `flatten-rules`, `generate`, `merge-config`
- `ownership`
  whether FORGE fully owns the target path or only generated files under it
- `pathMode`
...
That lets FORGE 2.0 say things like:
...
FORGE 2.0 needs the next layer: operation planning.
...
```json
{
  ...
  "destination": "/Users/example/.claude/rules/forge/common/coding-style.md",
  "ownership": "managed",
...
Install-state is the durable contract that FORGE 1.x is missing.

Suggested path conventions:

- Claude target:
  `~/.claude/forge/install-state.json`
- Cursor target:
  `./.cursor/forge-install-state.json`
- Antigravity target:
  `./.agents/forge-install-state.json`
- future Codex target:
  `~/.codex/forge-install-state.json`

Suggested payload:

```json
{
  "schemaVersion": "forge.install.v1",
...
- enough detail for uninstall to remove only FORGE-managed outputs
...
- `forge list-installed` routes to `node scripts/list-installed.js`
- `forge uninstall` routes to `node scripts/uninstall.js`
- `forge doctor` routes to `node scripts/doctor.js`
- `forge repair` routes to `node scripts/repair.js`
...
- load install-state
- remove only FORGE-managed destinations recorded in state
...
FORGE 2.0 should translate legacy language arguments into a compatibility request.
...
FORGE 2.0 should improve this carefully.
...
3. define `forge.install.v1` state schema
...
4. decide when `forge-install` should become a thin alias for `forge install`
...
3. reduce package shipping surface to the module graph instead of broad folders
4. decide whether `forge-install` remains separate or becomes `forge install`
5. add tests that lock down:
   - target-specific merge/remove behavior
   - repair and uninstall safety for non-copy operations
   - unified `forge` CLI routing and compatibility guarantees
...
That is the shortest path from FORGE 1.x installer sprawl to an FORGE 2.0
