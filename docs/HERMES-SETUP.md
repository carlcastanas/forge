# Hermes x FORGE Setup

Hermes is the operator shell. FORGE is the reusable system behind it.
...
- FORGE skills, agents, commands, hooks, and MCP configs from this repo
...
Use Hermes as the front door and FORGE as the reusable workflow substrate.
...
Telegram / CLI / TUI
      ...
      Hermes
        ↓
 FORGE skills + hooks + MCPs + shared Memory Vault
        ↓
...
- `~/.hermes/config.yaml`
  ...
  - plugin loading
- `~/.hermes/skills/forge-imports/`
  - FORGE skills copied in for Hermes-native use
...
FORGE Memory Vault provides one file-first handoff layer instead of a separate
...
Hermes can call the CLI directly or use the opt-in `forge-memory-mcp` stdio
...
`FORGE_MEMORY_HARNESS` identity; they must not connect to one shared server
process. Every process must launch from the same repository working directory
or receive identical `FORGE_MEMORY_PROJECT_ROOT` and `FORGE_MEMORY_USER_ROOT`
overrides.
...
```bash
forge memory search "research parser" --target-harness codex
forge memory read <memory-id>
```

For MCP access, copy only the `forge-memory-vault` entry from
`mcp-configs/mcp-servers.json` into each harness that needs it. FORGE does not
enable this server in the default `.mcp.json`. Launch each server with its own
lowercase identity, for example `FORGE_MEMORY_HARNESS=hermes`. The server binds
...
`FORGE_MEMORY_ALLOW_USER_SCOPE=1`, and the tool call must request `user`.
...
- FORGE for skills, rules, prompts, and cross-harness conventions
- FORGE Memory Vault for explicit, local-first agent handoffs
...
0. Run `forge migrate audit --source ~/.hermes` first to inventory the legacy workspace and see which parts already map onto FORGE2.
0.5. Plan and scaffold migration artifacts before importing anything:
   - generate reviewable plans with `forge migrate plan` and `forge migrate scaffold`
   - scaffold reusable legacy skills with `forge migrate import-skills --output-dir migration-artifacts/skills`
   - scaffold tool translation templates with `forge migrate import-tools --output-dir migration-artifacts/tools`
   - scaffold bridge plugin templates with `forge migrate import-plugins --output-dir migration-artifacts/plugins`
   - preview recurring jobs with `forge migrate import-schedules --dry-run`
   - preview gateway dispatch with `forge migrate import-remote --dry-run`
   - preview safe env/service context with `forge migrate import-env --dry-run`
   - import sanitized workspace memory with `forge migrate import-memory`
1. Install FORGE and verify the baseline harness setup with `node tests/run-all.js`; the expected result is a zero-failure test summary.
2. Install Hermes and point it at FORGE-imported skills.
3. Initialize the shared FORGE Memory Vault. Register `forge-memory-mcp` only if
   Hermes needs tool access instead of the `forge memory` CLI.
...
FORGE v2.0.0-rc.1 documents the Hermes surface and ships launch collateral now.
