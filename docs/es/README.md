...
### v2.0.0-rc.1 — Actualización de Superficie, Flujos de Trabajo de Operador y Alpha de FORGE 2.0 (Abr 2026)

- **Dashboard GUI** — Nueva aplicación de escritorio basada en Tkinter (`forge_dashboard.py` o `npm run dashboard`) con alternancia de tema oscuro/claro, personalización de fuente y logo del proyecto en el encabezado y la barra de tareas.
...
- **Hardening del ecosistema** — Forge Shield, controles de costos de FORGE Tools, trabajo en el portal de facturación y actualizaciones del sitio web continúan publicándose junto al plugin principal en lugar de desviarse hacia silos separados.
...
- **Primera versión centrada en el harness** — FORGE ahora se enmarca explícitamente como un sistema de rendimiento del harness de agentes, no solo un paquete de configuración.
...
- **Controles de ejecución de hooks** — `FORGE_HOOK_PROFILE=minimal|standard|strict` y `FORGE_DISABLED_HOOKS=...` para control en tiempo de ejecución sin editar los archivos de hooks.
...
### v1.6.0 — Codex CLI, Forge Shield y Marketplace (Feb 2026)
...
- **Integración de Forge Shield** — La skill `/security-scan` ejecuta Forge Shield directamente desde Claude Code; 1282 pruebas, 102 reglas
- **GitHub Marketplace** — La GitHub App FORGE Tools disponible en github.com/marketplace/forge-tools con niveles gratuito/pro/empresarial
...
- **Asistente de instalación interactivo** — La nueva skill `configure-forge` proporciona configuración guiada con detección de fusión/sobrescritura
...
>
> Si tu configuración local de Claude fue eliminada o restablecida, eso no significa que necesites volver a comprar FORGE. Empieza con `node scripts/forge.js list-installed`, luego ejecuta `node scripts/forge.js doctor` y `node scripts/forge.js repair` antes de reinstalar cualquier cosa. Eso generalmente restaura los archivos gestionados por FORGE sin reconstruir tu configuración. Si el problema es el acceso a la cuenta o al marketplace para FORGE Tools, gestiona la recuperación de la facturación/cuenta por separado.
...
[Instalar GitHub App](https://github.com/apps/skill-creator) | forge.example.com
