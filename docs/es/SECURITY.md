...
Si descubres una vulnerabilidad de seguridad en FORGE, por favor repórtala de forma responsable.
...
- El plugin de FORGE y todos los scripts de este repositorio
...
FORGE se ejecuta dentro de Claude Code, que inyecta **recordatorios efímeros del lado del cliente** en la entrada del modelo en cada turno (recordatorios de TodoWrite, avisos de cambio de fecha, avisos de archivo modificado, etc.). Estos bloques:
...
- **Forge Shield**: Analiza tu configuración de agentes en busca de vulnerabilidades — `npx forge-shield scan`
