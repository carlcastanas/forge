...
FORGE accepts ideas from outside repos, but shipped skills need to become FORGE-native surfaces.
...
- adapt it to FORGE's current install surfaces, validation flow, and repo conventions
- remove unnecessary external branding, dependency assumptions, and upstream-specific framing

The goal is reuse without turning FORGE into a thin wrapper around someone else's runtime.
...
- there is no better FORGE-native name already in the repo
...
Rename the skill when FORGE meaningfully expands, narrows, or repackages the original work.

Typical triggers:

- FORGE adds substantial new behavior, structure, or guidance
- the original name is vendor-forward or community-brand-forward instead of workflow-forward
- the contribution overlaps an existing FORGE surface and needs a clearer boundary
...
- prefer FORGE-native names like `product-capability` over vague imported planning labels if the scope changed materially

## Dependency Policy

FORGE prefers the narrowest native surface that gets the job done:
...
- vendor or recreate the relevant logic inside FORGE when practical
...
1. Is this a real reusable surface in FORGE, or just documentation for another tool?
2. Does the current name still match the FORGE-shaped surface?
3. Is there already an FORGE skill that owns most of this behavior?
4. Are we importing a concept, or importing someone else's product identity?
5. Would an FORGE user understand the purpose of this skill without knowing the upstream repo?
