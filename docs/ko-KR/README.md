> **번역본.** 이 문서는 영문판보다 뒤처져 있을 수 있습니다. 기준은 [README.md](../../README.md) 입니다.
...
- **하네스 중심 릴리스** — FORGE는 이제 단순 설정 모음이 아닌, 에이전트 하네스 성능 시스템으로 명시됩니다.
- **훅 안정성 개선** — SessionStart 루트 폴백, Stop 단계 세션 요약, 취약한 인라인 원라이너를 스크립트 기반 훅으로 교체.
- **훅 런타임 제어** — `FORGE_HOOK_PROFILE=minimal|standard|strict`와 `FORGE_DISABLED_HOOKS=...`로 훅 파일 수정 없이 런타임 제어.
...
### v1.6.0 — Codex CLI, Forge Shield & 마켓플레이스 (2026년 2월)
...
- **Forge Shield 통합** — `/security-scan`으로 Claude Code에서 직접 Forge Shield 실행; 1282개 테스트, 102개 규칙
- **GitHub 마켓플레이스** — github.com/marketplace/forge-tools에서 무료/프로/엔터프라이즈 티어 제공
...
[GitHub 앱 설치](https://github.com/apps/skill-creator) | forge.example.com
...
> cp -r forge/rules/typescript ~/.claude/rules/typescript   # 사용하는 스택 선택
>
...
|-------------|-------------|-----------------|
| 새 기능 계획하기 | `/forge:plan "인증 추가"` | planner |
| 시스템 아키텍처 설계 | `/forge:plan` + architect 에이전트 | architect |
...
**새로운 기능 시작:**
```
/forge:plan "OAuth를 사용한 사용자 인증 추가"
                                              → planner가 구현 청사진 작성
