# Hermes / OpenClaw -> FORGE Migration

This document is the public migration guide for moving a Hermes or OpenClaw-style operator setup into the current FORGE model.
...
FORGE is the durable public system:
...
- install surfaces
- session adapters
- FORGE 2.0 control-plane work

Hermes and OpenClaw are useful inputs because they contain repeated operator workflows that can be distilled into FORGE-native surfaces.

That means the shortest safe path is:

1. extract the reusable behavior
2. translate it into FORGE-native skills, hooks, docs, or adapter work
...
- FORGE hook / command automation for local repeatability
- FORGE 2.0 scheduler work under issue `#1050`
...
- FORGE 2.0 as the future long-lived scheduling/control plane
...
- FORGE session adapter and control-plane work
- orchestration/session inspection commands
- FORGE 2.0 control-plane backlog under:
  - `#1045`
  - `#1046`
  - `#1047`
  - `#1048`
...
- FORGE-native top-level skills when the workflow is reusable
...
- FORGE-native operator skills when the workflow logic is the real asset
- adapter/control-plane work when the missing piece is session/runtime coordination

Do not import opaque third-party runtimes into FORGE just because a private workflow depended on them.

If a workflow is valuable:

1. understand the behavior
2. rebuild the minimum FORGE-native version
...
- FORGE 2.0 adapter/control-plane discovery docs
- orchestration/session inspection substrate
- operator workflow skills
- cost and workflow audit skills
- cross-harness install surfaces
- Forge Shield for config and agent-surface scanning
...
- continuing the FORGE 2.0 operator/control-plane buildout

FORGE 2.0 now ships a bounded migration audit entrypoint:

- `forge migrate audit --source ~/.hermes`
- `forge migrate plan --source ~/.hermes --output migration-plan.md`
- `forge migrate scaffold --source ~/.hermes --output-dir migration-artifacts`
- `forge migrate import-skills --source ~/.hermes --output-dir migration-artifacts/skills`
- `forge migrate import-tools --source ~/.hermes --output-dir migration-artifacts/tools`
- `forge migrate import-plugins --source ~/.hermes --output-dir migration-artifacts/plugins`
- `forge migrate import-schedules --source ~/.hermes --dry-run`
- `forge migrate import-remote --source ~/.hermes --dry-run`
- `forge migrate import-env --source ~/.hermes --dry-run`
- `forge migrate import-memory --source ~/.hermes`

Use that first to inventory the legacy workspace and map detected surfaces onto the current FORGE2 scheduler, remote dispatch, memory graph, templates, and manual-translation lanes.
...
1. Keep the public FORGE repo as the canonical reusable layer.
2. Port reusable Hermes/OpenClaw workflows into FORGE-native skills one lane at a time.
3. Keep private auth and personal context outside the repo.
4. Use GitHub / Linear / KB systems as durable truth.
5. Treat FORGE 2.0 as the path to a native operator shell, not as a finished product.
