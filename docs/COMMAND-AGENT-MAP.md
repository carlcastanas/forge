...
| `/security-scan` | security-reviewer (skill) | Forge Shield via security-scan skill |

## Non-Slash CLI Surfaces

| CLI surface | Primary skill/runtime | Notes |
|-------------|-----------------------|-------|
| `forge memory init` | unified-memory / `scripts/memory.js` | Initialize project, team, or user Markdown vault scopes |
| `forge memory save` | unified-memory / `scripts/memory.js` | Create unreviewed memory; body must come from stdin or a regular file |
| `forge memory handoff` | unified-memory / `scripts/memory.js` | Create a targeted, cross-harness handoff |
| `forge memory search` | unified-memory / `scripts/memory.js` | Bounded lexical search over selected vault scopes |
| `forge memory read` | unified-memory / `scripts/memory.js` | Read one memory plus derived backlinks |
| `forge memory doctor` | unified-memory / `scripts/memory.js` | Audit malformed files, duplicate IDs, broken links, and symlinks |
| `forge-memory-mcp` | unified-memory / `scripts/memory-mcp.mjs` | Optional stdio MCP adapter; exposes save/search/read/doctor only |
...
- **eval-harness**: `/eval`
- **security-scan**: `/security-scan` (runs Forge Shield)
- **strategic-compact**: suggested at compaction points (hooks)
- **unified-memory**: `forge memory ...` and the opt-in `forge-memory-mcp` server
