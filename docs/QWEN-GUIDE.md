...
FORGE can install its managed command, agent, skill, rule, and MCP surfaces into the Qwen CLI home directory.
...
The Qwen adapter writes into `~/.qwen/` and records managed file ownership in `~/.qwen/forge-install-state.json`.
...
Rerun the same install command after pulling FORGE updates. The installer uses the install-state file to update FORGE-managed files without claiming unrelated user files in `~/.qwen/`.
...
That removes files recorded in `~/.qwen/forge-install-state.json` and leaves unrelated Qwen configuration alone.
