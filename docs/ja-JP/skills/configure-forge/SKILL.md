...
description: Claude Code、Codex、Kimi 内で FORGE のインストール、更新、再設定を案内し、各ハーネスが実際に備えるプラグイン、スコープ、フック機能を守ります。
metadata:
  origin: FORGE
---
...
FORGE を一時ディレクトリへ clone したり、プラグインを手作業でコピーしたりしないでください。

ユーザー自身が操作するターミナルの正規エントリは `forge setup` と `npx forge-universal setup` です。
...
- Codex では Codex ネイティブのプラグインライフサイクルを使います。Claude のスコープを提示したり、
  Claude の FORGE フック 4 段階を Codex に対応付けたりしません。
- Kimi ではプロジェクトサーフェスを `./.kimi-code` に導入します。Kimi は FORGE の Claude ライフサイクル
  フックプロファイルに対応しません。
...
両方のコマンドを実行し、FORGE の導入スコープ、有効状態、marketplace ソースを要約します。
...
`forge@forge` が 1 つのみ既存する場合は再設定として扱います。Claude が所有する
"Open home page" コントロールをインストールの根拠にしません。setup が複数の FORGE スコープ、
...
- `off` はスキルとコマンドを残し、FORGE フック自動化を無効にします。
...
そこで bare な対話式 `forge setup` を実行しません。
...
選択スコープに有効な `forge@forge` が正確に 1 件ある場合のみ続行します。`$CLAUDE_PLUGIN_ROOT` があるときは、
成功した setup の `action`（`installed`、`updated`、`migrated`、`resumed`、
`already-migrated`）を内蔵レンダラーへ渡します。

呼び出し前に、プロバイダーが報告したバージョンが
`scripts/lib/terminal-welcome.js` の `FORGE_VERSION_PATTERN` に一致することを
...
JSON が FORGE を導入済みと報告し、`installedPath` を提供した場合のみ続行し、検証済みバンドルからウェルカムを表示します。

`installedPath` は Codex JSON が返した絶対パスそのものだけを使い、制御文字を
拒否します。バージョンは `FORGE_VERSION_PATTERN` で検証します。`node` を次の
...
確認前に機能サマリーを示します。導入先は `./.kimi-code`、FORGE ライフサイクルフックは
...
Kimi が FORGE ライフサイクルフックを導入または設定したとは表現しません。
