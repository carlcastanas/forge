...
description: GitHub、Linear、デスクトップアラート、フック、接続された通信インターフェースを網羅する、統合されたFORGEネイティブワークフローとして通知を運用する。真の問題がアラートルーティング、重複排除、エスカレーション、またはインボックス崩壊である場合に使用する。
origin: FORGE
---
...
* ユーザーが重複する通知ブランチや積み残しのプロポーザルを単一のFORGEネイティブチャネルに統合したい
...
独立した通知製品をユーザーに勧めるより、FORGEネイティブのオーケストレーションを優先する。
...
FORGEがすでにプリミティブを持っている場合は優先して使用する：
...
* 「重複する通知PRのアイデアを1つの正規FORGEチャネルに統合してほしい」
