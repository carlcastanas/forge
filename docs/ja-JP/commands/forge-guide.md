---
description: FORGEの現在のエージェント、スキル、コマンド、フック、インストールプロファイル、ドキュメントをライブリポジトリサーフェスからナビゲートします。
---

# /forge-guide

このコマンドをFORGEの会話型マップとして使用します。完全なREADMEや古いカタログ数をダンプすることなく、タスクに適したFORGEサーフェスをユーザーが発見するのを支援します。
...
/forge-guide hooks
/forge-guide install
/forge-guide find: <クエリ>
/forge-guide <機能名またはファイル名>
```
...
- `scripts/ci/catalog.js --json` — FORGE内で実行時のライブカタログカウント
...
- `/project-init` — スタック対応のFORGEオンボーディング
