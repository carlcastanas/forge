---
description: エージェント、フック、MCP、パーミッション、シークレットのサーフェスに対してForge Shieldを実行します。
...
現在のプロジェクトまたはターゲットパスに対してForge Shieldを実行し、所見を優先順位付きの修正計画に変換します。
...
- `--fix`: 安全かつ自動修正可能と明示的にマークされたForge Shieldの修正のみを適用。
...
ローカルForge Shield開発の場合、Forge Shieldチェックアウトから実行:

```bash
npm run scan -- --path "${TARGET_PATH:-.}" --format text
```

所見を作り出さないこと。Forge Shieldの出力を信頼できるソースとして使用し、スキャナーの事実とフォローアップの判断を分離。
