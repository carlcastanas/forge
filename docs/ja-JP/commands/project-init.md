---
description: プロジェクトのスタックを検出し、リポジトリのインストールマニフェストとスタックマッピングを使用してドライランFORGEオンボーディング計画を生成します。
---

# /project-init

現在のプロジェクト用の安全でレビュー可能なFORGEオンボーディング計画を作成します。このコマンドはドライランモードで開始し、明示的なユーザー承認後にのみファイルを書き込みます。
...
3. FORGEのインストーラーとマニフェストツールを使用。インストールのショートカットとしてファイルを手動コピーしたり任意のリモートをクローンしない。
...
- FORGE設定: `forge-install.json`
- オプションのスタックマップ: FORGEリポジトリ内の`config/project-stack-mappings.json`

FORGEチェックアウトが利用可能な場合、`config/project-stack-mappings.json`をスタックからルール/スキルへの参照として使用。ファイルが利用できない場合、インストール済みのFORGEマニフェストと明示的なユーザーの選択にフォールバック。
...
3. 最小限の有用なFORGE計画を解決:
   - プロジェクトに`forge-install.json`がある: `node scripts/install-plan.js --config forge-install.json --json`
...
- `/forge-guide` — インストール前のインタラクティブな機能ディスカバリー
