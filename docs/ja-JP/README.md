> **翻訳版。** このページは英語版より古い場合があります。正典は [README.md](../../README.md) です。
...
- **インタラクティブインストールウィザード** — 新しい`configure-forge`スキルがマージ/上書き検出付きガイドセットアップを提供
...
|   |-- configure-forge/              # インタラクティブインストールウィザード（新規）
|   |-- security-scan/              # Forge Shield セキュリティ監査統合（新規）
|
...
[GitHub アプリをインストール](https://github.com/apps/skill-creator) | forge.example.com
...
> cp -r forge/rules/common ~/.claude/rules/common
> cp -r forge/rules/typescript ~/.claude/rules/typescript   # スタックを選択
> cp -r forge/rules/python ~/.claude/rules/python
> cp -r forge/rules/golang ~/.claude/rules/golang
>
...
`/plugin install` で FORGE を導入した場合は、これらのフックを `settings.json` にコピーしないでください。Claude Code v2.1+ はプラグインの `hooks/hooks.json` を自動読み込みするため、二重登録すると重複実行や `${CLAUDE_PLUGIN_ROOT}` の解決失敗が発生します。
...
forge-universal は [Cursor IDE](https://cursor.com) の事前翻訳設定を含みます。`.cursor/` ディレクトリには、Cursor フォーマット向けに適応されたルール、エージェント、スキル、コマンド、MCP 設定が含まれています。
...
FORGEは**フルOpenCodeサポート**をプラグインとフック含めて提供。
