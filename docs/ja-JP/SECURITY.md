...
FORGEでセキュリティ脆弱性を発見した場合は、責任ある方法で報告してください。

**セキュリティ脆弱性についてGitHubの公開Issueを作成しないでください。**

<security@example.com>>** に以下を含むメールを送信してください：
...
- FORGEプラグインおよびこのリポジトリ内のすべてのスクリプト
...
FORGEはClaude Code内で実行され、モデルの入力に毎ターン**エフェメラルなクライアントサイドのシステムリマインダー**を注入します（TodoWriteのナッジ、日付変更通知、ファイル変更通知など）。これらのブロックは：
...
- **Forge Shield**: エージェント設定の脆弱性をスキャン — `npx forge-shield scan`
