# FORGE Selective Install Design

## Purpose

This document defines the user-facing selective-install design for FORGE.
...
- how users choose FORGE components
- what the CLI should feel like
- what config file should exist
- how installation should behave across harness targets
- how the design maps onto the current FORGE codebase without requiring a rewrite

## Problem

Today FORGE still feels like a large payload installer even though the repo now
...
The selective-install system should make FORGE feel composable instead of
all-or-nothing.
...
1. Let users install a small default FORGE footprint quickly.
...
5. Preserve backward compatibility with the current `forge-install typescript`
   style during rollout.

## Non-Goals

- packaging FORGE into multiple npm packages in the first phase
...
A user should be able to get a useful FORGE install with one command:

```bash
forge install --target claude --profile core
```
...
```bash
forge install --target cursor --profile developer --with lang:typescript --with framework:nextjs --dry-run
```
...
```bash
forge install --config forge-install.json
```
...
These are the default FORGE building blocks:
...
Capability packs are cross-cutting FORGE feature bundles.
...
- `core`
  minimal baseline, safe default for most users trying FORGE
- `developer`
...
```bash
forge install --target claude --profile developer --with lang:typescript --with framework:nextjs --without capability:orchestration
```
...
```bash
forge install [--target <target>] [--profile <name>] [--with <component>]... [--without <component>]... [--config <path>] [--dry-run] [--json]
```
...
```bash
forge install --target claude --profile core
forge install --target cursor --profile developer --with lang:typescript --with framework:nextjs
forge install --target antigravity --with capability:security --with lang:python
forge install --config forge-install.json
```
...
```bash
forge plan [same selection flags as install]
```
...
```bash
forge-install typescript
forge-install --target cursor typescript
forge typescript
```
...
The commands differ in action, not in how FORGE understands the selected install.
...
- `forge-install.json` config file parsing
...
## Mapping To Current FORGE Manifests
...
That is the shortest path from the current FORGE codebase to a real selective
install experience that feels like FORGE 2.0 instead of a large legacy installer.
