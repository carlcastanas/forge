...
function runTests() {
  ...
  if (test('GitHub Integration category scores against a fully-wired consumer fixture', () => {
    ...
    try {
      fs.mkdirSync(path.join(homeDir, '.claude', 'plugins', 'forge', '.claude-plugin'), { recursive: true });
      fs.writeFileSync(
        path.join(homeDir, '.claude', 'plugins', 'forge', '.claude-plugin', 'plugin.json'),
        JSON.stringify({ name: 'forge' }, null, 2)
      );
  ...
  if (test('detects repo mode from structural markers when package name differs', () => {
    ...
    try {
      ...
      fs.writeFileSync(path.join(projectRoot, '.claude-plugin', 'plugin.json'), JSON.stringify({ name: 'forge' }, null, 2));
      fs.writeFileSync(
        path.join(projectRoot, 'package.json'),
  ...
  if (test('audits consumer projects from cwd instead of the FORGE repo root', () => {
  ...
  if (test('prints no top actions when consumer checks all pass', () => {
    ...
    try {
      fs.mkdirSync(path.join(projectRoot, '.claude', 'plugins', 'forge@forge'), { recursive: true });
      fs.writeFileSync(
        path.join(projectRoot, '.claude', 'plugins', 'forge@forge', 'plugin.json'),
        JSON.stringify({ name: 'forge' }, null, 2)
      );
      fs.mkdirSync(path.join(projectRoot, '.claude'), { recursive: true });
  ...
  if (test('detects marketplace-installed Claude plugins under home marketplaces/', () => {
    ...
    try {
      fs.mkdirSync(path.join(homeDir, '.claude', 'plugins', 'marketplaces', 'forge', '.claude-plugin'), { recursive: true });
      fs.writeFileSync(
        path.join(homeDir, '.claude', 'plugins', 'marketplaces', 'forge', '.claude-plugin', 'plugin.json'),
        JSON.stringify({ name: 'forge' }, null, 2)
      );
  ...
  if (test('detects marketplace-installed Claude plugins under project marketplaces/', () => {
    ...
    try {
      fs.mkdirSync(path.join(projectRoot, '.claude', 'plugins', 'marketplaces', 'forge', '.claude-plugin'), { recursive: true });
      fs.writeFileSync(
        path.join(projectRoot, '.claude', 'plugins', 'marketplaces', 'forge', '.claude-plugin', 'plugin.json'),
        JSON.stringify({ name: 'forge' }, null, 2)
      );
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          plugins: {
            'forge@forge': [
              { installPath: path.join('cache', 'forge', 'forge', '2.0.0') },
            ],
          },
        }, null, 2)
      );
      ...
      try {
        ...
        assert.ok(found);
        assert.ok(found.includes(`${path.sep}cache${path.sep}forge${path.sep}forge${path.sep}2.0.0${path.sep}`));
      } finally {
    ...
    try {
      for (const version of ['1.8.0', '1.10.0']) {
        ...
        fs.writeFileSync(
          ...
          JSON.stringify({ name: 'forge', version }, null, 2)
        );
      }
