...
function runTests() {
  ...
  if (test('discussion-disabled repos skip the heavy discussion query', () => {
    ...
    try {
      seedRepo(projectRoot);
      const shimPath = writeGhShim(projectRoot, {
        'pr list --repo carlcastanas/forge-shield --state open --json number,title,isDraft,mergeStateStatus,updatedAt,url,author': [],
        'issue list --repo carlcastanas/forge-shield --state open --json number,title,updatedAt,url,author,labels': [],
