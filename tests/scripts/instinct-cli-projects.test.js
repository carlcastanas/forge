...
function createTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'forge-instinct-cli-projects-'));
}
...
test('status migrates legacy no-remote linked worktree project dirs to main worktree id', () => {
  ...
  try {
    ...
    runGit(mainWorktree, ['config', 'user.email', 'forge@example.test']);
    runGit(mainWorktree, ['config', 'user.name', 'FORGE Test']);
