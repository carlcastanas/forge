...
function runTests() {
  ...
  if (test('runs from a temporary directory, reads the final response, and cleans up', () => {
    ...
    let removed;
    let verified = false;
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-council-test-'));
  ...
  if (process.env.FORGE_CODEX_ISOLATION_INTEGRATION === '1') {
    if (test('real Codex cannot read an adversarial sentinel outside the review directory', () => {
      const sentinelDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-council-sentinel-'));
      const sentinelPath = path.join(sentinelDir, 'outside-review.txt');
      const secret = `FORGE-SENTINEL-${Date.now()}-${process.pid}`;
