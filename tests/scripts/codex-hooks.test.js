...
const syncScript = path.join(repoRoot, 'scripts', 'sync-forge-to-codex.sh');
...
function makeHermeticCodexEnv(homeDir, codexDir, extraEnv = {}) {
  ...
  return {
    ...
    FORGE_GLOBAL_HOOKS_DIR: hooksDir,
    CLAUDE_PACKAGE_MANAGER: 'npm',
    CLAUDE_CODE_PACKAGE_MANAGER: 'npm',
    LANG: 'C.UTF-8',
    LC_ALL: 'C.UTF-8',
...
function seedPluginCache(codexDir, manifest, files = []) {
  const cacheDir = path.join(codexDir, 'plugins', 'cache', 'forge', 'forge', packageVersion);
...
const cacheManifestWithLocalRefs = {
  ...
  mcpServers: './.mcp.json',
  interface: {
    composerIcon: './assets/forge-icon.svg',
...
if (
  test('shell test runner honors a per-invocation BASH_PATH override', () => {
    const tempDir = createTempDir('forge-missing-bash-');
    try {
...
`,
  ...
  const result = runBash(prePushHook, {
    env: {
      ...
      FORGE_PREPUSH_AUDIT: audit ? '1' : '0',
      FORGE_SKIP_GIT_HOOKS: '0',
      FORGE_SKIP_PREPUSH: '0',
      MSYS_NO_PATHCONV: '1',
    },
    cwd: projectDir,
...
if (
  test('check-plugin-cache fails when the installed cache is missing manifest-referenced files', () => {
    ...
    try {
      ...
      assert.match(result.stdout, /sync-forge-to-codex\.sh/);
    } finally {
      cleanup(homeDir);
    }
  })
...
if (
  test('check-plugin-cache reports a missing installed cache clearly', () => {
    ...
    try {
      ...
      assert.match(result.stdout, /codex plugin marketplace add carlcastanas\/FORGE/);
    } finally {
      cleanup(homeDir);
    }
  })
  ...
  test('install-global-git-hooks.sh handles quoted hook paths without shell injection', () => {
    ...
    try {
      const result = runBash(installScript, {
        env: {
          HOME: homeDir,
          FORGE_GLOBAL_HOOKS_DIR: weirdHooksDir,
        },
      });
...
if (
  test('merge-mcp-config no-ops after all recommended servers are present', () => {
    ...
    try {
      ...
      assert.match(second.stdout, /All FORGE MCP servers already present/);
...
if (
  test('merge-mcp-config repairs the invalid exa url entry from earlier FORGE versions (#2224)', () => {
...
if (
  test('merge-mcp-config update dry-run refreshes managed sections and leaves user servers alone', () => {
    ...
    try {
      ...
      assert.match(result.stdout, /\[mcp_servers\.chrome-devtools\]/);
      // Retired servers are no longer FORGE-managed: never removed or re-added.
  ...
  test('merge-mcp-config removes disabled servers without appending replacements', () => {
    ...
    try {
      ...
      const result = runNode(mergeMcpConfigScript, [configPath], {
        ...
        FORGE_DISABLED_MCPS: 'chrome-devtools',
      });

      assert.strictEqual(result.status, 0, `${result.stdout}\n${result.stderr}`);
      assert.match(result.stdout, /Disabled via FORGE_DISABLED_MCPS/);
