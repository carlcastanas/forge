...
function createFixture(extraEnv = {}) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-memory-mcp-'));
...
async function withClient(fn, options = {}) {
  ...
  const initialized = await request('initialize', {
    ...
    capabilities: {},
    clientInfo: { name: 'forge-memory-test', version: '1.0.0' },
  });
...
async function main() {
  console.log('\n=== Testing FORGE memory MCP server ===\n');
  ...
  await test('starts when the npm bin invokes the server through a symlink', async () => {
    const binRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-memory-bin-'));
    const binPath = path.join(binRoot, 'forge-memory-mcp');
  ...
  await test('denies user scope unless the server explicitly grants it', async () => {
    ...
    }, { env: { FORGE_MEMORY_ALLOW_USER_SCOPE: '1' } });
  });
  ...
  await test('requires server identity and strictly validates JSON-RPC envelopes', async () => {
    ...
    const fixture = createFixture({ FORGE_MEMORY_HARNESS: undefined });
    try {
      ...
      assert.match(started.stderr, /FORGE_MEMORY_HARNESS/);
