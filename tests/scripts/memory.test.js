...
function createFixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-memory-cli-'));
  ...
  return {
    ...
    env: {
      ...
      HOME: homeDir,
      USERPROFILE: homeDir,
      FORGE_MEMORY_PROJECT_ROOT: path.join(projectRoot, '.forge', 'memory'),
      FORGE_MEMORY_USER_ROOT: path.join(homeDir, '.forge', 'memory'),
    },
  };
}
...
test('shows memory command help directly and through the forge router', () => {
  const fixture = createFixture();
  try {
    const direct = run(MEMORY_SCRIPT, ['--help'], fixture);
    assert.strictEqual(direct.status, 0, direct.stderr);
    assert.ok(direct.stdout.includes('forge memory save'));
    assert.ok(direct.stdout.includes('forge-memory-mcp'));

    const routed = run(FORGE_SCRIPT, ['memory', '--help'], fixture);
    assert.strictEqual(routed.status, 0, routed.stderr);
    assert.ok(routed.stdout.includes('forge memory search'));
...
test('routes stdin through forge memory without dropping the body', () => {
  const fixture = createFixture();
  try {
    const saved = json(run(FORGE_SCRIPT, [
      'memory',
      'save',
      '--title', 'Routed stdin',
      '--stdin',
...
test('initializes selected scopes and reports their roots as JSON', () => {
  ...
  try {
    ...
    assert.strictEqual(payload.schemaVersion, 'forge.memory.init.v1');
...
test('saves and reads a targeted handoff without a harness-specific inbox', () => {
  ...
  try {
    ...
    assert.strictEqual(saved.schemaVersion, 'forge.memory.write.v1');
    ...
    assert.strictEqual(read.schemaVersion, 'forge.memory.read.v1');
...
test('accepts body content over stdin and finds it through bounded JSON search', () => {
  ...
  try {
    ...
    assert.strictEqual(search.schemaVersion, 'forge.memory.search.v1');
...
test('doctor is machine-readable and clean for a valid vault', () => {
  ...
  try {
    ...
    assert.strictEqual(report.schemaVersion, 'forge.memory.doctor.v1');
...
test('rejects ambiguous body sources and does not expose a trust promotion flag', () => {
  ...
  try {
    ...
    assert.strictEqual(
      fs.existsSync(path.join(fixture.projectRoot, '.forge', 'memory')),
      false
    );
  } finally {
    fs.rmSync(fixture.root, { recursive: true, force: true });
...
test('global dry-run rejects every mutating memory command without creating a vault', () => {
  ...
  cases.forEach(args => {
    ...
    try {
      ...
      assert.strictEqual(
        fs.existsSync(path.join(fixture.homeDir, '.forge', 'memory')),
        false,
