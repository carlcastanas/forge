...
function run(args = [], options = {}) {
  const inheritedEnv = Object.fromEntries(
    Object.entries(process.env).filter(([key]) => key !== 'FORGE_DRY_RUN' && key !== 'CODEX_HOME')
  );
...
function runTests() {
  ...
  if (test('uninstalls files from a real install-apply state and preserves unrelated files', () => {
    ...
    try {
      ...
      const statePath = path.join(normalizedProjectRoot, '.cursor', 'forge-install-state.json');
  ...
  if (test('reverses non-copy operations and keeps unrelated files', () => {
    ...
    try {
      ...
      const statePath = path.join(normalizedTargetRoot, 'forge-install-state.json');
  ...
  // #2952: the global `forge --dry-run uninstall` prefix sets FORGE_DRY_RUN=1.
  // The uninstaller must honor it exactly like the subcommand-level flag.
  if (test('honors global FORGE_DRY_RUN=1 without mutating managed files (#2952)', () => {
    const homeDir = createTempDir('uninstall-home-');
    const projectRoot = createTempDir('uninstall-project-');
    ...
    try {
      ...
      const parsed = JSON.parse(uninstallResult.stdout);
      assert.strictEqual(parsed.dryRun, true, 'FORGE_DRY_RUN=1 must enable dry-run mode');
      assert.ok(parsed.results[0].plannedRemovals.includes(renderedPath));
  ...
  if (test('auto-detects legacy sync-forge-to-codex.sh install and removes artifacts without touching conversations or unrelated config keys', () => {
    ...
    try {
      ...
      assert.strictEqual(uninstallResult.code, 0, uninstallResult.stderr);
      assert.ok(!uninstallResult.stdout.includes('No FORGE install-state files found'), uninstallResult.stdout);
  ...
  if (test('rejects an invalid global dry-run value before legacy cleanup', () => {
    ...
    try {
      ...
      const uninstallResult = run(['--legacy-codex-sync'], {
        cwd: projectRoot,
        homeDir,
        env: { FORGE_DRY_RUN: 'true' },
      });

      assert.strictEqual(uninstallResult.code, 1);
      assert.match(uninstallResult.stderr, /FORGE_DRY_RUN must be "1" or "0" when set/);
