    ...
    try {
      ...
      assert.ok(report.requirements.some(item => (
        item.id === 'forge-shield-enterprise-iteration'
          && item.gap === 'deepen live operator approval and readback after the release gates'
      ...
      assert.ok(report.requirements.some(item => (
        item.id === 'owner-approval-packet'
          && item.status === 'current'
          && item.evidence.includes('release, package, plugin, video, social, and outbound decisions')
      ...
      assert.ok(report.requirements.some(item => (
        item.id === 'linear-roadmap-and-progress'
          && item.status === 'current'
          && item.evidence.includes('May 20 release-gate comments')
