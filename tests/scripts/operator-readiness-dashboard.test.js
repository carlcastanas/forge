...
function seedRepo(rootDir, overrides = {}) {
  const files = {
    ...
    'docs/releases/2.0.0/forge-2-hypergrowth-release-command-center.md': [
...
function runTests() {
  ...
  if (test('seeded repo emits an objective audit with remaining work', () => {
    ...
    try {
      ...
      assert.strictEqual(report.schema_version, 'forge.operator-readiness-dashboard.v1');
      ...
      assert.ok(report.requirements.some(item => item.id === 'forge-tools-next-level' && item.status === 'in_progress'));
      assert.ok(report.requirements.some(item => (
        item.id === 'forge-shield-enterprise-iteration'
      ...
      assert.ok(report.requirements.some(item => (
        item.id === 'forge-tools-next-level'
          ...
          && item.evidence.includes('FORGE-Tools Dependabot alert remediation')
      )));
      assert.ok(report.requirements.some(item => (
      ...
      assert.ok(report.requirements.some(item => (
        item.id === 'supply-chain-local-protection'
          && item.artifact.includes('Forge Shield package-manager hardening')
      ...
      assert.ok(!report.top_actions.some(item => item.id === 'forge-preview-pack'));
  ...
  if (test('preview pack and Hermes gates stay in progress until smoke gate is wired', () => {
    ...
    try {
      ...
      const report = buildSeededReport(rootDir);
      const previewPack = report.requirements.find(item => item.id === 'forge-preview-pack');
  ...
  if (test('Forge Shield enterprise evidence covers export and policy promotion markers', () => {
    const cases = [
      {
        marker: 'Forge Shield PR #92',
        gap: 'workflow automation around protected rollout and richer runtime review UX pending after policy promotion shipped'
      },
      {
        marker: 'Forge Shield #92',
        gap: 'workflow automation around protected rollout and richer runtime review UX pending after policy promotion shipped'
      },
      {
      ...
      {
        marker: 'Forge Shield PR #91',
        gap: 'workflow automation plus policy promotion/review UX pending after policy export shipped'
      },
      {
        marker: 'Forge Shield #91',
        gap: 'workflow automation plus policy promotion/review UX pending after policy export shipped'
      },
      {
    ...
    for (const { marker, gap } of cases) {
      const rootDir = createTempDir('operator-dashboard-forge-shield-');
      ...
      try {
        ...
        const report = buildSeededReport(rootDir);
        const item = report.requirements.find(requirement => requirement.id === 'forge-shield-enterprise-iteration');
  ...
  if (test('markdown output can be written as the dashboard artifact', () => {
    ...
    try {
      ...
      assert.ok(written.includes('# FORGE Operator Readiness Dashboard'));
  ...
  if (test('text output renders compact status and top actions', () => {
    ...
    try {
      ...
      assert.ok(stdout.includes('FORGE Operator Readiness Dashboard'));
