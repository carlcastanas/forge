/**
 * Source-level tests for scripts/sync-forge-to-codex.sh
 */
...
const scriptPath = path.join(__dirname, '..', '..', 'scripts', 'sync-forge-to-codex.sh');
...
function runTests() {
  console.log('\n=== Testing sync-forge-to-codex.sh ===\n');
  ...
  if (test('sync script carries the Codex navigation guide referenced by AGENTS', () => {
    ...
    assert.ok(source.includes('require_path "$CODEX_NAV_GUIDE_SRC" "FORGE Codex navigation guide"'), 'Expected sync preflight for Codex navigation guide');
  ...
  if (test('sync script avoids GNU-only grep -P parsing', () => {
    assert.ok(!source.includes('grep -oP'), 'sync-forge-to-codex.sh should remain portable across BSD and GNU environments');
  })) passed++; else failed++;
  ...
  if (test('sync records a versioned ownership manifest before mutating Codex state', () => {
    ...
    assert.ok(source.includes('record_managed_path "${FORGE_GLOBAL_HOOKS_DIR:-$CODEX_HOME/git-hooks}/pre-commit"'));
  })) passed++; else failed++;
