...
function runTests() {
  ...
  if (test('README leads with the idempotent guided plugin setup path', () => {
    ...
    const topGuidedCommandIndex = readme.indexOf('npx forge-universal setup', topClaudeSectionIndex);
    const nativePluginCommandIndex = readme.indexOf('/plugin marketplace add', topClaudeSectionIndex);
    const installSectionIndex = readme.indexOf('## Install FORGE');
    const guidedCommandIndex = readme.indexOf('npx forge-universal setup', installSectionIndex);
    ...
    assert.ok(
      readme.includes('installs, updates, or safely moves `forge@forge`'),
    ...
    assert.ok(
      ...
      'README should distinguish provider-owned slash behavior from FORGE setup behavior'
    );
    assert.ok(
      readme.includes('`/forge:configure-forge`'),
    ...
    assert.ok(
      ...
      'README should not imply that the current setup wizard installs every FORGE harness'
    );
  })) passed++; else failed++;

  if (test('README documents modern package-runner alternatives', () => {
    assert.ok(readme.includes('pnpm dlx forge-universal setup'));
    assert.ok(readme.includes('yarn dlx forge-universal setup'));
    assert.ok(readme.includes('bunx forge-universal setup'));
    assert.ok(
  ...
  if (test('README documents reset and uninstall flow', () => {
    assert.ok(
      readme.includes('### Reset / Uninstall FORGE'),
    ...
    assert.ok(
      readme.includes('node scripts/forge.js list-installed'),
      'README should document install-state inspection before reinstalling'
    );
    assert.ok(
      readme.includes('node scripts/forge.js doctor'),
    ...
    assert.ok(
      readme.includes('FORGE only removes files recorded in its install-state.'),
  ...
  if (test('README documents low-context no-hooks install path', () => {
    ...
    assert.ok(
      readme.includes('npx forge-universal install --profile minimal --target claude'),
      'README should document the published universal-package minimal profile command'
    );
    assert.ok(
      !/^\s*npx forge-install\b/m.test(readme),
      'README code examples must not invoke the unpublished forge-install package'
    );
    assert.ok(
  ...
  if (test('README documents consult-based component discovery', () => {
    ...
    assert.ok(
      readme.includes('npx forge-universal consult "security reviews" --target claude'),
  ...
  if (test('README never invokes the unrelated forge npm package', () => {
    assert.ok(
      !/\bnpx forge\s/.test(readme),
      'README one-shot commands should use the published forge-universal package name'
    );
  })) passed++; else failed++;

  if (test('README gives the native guided Codex and managed Kimi dry-run paths', () => {
    assert.ok(
      readme.includes('npx forge-universal install --guided --harness codex --dry-run'),
      'README should verify Codex through the native guided reconciler'
    );
    assert.ok(
      !readme.includes('npx forge-universal install --profile core --target codex --dry-run'),
      'README should not present the legacy managed Codex adapter as the native lifecycle'
    );
    assert.ok(
      readme.includes('npx forge-universal install --profile core --target kimi --dry-run')
    );
  ...
  if (test('README describes the post-release universal install contract', () => {
    assert.ok(
      ...
      'README should state the runtime required by forge-universal'
    );
    assert.ok(
    ...
    assert.ok(
      readme.includes('| FORGE hooks | Native plugin hooks | Native reviewed subset with explicit trust |'),
    ...
    assert.ok(
      readme.includes("# Recommended current install: add FORGE's native plugin from the repo marketplace"),
    ...
    assert.ok(
      !readme.includes('# Automatic setup: sync FORGE assets'),
  ...
  if (test('README documents Cursor agent namespace and loading caveat', () => {
    assert.ok(
      readme.includes('`.cursor/agents/forge-*.md`'),
    ...
    assert.ok(
      readme.includes('FORGE does not install root `AGENTS.md` into `.cursor/`.'),
  ...
  if (test('README explains plugin-path cleanup and rules scoping', () => {
    ...
    assert.ok(
      readme.includes('~/.claude/rules/forge/'),
      'README should steer plugin-path rules into an FORGE-owned namespace'
    );
  })) passed++; else failed++;

  if (test('rules README mirrors FORGE namespaced install path', () => {
    assert.ok(
      rulesReadme.includes('mkdir -p ~/.claude/rules/forge'),
      'rules README should create the FORGE-owned user-level rules namespace'
    );
    assert.ok(
      rulesReadme.includes('cp -r rules/common ~/.claude/rules/forge/'),
      'rules README should copy common rules under ~/.claude/rules/forge/'
    );
    assert.ok(
      rulesReadme.includes('cp -r rules/typescript ~/.claude/rules/forge/'),
      'rules README should copy language rules under ~/.claude/rules/forge/'
    );
    assert.ok(
      rulesReadme.includes('mkdir -p .claude/rules/forge'),
      'rules README should document the project-local FORGE namespace'
    );
    assert.ok(
