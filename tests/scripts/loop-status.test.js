...
function createTempHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'forge-loop-status-home-'));
}
...
function runTests() {
  ...
  if (test('parses write-dir snapshot option', () => {
    const options = parseArgs([
      ...
      'scripts/loop-status.js',
      '--write-dir',
      '/tmp/forge-loop-snapshots',
    ]);

    assert.strictEqual(options.writeDir, '/tmp/forge-loop-snapshots');
  })) passed++; else failed++;
  ...
  if (test('exit-code mode returns 1 for scan errors without attention signals', () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-loop-status-missing-'));
  ...
  if (test('watch mode emits repeated JSON status frames', () => {
    ...
    try {
      ...
      assert.strictEqual(frames[0].schemaVersion, 'forge.loop-status.v1');
      assert.strictEqual(frames[1].schemaVersion, 'forge.loop-status.v1');
  ...
  if (test('writes per-session status snapshots and index when write-dir is set', () => {
    ...
    try {
      ...
      assert.strictEqual(stdoutPayload.schemaVersion, 'forge.loop-status.v1');
      ...
      assert.strictEqual(indexPayload.schemaVersion, 'forge.loop-status.index.v1');
      ...
      assert.strictEqual(snapshotPayload.schemaVersion, 'forge.loop-status.session.v1');
  ...
  if (test('cleans temporary snapshot files when atomic rename fails', () => {
    const snapshotDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-loop-status-rename-failure-'));
