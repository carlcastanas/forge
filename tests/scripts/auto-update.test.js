...
function ensureFakeRepo(repoRoot) {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: 'forge', version: '1.10.0' }, null, 2)
  );
...
function runTests() {
  ...
  if (test('parseArgs reads repo-root, target, dry-run, and json flags', () => {
    ...
    assert.strictEqual(parsed.repoRoot, '/tmp/forge');
  ...
  if (test('deriveRepoRootFromState uses sourcePath and sourceRelativePath', () => {
    const state = {
      operations: [
        {
          sourcePath: path.join('/tmp', 'forge', 'scripts', 'setup-package-manager.js'),
    ...
    assert.strictEqual(
      ...
      path.resolve(path.join('/tmp', 'forge'))
    );
  })) passed += 1; else failed += 1;
  ...
  if (test('deriveRepoRootFromState fails when source metadata is unavailable', () => {
    assert.throws(
      ...
      /Unable to infer FORGE repo root/
    );
  })) passed += 1; else failed += 1;
  ...
  if (test('determineInstallCwd uses the project root for project installs', () => {
    ...
    assert.strictEqual(determineInstallCwd(record, '/tmp/forge'), path.join('/tmp', 'project'));
  })) passed += 1; else failed += 1;
  ...
  if (test('runAutoUpdate rejects mixed inferred repo roots', () => {
    ...
    try {
      ...
      assert.throws(
        ...
        /Multiple FORGE repo roots detected/
      );
    } finally {
      cleanup(homeDir);
      cleanup(projectRoot);
            ...
            if (command === process.execPath) {
              return {
                stdout: JSON.stringify({
                  dryRun: false,
                  result: {
                    installStatePath: path.join(projectRoot, '.cursor', 'forge-install-state.json'),
                  },
                }),
                stderr: '',
              };
  ...
  if (test('runAutoUpdate excludes residual legacy Antigravity records', () => {
    ...
    try {
      ...
      const legacy = {
        ...canonical,
        installStatePath: path.join(projectRoot, '.agent', 'forge-install-state.json'),
        legacy: true,
      };
      const commands = [];
  ...
  if (test('runAutoUpdate gives legacy-only OpenCode migration guidance', () => {
    ...
    try {
      ...
      const legacy = {
        ...
        installStatePath: path.join(homeDir, '.opencode', 'forge-install-state.json'),
        legacy: true,
