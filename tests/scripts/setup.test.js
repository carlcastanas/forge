...
const forgeScript = path.join(repoRoot, 'scripts', 'forge.js');
...
function createFixture(state = {}) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'forge setup cli '));
...
function runSetup(fixture, args, options = {}) {
  const env = {
    ...
    FORGE_TEST_CLAUDE_STATE: fixture.statePath,
    FORGE_TEST_CLAUDE_CALLS: fixture.callsPath,
    ...options.env,
  };
...
const SETUP_SPINNER_PATTERN = /[⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏]\s+Applying FORGE setup/;
...
test('an existing install without --scope updates its detected scope', () => {
  withFixture({
    plugins: [{ id: 'forge@forge', scope: 'project', enabled: true, version: '1.9.0' }],
    marketplaces: [{
      name: 'forge',
      source: 'github',
      repo: 'your-org/FORGE',
...
test('dry-run leaves a pristine HOME unchanged when Claude inventory creates backups', () => {
  withFixture({}, fixture => {
    ...
    const result = runSetup(fixture, [
      ...
      env: { FORGE_TEST_CLAUDE_CREATE_READ_ARTIFACTS: '1' },
    });
    assert.strictEqual(result.status, 0, result.stderr);
    assert.deepStrictEqual(fs.readdirSync(fixture.homeDir), []);
    assert.deepStrictEqual(fs.readdirSync(fixture.projectRoot), []);
...
test('dry-run isolates pre-existing Claude backups and symlinked project settings', () => {
  ...
  withFixture({}, fixture => {
    ...
    fs.writeFileSync(settingsTarget, '{"enabledPlugins":{"forge@forge":true}}\n');
    ...
    const result = runSetup(fixture, [
      ...
      defaultClaudeConfig: true,
      env: {
        FORGE_TEST_CLAUDE_CREATE_READ_ARTIFACTS: '1',
        FORGE_TEST_CLAUDE_OVERWRITE_READ_ARTIFACTS: '1',
        FORGE_TEST_CLAUDE_WRITE_XDG_DATA: '1',
...
test('setup automatically migrates an existing install to the selected scope and hooks', () => {
  withFixture({
    plugins: [{ id: 'forge@forge', scope: 'local', enabled: true, version: '1.9.0' }],
    marketplaces: [{
      name: 'forge',
      source: 'github',
      repo: 'your-org/FORGE',
    ...
    assert.ok(calls.some(argv => (
      argv.join(' ') === 'plugin install forge@forge --scope user'
        + ' --config hooks_enabled=true --config hook_profile=minimal'
    )));
    assert.ok(calls.some(argv => (
      argv.join(' ') === 'plugin uninstall forge@forge --scope local --keep-data'
    )));
    ...
    assert.strictEqual(settings.pluginConfigs['forge@forge'].options.hooks_enabled, true);
    assert.strictEqual(settings.pluginConfigs['forge@forge'].options.hook_profile, 'minimal');
  });
});
...
test('setup resumes a safe two-scope migration without requiring --move-scope', () => {
  withFixture({
    plugins: [
      { id: 'forge@forge', scope: 'local', enabled: true, version: '1.9.0' },
      { id: 'forge@forge', scope: 'user', enabled: true, version: '2.0.0' },
    ],
    marketplaces: [{
      name: 'forge',
      source: 'github',
  ...
  for (const sourceScope of scopes) {
    for (const destinationScope of scopes.filter(scope => scope !== sourceScope)) {
      for (const hookMode of hooks) {
        withFixture({
          plugins: [
            { id: 'forge@forge', scope: sourceScope, enabled: true, version: '1.9.0' },
            { id: 'forge@forge', scope: destinationScope, enabled: true, version: '2.0.0' },
          ],
          marketplaces: [{
            name: 'forge',
            source: 'github',
          ...
          assert.ok(calls.some(argv => (
            argv.join(' ') === `plugin uninstall forge@forge --scope ${sourceScope} --keep-data`
          )));
          const state = JSON.parse(fs.readFileSync(fixture.statePath, 'utf8'));
          assert.deepStrictEqual(state.plugins, [{
            id: 'forge@forge',
          ...
          const stored = settings.pluginConfigs['forge@forge'].options;
...
test('--move-scope remains explicit about its destination', () => {
  withFixture({
    plugins: [{ id: 'forge@forge', scope: 'user', enabled: true, version: '1.9.0' }],
  }, fixture => {
    const result = runSetup(fixture, [
      '--mode', 'claude-plugin',
      '--move-scope',
...
test('destination-only --move-scope is an idempotent first call', () => {
  withFixture({
    plugins: [{ id: 'forge@forge', scope: 'local', enabled: true, version: '2.0.0' }],
  }, fixture => {
    const result = runSetup(fixture, [
      '--mode', 'claude-plugin',
      '--scope', 'local',
...
test('destination-only --move-scope applies explicit hook preferences', () => {
  withFixture({
    ...
    assert.strictEqual(settings.pluginConfigs['forge@forge'].options.hooks_enabled, true);
    assert.strictEqual(settings.pluginConfigs['forge@forge'].options.hook_profile, 'strict');
  });
});
...
test('migration dry-run JSON exposes ordered actions without mutation', () => {
  withFixture({
    ...
    assert.deepStrictEqual(payload.plannedActions.slice(-4), [
      ['plugin', 'list', '--json'],
      ['plugin', 'list', '--json'],
      ['plugin', 'uninstall', 'forge@forge', '--scope', 'user', '--keep-data'],
      ['plugin', 'list', '--json'],
    ]);
    assert.strictEqual(hasMutation(fixture), false);
  });
...
test('migration JSON failures retain phase, scopes, and exact recovery', () => {
  withFixture({
    ...
    failures: [{
      argv: ['plugin', 'uninstall', 'forge@forge', '--scope', 'user', '--keep-data'],
      status: 9,
    ...
    assert.deepStrictEqual(payload.error.recovery, [
      'claude plugin uninstall forge@forge --scope user --keep-data',
...
test('forge setup delegates to the focused setup command', () => {
  const result = spawnSync(process.execPath, [forgeScript, 'setup', '--help'], {
    cwd: repoRoot,
    encoding: 'utf8',
    timeout: 15000,
  });
  assert.strictEqual(result.status, 0, result.stderr);
  assert.match(result.stdout, /FORGE (guided )?setup/i);
  assert.match(result.stdout, /claude-plugin/);
});

test('forge setup preserves a real terminal for the interactive wizard', () => {
  if (process.platform === 'win32') return;
  ...
  withFixture({}, fixture => {
    ...
    assert.match(result.stdout, /How should FORGE hooks run\?/);
...
test('confirmed interactive apply starts immediately and clears the spinner on success', () => {
  ...
  withFixture({}, fixture => {
    ...
    assertSetupSpinnerLifecycle(
      ...
      /FORGE installed forge@forge at project scope/
    );
  });
});
  ...
  for (const [scopeIndex, scope] of scopes.entries()) {
    for (const [hookIndex, hookMode] of hooks.entries()) {
      withFixture({}, fixture => {
        ...
        assert.ifError(result.error);
        assert.strictEqual(result.status, 0, `${result.stdout}\n${result.stderr}`);
        assert.match(result.stdout, new RegExp(`FORGE installed forge@forge at ${scope} scope`));
...
test('all interactive choices from an existing install update or migrate to the selected configuration', () => {
  ...
  for (const [sourceIndex, sourceScope] of scopes.entries()) {
    for (const [selectedIndex, selectedScope] of scopes.entries()) {
      for (const [hookIndex, hookMode] of hooks.entries()) {
        withFixture({
          plugins: [{ id: 'forge@forge', scope: sourceScope, enabled: true, version: '1.9.0' }],
          marketplaces: [{
            name: 'forge',
            source: 'github',
            repo: 'your-org/FORGE',
          ...
          assert.match(
            result.stdout,
            new RegExp(`FORGE ${expectedAction} forge@forge at ${selectedScope} scope`)
          );
          assert.match(result.stdout, new RegExp(`Hook preference: ${hookMode}`));
...
test('interactive named choices install and persist the selected configuration', () => {
  ...
  withFixture({}, fixture => {
    ...
    assert.ifError(result.error);
    assert.strictEqual(result.status, 0, `${result.stdout}\n${result.stderr}`);
    assert.match(result.stdout, /FORGE installed forge@forge at project scope/);
...
test('invalid interactive choices explain the problem and allow a retry', () => {
  ...
  withFixture({}, fixture => {
    ...
    assert.match(result.stdout, /FORGE would-install forge@forge at project scope/);
...
test('interactive cancellation after non-default choices performs no mutation', () => {
  ...
  withFixture({}, fixture => {
    ...
    assert.ifError(result.error);
    assert.strictEqual(result.status, 0, `${result.stdout}\n${result.stderr}`);
    assert.match(result.stdout, /FORGE cancelled forge@forge at project scope/);
    assertNoSetupSpinner(`${result.stdout}${result.stderr}`);
    assert.strictEqual(hasMutation(fixture), false);
...
test('interactive mode flag still prompts for missing scope and hook choices', () => {
  ...
  withFixture({}, fixture => {
    ...
    assert.match(result.stdout, /Where should Claude enable forge@forge\?/);
    assert.match(result.stdout, /How should FORGE hooks run\?/);
    assert.match(result.stdout, /FORGE would-install forge@forge at local scope/);
    assert.match(result.stdout, /Hook preference: strict/);
  });
});
...
test('interactive defaults preserve an existing install scope and hook preference', () => {
  ...
  withFixture({
    ...
    assert.match(result.stdout, /FORGE would-update forge@forge at local scope/);
    assert.match(result.stdout, /Hook preference: minimal/);
    assert.strictEqual(hasMutation(fixture), false);
  });
});
...
test('partial migration requires an explicit destination and preserves stored hook defaults', () => {
  ...
  withFixture({
    plugins: [
      { id: 'forge@forge', scope: 'user', enabled: true, version: '1.9.0' },
      { id: 'forge@forge', scope: 'project', enabled: true, version: '2.0.0' },
    ],
    marketplaces: [{
      name: 'forge',
      source: 'github',
    ...
    assert.match(result.stdout, /Please choose 1, 2, or 3/);
    assert.match(result.stdout, /Choose \[2\]:/);
    assert.match(result.stdout, /FORGE would-resume forge@forge at project scope/);
