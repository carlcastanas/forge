...
function approvedPacketContent(overrides = {}, release = CURRENT_RELEASE) {
  ...
  return [
    `# FORGE v${release} Owner Approval Packet`,
    '',
    '## Decision Register',
    '',
...
function finalLedgerContent(extra = '', release = CURRENT_RELEASE) {
  return [
    `# FORGE v${release} Release URL Ledger`,
    '',
...
function manifestContent(release = CURRENT_RELEASE) {
  return [
    `# FORGE v${release} Preview Pack Manifest`,
    '',
    '| Artifact | Role | Gate |',
    '| --- | --- | --- |',
...
function runTests() {
  ...
  if (test('seeded approved release passes every publication approval check', () => {
    ...
    try {
      ...
      assert.strictEqual(report.schema_version, 'forge.release-approval-gate.v1');
    ...
    try {
      ...
      seedRepo(rootDir, {
        [`${releaseDir}/owner-approval-packet-2026-05-19.md`]: approvedPacketContent({
          ...
          'Conference, podcast, and community outreach': 'block',
        }),
      });
      ...
      assert.ok(decisions.evidence.includes('Conference, podcast, and community outreach=block'));
    ...
    try {
      ...
      seedRepo(rootDir, {
        [`${releaseDir}/release-url-ledger-2026-05-19.md`]: [
          `# FORGE v${CURRENT_RELEASE} Release URL Ledger`,
          '',
          '## Approval-Gated URLs',
          '',
  ...
  if (test('announcement drafts fail on unresolved placeholders and private paths', () => {
    ...
    try {
      const releaseDir = releaseDirFor(CURRENT_RELEASE);
      seedRepo(rootDir, {
        [`${releaseDir}/x-thread.md`]: 'Ship copy with <video-url> and /Users/you/raw-footage.',
      });
