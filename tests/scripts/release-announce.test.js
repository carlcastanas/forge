...
assert.equal(releaseMarker('v2.2.0'), '<!-- forge-release:v2.2.0 -->');
...
assert.equal(announcementKey({ repository: 'your-org/FORGE', discussionId: 'D_kw123' }), 'your-org/FORGE:discussion:D_kw123');

const payload = buildDiscordPayload({
  title: '@everyone FORGE 2.2.0',
  body: 'A'.repeat(5000),
  url: 'https://github.com/your-org/forge/discussions/3000',
  key: 'your-org/FORGE:discussion:D_kw123',
});
assert.deepEqual(payload.allowed_mentions, { parse: [] });
assert.equal(payload.embeds.length, 1);
assert.ok(payload.embeds[0].description.length <= 4000);
assert.equal(payload.embeds[0].footer.text, 'forge:D_kw123');
assert.equal(payload.embeds[0].url, 'https://github.com/your-org/forge/discussions/3000');
assert.equal(payload.enforce_nonce, true);
assert.match(payload.nonce, /^forge-[a-f0-9]{16}$/);
...
const receiptMarker = discussionReceiptMarker('your-org/FORGE:discussion:D_kw123');
assert.match(receiptMarker, /^<!-- forge-discord-receipt:[a-f0-9]{32} -->$/);
