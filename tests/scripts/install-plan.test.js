...
function runTests() {
  ...
  if (test('shows help with no arguments', () => {
    ...
    assert.ok(result.stdout.includes('Inspect FORGE selective-install manifests'));
  })) passed++; else failed++;
  ...
  if (test('loads planning intent from forge-install.json', () => {
    const configDir = path.join(__dirname, '..', 'fixtures', 'tmp-install-plan-config');
    const configPath = path.join(configDir, 'forge-install.json');
  ...
  if (test('auto-detects planning intent from project forge-install.json', () => {
