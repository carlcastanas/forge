...
test('forge welcome disables ANSI color when stdout is redirected', () => {
  const env = { ...process.env, TERM: 'xterm-256color' };
  delete env.NO_COLOR;
  const result = spawnSync(process.execPath, [forgeScript, 'welcome'], {
    cwd: repoRoot,
    encoding: 'utf8',
    env,
  });
...
test('forge welcome supports explicit update and configured outcomes', () => {
  const cases = [
    ['updated', /FORGE is updated/],
    ['configured', /FORGE is configured/],
    ['migrated', /FORGE is configured/],
    ['resumed', /FORGE is configured/],
    ['already-migrated', /FORGE is configured/],
  ];
