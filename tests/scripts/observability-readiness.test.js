...
function seedMinimalRepo(rootDir, overrides = {}) {
  const files = {
    ...
    'scripts/work-items.js': 'sync-github github-pr github-issue sourceClosedAt forge-work-items-sync-github',
    'scripts/hooks/session-activity-tracker.js': 'tool-usage.jsonl session_id tool_name',
    'forge2/src/observability/mod.rs': 'ToolCallEvent RiskAssessment ToolLogger',
    'forge2/src/session/store.rs': 'insert_tool_log query_tool_logs',
    'forge2/src/session/manager.rs': 'sync_tool_activity_metrics tool-usage.jsonl',
    ...
    'examples/hud-status-contract.json': JSON.stringify({
      schema_version: 'forge.hud-status.v1',
      context: {},
      toolCalls: {},
      activeAgents: [],
      todos: {},
    ...
    'tests/docs/forge2-release-surface.test.js': 'publication-readiness.md',
  };
...
function runTests() {
  ...
  if (test('missing tool logger surfaces become prioritized top actions', () => {
    ...
    try {
      seedMinimalRepo(projectRoot, {
        'forge2/src/observability/mod.rs': 'ToolCallEvent only'
      });
      const report = buildReport(projectRoot);

      assert.strictEqual(report.ready, false);
      assert.ok(report.top_actions.some(action => action.id === 'forge2-tool-risk-ledger'));
      assert.ok(report.checks.some(check => check.id === 'forge2-tool-risk-ledger' && !check.pass));
    } finally {
      cleanup(projectRoot);
    }
  })) passed++; else failed++;
