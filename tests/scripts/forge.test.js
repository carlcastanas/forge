...
const SCRIPT = path.join(__dirname, '..', '..', 'scripts', 'forge.js');
...
function runCli(args, options = {}) {
  ...
  const inheritedEnv = Object.fromEntries(
    Object.entries(process.env).filter(([key]) => key !== 'FORGE_DRY_RUN')
  );
...
function main() {
  ...
  const tests = [
    ...
    ['delegates explicit install command', () => {
      const homeDir = createTempDir('forge-cli-install-home-');
      try {
    ...
    ['delegates consult command', () => {
      ...
      assert.strictEqual(result.status, 0, result.stderr);
      const payload = parseJson(result.stdout);
      assert.strictEqual(payload.schemaVersion, 'forge.consult.v1');
    ...
    ['delegates lifecycle commands', () => {
      const homeDir = createTempDir('forge-cli-home-');
      const projectRoot = createTempDir('forge-cli-project-');
    ...
    ['keeps uninstall read-only when global --dry-run precedes the command', () => {
      const homeDir = createTempDir('forge-cli-uninstall-home-');
      const projectRoot = createTempDir('forge-cli-uninstall-project-');
    ...
    ['delegates session-inspect command', () => {
      ...
      assert.strictEqual(payload.workers[0].branch, 'feat/forge-cli');
    }],
    ['delegates loop-status command', () => {
      const homeDir = createTempDir('forge-cli-home-');
      const transcriptDir = path.join(homeDir, '.claude', 'projects', '-tmp-forge');
      ...
      assert.strictEqual(result.status, 0, result.stderr);
      const payload = parseJson(result.stdout);
      assert.strictEqual(payload.schemaVersion, 'forge.loop-status.v1');
    ...
    ['delegates feedback command', () => {
      ...
      assert.strictEqual(result.status, 0, result.stderr);
      const payload = parseJson(result.stdout);
      assert.strictEqual(payload.schemaVersion, 'forge.feedback.v1');
    ...
    ['delegates security-ioc-scan command', () => {
      const projectRoot = createTempDir('forge-cli-ioc-scan-');
      fs.writeFileSync(
