/**
 * Published npm binary aliases for the primary FORGE CLI.
 *
...
function getPackedFixture() {
  ...
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-universal-bin-'));
...
function prepareLocalPackedProject(packageManager) {
  ...
  const packageDirectory = path.join(modulesDirectory, 'forge-universal');
  const binDirectory = path.join(modulesDirectory, '.bin');

  fs.mkdirSync(projectDirectory, { recursive: true });
  fs.writeFileSync(
    path.join(projectDirectory, 'package.json'),
    `${JSON.stringify({ name: 'forge-packed-smoke', private: true }, null, 2)}\n`
  );
  ...
  for (const executable of ['forge', 'forge-universal']) {
    ...
    if (process.platform === 'win32') {
      ...
      fs.writeFileSync(
        cmdPath,
        `@ECHO off\r\nnode "%~dp0\\..\\forge-universal\\${target}" %*\r\n`
      );
    } else {
      fs.symlinkSync(
        path.join('..', 'forge-universal', packageJson.bin[executable]),
    ...
    case 'npm':
      {
        ...
        return {
          ...
          args: [
            'exec',
            '--offline',
            '--package=./node_modules/forge-universal',
            '--',
            executable,
            ...args,
          ],
...
console.log(`\n=== FORGE universal packed binary tests (${activePackageManager}) ===\n`);
...
test('published package exposes forge and forge-universal through scripts/forge.js', () => {
  assert.strictEqual(packageJson.bin.forge, 'scripts/forge.js');
  assert.strictEqual(packageJson.bin['forge-universal'], 'scripts/forge.js');
  assert.deepStrictEqual(packageLock.packages[''].bin, packageJson.bin);

  const fixture = getPackedFixture();
  assert.ok(
    fixture.publishedPaths.has('scripts/forge.js'),
    'npm package should publish the shared CLI target'
  );
});

test('packed forge-universal launches the guided Claude setup help', () => {
  const result = launchPackedBinary('forge-universal', ['setup', '--help']);
  assert.match(result.stdout, /FORGE guided setup/);
});

test('packed forge-universal launches the guided multi-harness help', () => {
  const result = launchPackedBinary(
    'forge-universal',
    ['install', '--guided', '--help']
  );
  assert.match(result.stdout, /FORGE guided multi-harness install/);
  assert.match(result.stdout, /Claude Code/);
  assert.match(result.stdout, /Codex/);
  assert.match(result.stdout, /Kimi/);
});

test('packed forge alias launches the primary dispatcher', () => {
  const result = launchPackedBinary('forge', ['--help']);
  assert.match(result.stdout, /FORGE selective-install CLI/);
  assert.match(result.stdout, /forge install --guided/);
});
