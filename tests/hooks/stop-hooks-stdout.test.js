 ...
 *
 * Stop hooks follow the FORGE pass-through convention (echo stdin on stdout).
...
const workDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-stop-stdout-')); // non-git cwd
const dataHome = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-stop-data-'));
...
function hookEnv() {
  const env = {
    ...
    FORGE_AGENT_DATA_HOME: dataHome,
    CLAUDE_SESSION_ID: `stop-stdout-test-${process.pid}`
  };
  delete env.FORGE_GATEGUARD;
  delete env.FORGE_DISABLED_HOOKS;
