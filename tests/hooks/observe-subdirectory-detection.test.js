...
function createTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'forge-observe-subdir-test-'));
}
...
function gitInit(dir) {
  ...
  const remoteResult = spawnSync(
    'git',
    ['remote', 'add', 'origin', 'https://github.com/example/forge-test.git'],
...
function readSingleProjectMetadata(homeDir) {
  const projectsDir = path.join(homeDir, '.local', 'share', 'forge-homunculus', 'projects');
