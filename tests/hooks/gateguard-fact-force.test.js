...
function runTests() {
  ...
  // --- Test 9: respects FORGE_DISABLED_HOOKS ---
  clearState();
  if (
    test('respects FORGE_DISABLED_HOOKS (skips when disabled)', () => {
      const input = {
        tool_name: 'Edit',
        tool_input: { file_path: '/src/disabled.js', old_string: 'a', new_string: 'b' }
      };
      const result = runHook(input, {
        FORGE_DISABLED_HOOKS: 'pre:edit-write:gateguard-fact-force'
      });
  ...
  clearState();
  if (
    test('respects FORGE_GATEGUARD=off without writing gate state', () => {
      const input = {
        tool_name: 'Write',
        tool_input: { file_path: '/src/env-disabled.js', content: 'export const ok = true;' }
      };
      const result = runHook(input, { FORGE_GATEGUARD: 'off' });
      const output = parseOutput(result.stdout);
  ...
  if (
    test('denial messages include direct recovery escape hatch', () => {
      ...
      assert.strictEqual(output.hookSpecificOutput.permissionDecision, 'deny');
      assert.ok(output.hookSpecificOutput.permissionDecisionReason.includes('FORGE_GATEGUARD=off'), 'denial reason should show the direct recovery env toggle');
      assert.ok(output.hookSpecificOutput.permissionDecisionReason.includes('FORGE_DISABLED_HOOKS'), 'denial reason should mention the existing hook-id disable control');
  ...
  if (
    test('destructive Bash denials omit recovery escape hatch', () => {
      ...
      assert.ok(!output.hookSpecificOutput.permissionDecisionReason.includes('FORGE_GATEGUARD=off'), 'destructive gate should not advertise disabling GateGuard');
      assert.ok(!output.hookSpecificOutput.permissionDecisionReason.includes('FORGE_DISABLED_HOOKS'), 'destructive gate should not advertise disabling its hook');
  ...
  function runFreshSessionEdit(filePath, extra = {}) {
    return runHook(
      ...
      { CLAUDE_SESSION_ID: '', FORGE_SESSION_ID: '' }
    );
  }
  ...
  if (
    test('emits a condensed single-line denial once the full-block budget is spent', () => {
      ...
      assert.ok(reason.includes('FORGE_GATEGUARD=off'), 'condensed message keeps a recovery hint');
