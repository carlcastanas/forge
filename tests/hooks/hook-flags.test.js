...
function runTests() {
  ...
  if (test('hooks default enabled when no preference source exists', () => {
    withEnv({
      FORGE_HOOKS_ENABLED: undefined,
      CLAUDE_PLUGIN_OPTION_HOOKS_ENABLED: undefined,
      FORGE_HOOK_CONFIG: undefined,
      CLAUDE_PLUGIN_ROOT: undefined,
      FORGE_PLUGIN_ROOT: undefined,
    }, () => {
  ...
  if (test('explicit FORGE environment overrides Claude plugin options', () => {
    withEnv({
      FORGE_HOOKS_ENABLED: 'true',
    ...
    assert.strictEqual(
      ...
      'standard',
      'an explicit empty FORGE profile must not fall through to plugin config'
    );
  })) passed++; else failed++;

  if (test('managed hook config is used after explicit and plugin preferences', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-hook-flags-'));
    const configPath = path.join(root, 'forge', 'setup.json');
    try {
  ...
  if (test('a hook evaluation reads managed config only once', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-hook-flags-read-once-'));
    ...
    try {
      ...
      assert.strictEqual(isHookEnabled('pre:test', {
        env: { FORGE_HOOK_CONFIG: configPath },
        profiles: ['minimal'],
      }), true);
  ...
  if (test('malformed managed config emits one sanitized diagnostic', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-hook-flags-invalid-'));
    const configPath = path.join(root, 'setup.json');
    ...
    try {
      ...
      assert.deepStrictEqual(readManagedHookConfig({ FORGE_HOOK_CONFIG: configPath }), {});
      assert.strictEqual(diagnostics.length, 1);
      assert.match(diagnostics[0], /Warning: unable to read managed FORGE hook config/);
  ...
  if (test('run-with-flags suppresses wrapper hooks when plugin hooks are off', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-hook-wrapper-'));
    ...
    try {
      ...
      delete env.FORGE_HOOKS_ENABLED;
  ...
  if (test('returns minimal when set to minimal', () => {
    withEnv({ FORGE_HOOK_PROFILE: 'minimal' }, () => {
      assert.strictEqual(getHookProfile(), 'minimal');
    });
  })) passed++; else failed++;

  if (test('returns standard when set to standard', () => {
    withEnv({ FORGE_HOOK_PROFILE: 'standard' }, () => {
      assert.strictEqual(getHookProfile(), 'standard');
    });
  })) passed++; else failed++;

  if (test('returns strict when set to strict', () => {
    withEnv({ FORGE_HOOK_PROFILE: 'strict' }, () => {
      assert.strictEqual(getHookProfile(), 'strict');
    });
  })) passed++; else failed++;
  ...
  if (test('defaults to standard for invalid value', () => {
    withEnv({ FORGE_HOOK_PROFILE: 'invalid' }, () => {
      assert.strictEqual(getHookProfile(), 'standard');
    });
  })) passed++; else failed++;

  if (test('defaults to standard for empty string', () => {
    withEnv({ FORGE_HOOK_PROFILE: '' }, () => {
      assert.strictEqual(getHookProfile(), 'standard');
    });
  })) passed++; else failed++;
  ...
  if (test('returns empty Set when env var not set', () => {
    withEnv({ FORGE_DISABLED_HOOKS: undefined }, () => {
  ...
  if (test('parses single hook id', () => {
    withEnv({ FORGE_DISABLED_HOOKS: 'my-hook' }, () => {
      const result = getDisabledHookIds();
  ...
  if (test('parses multiple comma-separated hook ids', () => {
    withEnv({ FORGE_DISABLED_HOOKS: 'hook-a,hook-b,hook-c' }, () => {
      const result = getDisabledHookIds();
  ...
  if (test('trims whitespace around hook ids', () => {
    withEnv({ FORGE_DISABLED_HOOKS: ' hook-a , hook-b ' }, () => {
      const result = getDisabledHookIds();
  ...
  if (test('normalizes hook ids to lowercase', () => {
    withEnv({ FORGE_DISABLED_HOOKS: 'MyHook,ANOTHER' }, () => {
      const result = getDisabledHookIds();
  ...
  if (test('returns true by default for a hook (standard profile)', () => {
    withEnv({ FORGE_HOOK_PROFILE: undefined, FORGE_DISABLED_HOOKS: undefined }, () => {
  ...
  if (test('returns false when hook is in disabled list', () => {
    withEnv({ FORGE_HOOK_PROFILE: undefined, FORGE_DISABLED_HOOKS: 'my-hook' }, () => {
  ...
  if (test('returns true when hook is not in disabled list', () => {
    withEnv({ FORGE_HOOK_PROFILE: undefined, FORGE_DISABLED_HOOKS: 'other-hook' }, () => {
      assert.strictEqual(isHookEnabled('my-hook'), true);
    });
  })) passed++; else failed++;

  if (test('returns false when current profile is not in allowed profiles', () => {
    withEnv({ FORGE_HOOK_PROFILE: 'minimal', FORGE_DISABLED_HOOKS: undefined }, () => {
  ...
  if (test('returns true when current profile is in allowed profiles', () => {
    withEnv({ FORGE_HOOK_PROFILE: 'strict', FORGE_DISABLED_HOOKS: undefined }, () => {
  ...
  if (test('disabled hooks take precedence over profile match', () => {
    withEnv({ FORGE_HOOK_PROFILE: 'strict', FORGE_DISABLED_HOOKS: 'my-hook' }, () => {
      assert.strictEqual(isHookEnabled('my-hook', { profiles: 'strict' }), false);
    });
  })) passed++; else failed++;
  ...
  if (test('allows standard profile by default', () => {
    withEnv({ FORGE_HOOK_PROFILE: 'standard', FORGE_DISABLED_HOOKS: undefined }, () => {
      assert.strictEqual(isHookEnabled('my-hook'), true);
    });
  })) passed++; else failed++;
