/**
 * Tests for scripts/hooks/forge-metrics-bridge.js
 *
 * Run with: node tests/hooks/forge-metrics-bridge.test.js
 */
...
const { run, hashToolCall, extractFilePaths, readSessionCost } = require('../../scripts/hooks/forge-metrics-bridge');
...
function makeTempHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'forge-metrics-bridge-test-'));
}

function runTests() {
  console.log('\n=== Testing forge-metrics-bridge.js ===\n');
  ...
  if (
    test('readSessionCost suppresses repeated malformed breadcrumbs across hook subprocesses', () => {
      ...
      try {
        ...
        const bridgePath = path.resolve(__dirname, '../../scripts/hooks/forge-metrics-bridge');
