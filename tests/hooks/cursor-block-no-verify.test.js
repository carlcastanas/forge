...
function runWrapper(input, env = {}) {
  ...
  const result = spawnSync('node', [wrapper], {
    input: rawInput,
    encoding: 'utf8',
    env: { ...process.env, FORGE_HOOK_PROFILE: 'standard', ...env },
    timeout: 15000,
...
// --- Disable via FORGE_DISABLED_HOOKS ---

if (test('respects FORGE_DISABLED_HOOKS=pre:bash:block-no-verify', () => {
  const r = runWrapper(
    { command: 'git commit --no-verify -m "msg"' },
    { FORGE_DISABLED_HOOKS: 'pre:bash:block-no-verify' }
  );
