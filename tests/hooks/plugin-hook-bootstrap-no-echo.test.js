 ...
 * (the actual entry point used by FORGE plugin hooks, NOT run-with-flags.js)
...
const FIXTURE_DIR = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-pr2380-fixtures-'));
...
function assertNestedChildCleanup(label, childSource, options) {
  ...
  const result = runSupervised(
    ...
    '',
    { FORGE_TEST_CHILD_PID_FILE: pidPath },
    options
  );
...
const persistentChildSource = `
  const fs = require('fs');
  fs.writeFileSync(process.env.FORGE_TEST_CHILD_PID_FILE, String(process.pid));
  setInterval(() => {}, 1000);
`;
...
// --- THE CORE FORGE PATTERN: most FORGE hooks do `process.stdout.write(run(data))`
...
if (
  test('CORE FORGE PATTERN: hook returning raw input as stdout is suppressed', () => {
    ...
    // transcript bloat — 12+ FORGE hook scripts use this exact pattern.
