...
function createTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'forge-observer-archive-'));
}
...
function runAnalyzeOnce(options = {}) {
  ...
  try {
    ...
    fs.writeFileSync(claudeStub, [
      ...
      '  for candidate in "${PROJECT_DIR}"/.observer-tmp/forge-observer-result.*; do',
    ...
    const inheritedEnv = Object.fromEntries(
      Object.entries(process.env).filter(
        ([key]) => key !== 'CLAUDE_PLUGIN_ROOT' && !key.startsWith('FORGE_OBSERVER_')
      )
    );
    const childEnv = {
      ...inheritedEnv,
      ...
      FORGE_OBSERVER_TIMEOUT_SECONDS: String(observerTimeoutSeconds),
      FORGE_OBSERVER_MAX_ANALYSIS_LINES: '500',
      FORGE_OBSERVER_MAX_TURNS: '20',
      FORGE_OBSERVER_MODEL: 'haiku',
      FORGE_OBSERVER_ALLOW_WINDOWS: 'false',
    };
