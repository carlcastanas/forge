...
function run(args = [], options = {}) {
  return spawnSync(process.execPath, [SCRIPT, ...args], {
    ...
    env: {
      ...
      FORGE_PLUGIN_ROOT: options.forgeRoot || '',
      ...(options.env || {}),
    },
...
function runTests() {
  ...
  if (test('normalizes Windows Git Bash POSIX drive roots', () => {
    assert.strictEqual(
      normalizePluginRootForPlatform('/c/Users/x/.claude/plugins/forge', 'win32'),
      'C:/Users/x/.claude/plugins/forge'
    );
    assert.strictEqual(
      normalizePluginRootForPlatform('/z/Work/FORGE/scripts/hooks/check-console-log.js', 'win32'),
      'Z:/Work/FORGE/scripts/hooks/check-console-log.js'
    );
  })) passed++; else failed++;
  ...
  if (test('leaves already-Windows roots unchanged', () => {
    ...
    assert.strictEqual(
      normalizePluginRootForPlatform('D:\\Users\\x\\.claude\\plugins\\forge', 'win32'),
      'D:\\Users\\x\\.claude\\plugins\\forge'
    );
  })) passed++; else failed++;

  if (test('leaves POSIX-looking roots unchanged off Windows', () => {
    assert.strictEqual(
      normalizePluginRootForPlatform('/c/Users/x/.claude/plugins/forge', 'darwin'),
      '/c/Users/x/.claude/plugins/forge'
    );
    assert.strictEqual(
      normalizePluginRootForPlatform('/c/Users/x/.claude/plugins/forge', 'linux'),
      '/c/Users/x/.claude/plugins/forge'
    );
  })) passed++; else failed++;
  ...
  if (test('does not mangle UNC or non-drive absolute paths on Windows', () => {
    assert.strictEqual(
      normalizePluginRootForPlatform('\\\\server\\share\\forge', 'win32'),
      '\\\\server\\share\\forge'
    );
    assert.strictEqual(
      normalizePluginRootForPlatform('/workspace/forge', 'win32'),
      '/workspace/forge'
    );
  })) passed++; else failed++;
...
process.stdout.write(JSON.stringify({
  ...
  forgeRoot: process.env.FORGE_PLUGIN_ROOT,
}));
`);
      ...
      assert.strictEqual(parsed.forgeRoot, root);
    } finally {
      cleanup(root);
    }
  })) passed++; else failed++;
