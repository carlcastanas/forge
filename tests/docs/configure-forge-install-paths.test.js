...
for (const relativePath of configureForgeDocs) {
  test(`${relativePath} delegates to guided plugin setup`, () => {
    ...
    assert.ok(!content.includes('rm -rf /tmp/forge'));
    assert.ok(!content.includes('cp -R "$FORGE_ROOT'));
  });
  ...
  test(`${relativePath} defines the Claude in-harness wizard contract`, () => {
    ...
    assert.ok(content.includes('FORGE_VERSION_PATTERN'));
  ...
  test(`${relativePath} verifies Codex and Kimi before their concrete welcomes`, () => {
    ...
    const kimiVerifyIndex = content.indexOf('forge doctor --target kimi');
    const kimiWelcomeIndex = content.indexOf('forge welcome --action configured');
