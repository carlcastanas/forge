...
console.log('\n=== Testing FORGE 2.0 release surface ===\n');
...
test('business launch copy stays aligned with the rc.1 public surface', () => {
  ...
  assert.ok(source.includes('FORGE v2.0.0-rc.1'), 'business launch copy should use the rc.1 release');
  ...
  assert.ok(source.includes('https://github.com/your-org/forge'), 'business launch copy should include the public repo URL');
  assert.ok(source.includes('https://github.com/your-org/forge/blob/main/docs/releases/2.0.0-rc.1/release-notes.md'), 'business launch copy should link to the rc.1 release notes');
...
test('announcement drafts avoid live-release claims before publication', () => {
  ...
  for (const relativePath of announcementFiles) {
    ...
    assert.ok(!/FORGE v2\.0\.0-rc\.1 is live\./.test(source), `${relativePath} must not claim rc.1 is live before the release gate completes`);
  }
});
...
test('Hermes setup uses release-candidate wording for the rc.1 surface', () => {
  ...
  assert.ok(source.includes('FORGE v2.0.0-rc.1 documents the Hermes surface'));
...
test('Hermes setup cross-links adjacent migration and architecture docs', () => {
  ...
  assert.ok(!source.includes('0.5. Generate and review artifacts with `forge migrate plan` /'));
});

test('release docs preserve the FORGE/Hermes boundary', () => {
  const releaseNotes = read('docs/releases/2.0.0-rc.1/release-notes.md');
  assert.ok(releaseNotes.includes('FORGE is the reusable substrate'));
...
test('preview pack manifest assembles release, Hermes, and publication gates', () => {
  ...
  for (const blocker of [
    ...
    'npm `forge-universal@2.0.0-rc.1`',
    'Claude plugin tag',
    'Codex repo-marketplace distribution evidence',
    'FORGE Tools billing/product readiness'
  ]) {
...
test('owner approval packet consolidates the final gated decisions', () => {
  ...
  for (const urlSurface of ['GitHub prerelease URL', 'npm rc package URL', 'Claude plugin tag URL', 'Primary launch video URL', 'FORGE Tools billing/readiness URL']) {
...
test('GA roadmap mirrors the current May 19 release evidence', () => {
  const roadmap = read('docs/FORGE-2.0-GA-ROADMAP.md');
  ...
  for (const marker of [
    ...
    'forge-may-19-post-pr-2002-sync-64cef8f668e0',
    'owner approval packet'
  ]) {
...
test('rc.1 quickstart gives a clone-to-cross-harness path', () => {
  ...
  assert.ok(quickstart.includes('git clone https://github.com/your-org/forge.git'));
  assert.ok(quickstart.includes('cd FORGE'));
...
test('launch checklist records the forge2 alpha version policy', () => {
  const cargoToml = read('forge2/Cargo.toml');
  const launchChecklist = read('docs/releases/2.0.0-rc.1/launch-checklist.md');
  assert.ok(cargoToml.includes('version = "0.1.0"'));
  assert.ok(launchChecklist.includes('`forge2/Cargo.toml` stays at `0.1.0`'));
  assert.ok(!launchChecklist.includes('confirm whether `forge2/Cargo.toml` moves'));
});
...
test('release video suite manifest gates the content launch lane', () => {
  ...
  for (const marker of [
    ...
    'FORGE_VIDEO_SOURCE_ROOT',
    'FORGE_VIDEO_RELEASE_SUITE_ROOT',
    'video-use compatible workflow',
    'Self-Eval Gate',
    'Do Not Publish If',
    'renders/forge-2-primary-launch-rough-v1.mp4',
...
test('release approval gate blocks publication until owner decisions and URLs are final', () => {
  ...
  for (const marker of [
    'forge.release-approval-gate.v1',
...
test('release video suite public docs do not expose private media paths', () => {
  const releaseVideoDocs = ['docs/releases/2.0.0-rc.1/video-suite-production.md', 'docs/releases/2.0.0/forge-2-hypergrowth-release-command-center.md'];
...
test('publication readiness checklist gates public release actions on evidence', () => {
  ...
  for (const surface of ['GitHub release', 'npm package', 'Claude plugin', 'Codex plugin', 'Codex repo marketplace', 'OpenCode package', 'FORGE Tools billing reference', 'Announcement copy']) {
  ...
  assert.ok(may15Evidence.includes('Forge Shield PR #83'));
  assert.ok(may15Evidence.includes('Forge Shield PR #85'));
  assert.ok(may15Evidence.includes('Forge Shield PR #86'));
  assert.ok(may15Evidence.includes('ci-context.json'));
  assert.ok(may15Evidence.includes('FORGE Tools PR #73'));
  assert.ok(may15Evidence.includes('FORGE-Tools PR #75'));
  ...
  assert.ok(source.includes('FORGE-Tools #92 main CI'));
  assert.ok(source.includes('FORGE-Tools #93 main CI'));
...
test('release name and plugin publication checklist freezes rc.1 surfaces', () => {
  ...
  const launchChecklist = read('docs/releases/2.0.0-rc.1/launch-checklist.md');
  const referenceArchitecture = read('docs/FORGE-2.0-REFERENCE-ARCHITECTURE.md');

  for (const value of [
    'Ship `v2.0.0-rc.1` as **FORGE**',
    '`your-org/FORGE`',
    '`forge-universal`',
    '`forge` on npm is occupied',
    '`@your-org/forge` is unclaimed on npm',
    'Claude plugin',
    'Codex plugin',
...
test('active release identity surfaces use canonical FORGE repo URLs', () => {
  const activeFiles = [
    'README.md',
  ...
  for (const relativePath of activeFiles) {
    const source = read(relativePath);
    if (source.includes('your-org/forge')) {
      offenders.push(relativePath);
    }
  }
