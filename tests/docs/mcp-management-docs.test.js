...
test('token optimization guide separates Claude MCP disables from FORGE config filters', () => {
  ...
  assert.ok(
    source.includes('`FORGE_DISABLED_MCPS` only affects FORGE-generated MCP config output'),
    'Token guide should scope FORGE_DISABLED_MCPS to config generation'
  );
  assert.ok(
