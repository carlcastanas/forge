...
const PACKAGE_NAME = 'forge-universal';
const HASH_PATTERN = /^[a-f0-9]{64}$/i;
const PACKAGE_PATH_PATTERN = /^release-artifacts\/forge-universal-[0-9A-Za-z.+-]+\.tgz$/;

function parseEnvironment(environment = process.env, cwd = process.cwd()) {
  const packageValue = environment.FORGE_RELEASE_PACKAGE;
  const hashValue = environment.FORGE_RELEASE_SHA256;

  if (!packageValue) {
    throw new Error('FORGE_RELEASE_PACKAGE must name the downloaded release .tgz');
  }
  if (!PACKAGE_PATH_PATTERN.test(String(packageValue))) {
    throw new Error('FORGE_RELEASE_PACKAGE must name one FORGE .tgz under release-artifacts');
  }
  if (!HASH_PATTERN.test(hashValue || '')) {
    throw new Error('FORGE_RELEASE_SHA256 must be a 64-character SHA-256 digest');
  }
...
function installPackage(projectDir, packagePath, environment) {
  const projectManifest = {
    name: 'forge-packed-artifact-lifecycle',
    version: '1.0.0',
    private: true,
    dependencies: {
...
function fakeClaudeProviderMain() {
  ...
  const statePath = process.env.FORGE_TEST_CLAUDE_STATE;
  const callsPath = process.env.FORGE_TEST_CLAUDE_CALLS;
  ...
  const createReadArtifacts = () => {
    if (process.env.FORGE_TEST_CLAUDE_CREATE_READ_ARTIFACTS !== '1') return;
  ...
  if (joined === 'plugin marketplace update forge') {
    return;
  }
  if (joined.startsWith('plugin install forge@forge ')) {
    writeState({
      ...state,
      plugins: [{ enabled: true, id: 'forge@forge', scope: 'user', version: '2.2.0' }],
    });
    return;
  }
  if (joined.startsWith('plugin update forge@forge ')) {
    writeState({
      ...state,
      plugins: (state.plugins || []).map(plugin => (
        plugin.id === 'forge@forge' && plugin.scope === 'user'
...
function runTargetSmoke(options) {
  ...
  const statePath = path.join(options.targetRoot, 'forge-install-state.json');
...
function runLifecycle(options) {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-packed-lifecycle-'));
  ...
  try {
    ...
    const statePath = path.join(cursorRoot, 'forge-install-state.json');
    ...
    const setupHelp = runPublicCli(['forge-universal', 'setup', '--help']);
    assert.match(setupHelp.stdout, /FORGE guided setup/);
    assert.match(setupHelp.stdout, /forge setup --mode claude-plugin/);
    ...
    assert.ok(
      fakeClaudeCalls.includes('plugin marketplace update forge'),
      'repeat packed Claude setup must update the official marketplace'
    );
    assert.ok(
      fakeClaudeCalls.some(call => call.startsWith('plugin install forge@forge ')),
      'initial packed Claude setup must install forge@forge'
    );
    assert.ok(
      fakeClaudeCalls.includes('plugin update forge@forge --scope user'),
      'repeat packed Claude setup must update forge@forge'
    );
    const claudeSettings = JSON.parse(
      fs.readFileSync(path.join(claudeConfigDir, 'settings.json'), 'utf8')
    );
    assert.strictEqual(
      claudeSettings.pluginConfigs['forge@forge'].options.hook_profile,
      'strict'
    );
    assert.strictEqual(
      fs.readFileSync(claudeSetupSentinel, 'utf8'),
    ...
    const guidedKimiStatePath = path.join(guidedKimiRoot, 'forge-install-state.json');
    ...
    const guidedKimiInstallArgs = [
      'forge-universal', 'install', '--guided',
      '--harness', 'kimi',
      '--profile', 'core',
    ];
    ...
    const guidedKimiDoctor = parseJsonOutput(
      runPublicCli(['forge', 'doctor', '--target', 'kimi', '--json']),
      'guided Kimi doctor'
    );
    ...
    const guidedKimiUninstall = parseJsonOutput(
      runPublicCli(['forge', 'uninstall', '--target', 'kimi', '--json']),
