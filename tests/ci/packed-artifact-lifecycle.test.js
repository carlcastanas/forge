...
test('resolves package and hash from explicit environment variables', () => {
  const options = lifecycle.parseEnvironment({
    FORGE_RELEASE_PACKAGE: 'release-artifacts/forge-universal-2.2.0.tgz',
    FORGE_RELEASE_SHA256: 'a'.repeat(64),
  }, '/workspace');

  assert.strictEqual(
    options.packagePath,
    path.resolve('/workspace', 'release-artifacts/forge-universal-2.2.0.tgz')
  );
  assert.strictEqual(options.expectedSha256, 'a'.repeat(64));
});

test('rejects missing, malformed, and non-tgz release inputs', () => {
  assert.throws(() => lifecycle.parseEnvironment({}, '/workspace'), /FORGE_RELEASE_PACKAGE/);
  assert.throws(() => lifecycle.parseEnvironment({
    FORGE_RELEASE_PACKAGE: 'package.zip',
    FORGE_RELEASE_SHA256: 'a'.repeat(64),
  }, '/workspace'), /\.tgz/);
  assert.throws(() => lifecycle.parseEnvironment({
    FORGE_RELEASE_PACKAGE: 'release-artifacts/forge-universal-2.2.0.tgz',
    FORGE_RELEASE_SHA256: 'not-a-hash',
  }, '/workspace'), /SHA-256/);
  assert.throws(() => lifecycle.parseEnvironment({
    FORGE_RELEASE_PACKAGE: '../release-artifacts/forge-universal-2.2.0.tgz',
    FORGE_RELEASE_SHA256: 'a'.repeat(64),
  ...
  assert.throws(() => lifecycle.parseEnvironment({
    FORGE_RELEASE_PACKAGE: '/tmp/forge-universal-2.2.0.tgz',
    FORGE_RELEASE_SHA256: 'a'.repeat(64),
  }, '/workspace'), /release-artifacts/);
});

test('hashFile computes a lowercase SHA-256 digest', () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-packed-hash-'));
...
test('public CLI invocations use npm exec instead of internal package paths', () => {
  ...
  assert.deepStrictEqual(invocation.args, [
    ...
    '/s',
    '/c',
    'npm exec --offline --yes=false -- forge-universal setup --help',
  ]);
  ...
  assert.strictEqual(unixInvocation.args[4], 'forge');
