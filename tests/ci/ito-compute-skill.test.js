...
function main() {
  ...
  const tests = [
    ["documents only the real CLI commands and MCP tools", () => {
      ...
      assert.match(skill, /FORGE_ITO_CLI_EXECUTABLE/);
      assert.match(skill, /explicit absolute built entry/);
      assert.match(skill, /never discovers[^\n]*through `PATH`/);
      assert.match(skill, /forge ito login --no-browser/);
      ...
      assert.match(skill, /FORGE (?:itself )?(?:does|performs) no browser automation/i);
    ...
    ["keeps README and integration docs aligned with the separated auth contract", () => {
      for (const relativePath of [
        "README.md",
        "docs/design/forge-ito-compute-integration.md",
      ]) {
        const source = read(relativePath);
        assert.match(source, /forge ito login \[?--no-browser\]?/i, relativePath);
        assert.match(source, /forge ito auth/i, relativePath);
        assert.match(source, /auth.*validat/i, relativePath);
        assert.match(source, /login.*(?:Keychain|device authorization)/is, relativePath);
        assert.doesNotMatch(source, /forge ito auth --no-browser/i, relativePath);
