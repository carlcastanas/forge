...
test('builds pinned Debian and Ubuntu images as a non-root user', () => {
  ...
  assert.match(dockerfile, /ENV HOME=\/tmp\/forge-home/);
  ...
  assert.match(compose, /image:\s*forge-plugin-setup:debian/);
  assert.match(compose, /image:\s*forge-plugin-setup:ubuntu/);
...
test('keeps checkout and source project read-only with hardened defaults', () => {
  ...
  assert.match(compose, /target:\s*\/forge\s*\n\s*read_only:\s*true/);
  assert.match(compose, /target:\s*\/source-project\s*\n\s*read_only:\s*true/);
  assert.match(compose, /CLAUDE_CONFIG_DIR:\s*\/tmp\/forge-claude-config/);
  assert.match(
    compose,
    /\/tmp:rw,nosuid,nodev,exec,size=\$\{FORGE_TMPFS_SIZE:-2g\},uid=1000,gid=1000,mode=0700/
  );
  assert.match(
    compose,
    /\/workspace:rw,nosuid,nodev,noexec,size=\$\{FORGE_WORKSPACE_SIZE:-1g\},uid=1000,gid=1000,mode=0700/
  );
...
test('real runner copies into tmpfs and exposes only explicit safe modes', () => {
  ...
  assert.match(runner, /FORGE_PROJECT_DIR:-\/workspace\/project/);
  ...
  assert.match(runner, /run_forge install/);
  assert.match(runner, /run_forge list-installed --json/);
  assert.match(runner, /run_forge doctor --target claude-project/);
  assert.match(runner, /\[\[ -e "\$project_dir\/\.claude" \]\]/);
  assert.doesNotMatch(
    runner,
    /scripts\/forge\.js" setup|--move-scope|\bmigrate\b/
  );
  assert.doesNotMatch(runner, /scripts\/forge\.js" install/);
...
test('prepares a local npm artifact through the confined public bin contract', () => {
  ...
  const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-packed-cli-'));
  ...
  function createFixture(name, options = {}) {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: options.packageName || 'forge-universal',
        version: '2.1.0',
        bin: options.bin === undefined ? { forge: 'scripts/forge.js' } : options.bin,
      })
    );
    fs.writeFileSync(path.join(packageRoot, 'scripts', 'forge.js'), '#!/usr/bin/env node\n');
    fs.chmodSync(path.join(packageRoot, 'scripts', 'forge.js'), 0o755);
  ...
  try {
    ...
    assert.strictEqual(
      ...
      path.join(validRoot, 'scripts', 'forge.js')
    );

    for (const [name, options, pattern] of [
      ['wrong-name', { packageName: 'not-forge' }, /package name/i],
      ['missing-bin', { bin: {} }, /bin\.forge/i],
      ['escaping-bin', { bin: { forge: '../escape.js' } }, /bin\.forge/i],
...
test('keeps Docker session values as argv entries and validates boundaries', () => {
  ...
  const result = runNode([
    files.interactivePlan,
    '--container', 'forge.plugin-shell_1',
...
test('validates dry-run target confinement and nonempty operations', () => {
  ...
  const safePlan = {
    ...
    plan: {
      ...
      installRoot,
      operations: [
        { destinationPath: path.join(installRoot, 'rules', 'forge', 'base.md') },
      ],
    },
  };
  const safe = runNode(
