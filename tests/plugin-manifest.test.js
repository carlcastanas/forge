...
test('agent.yaml uses canonical FORGE identity', () => {
  const agentYamlSource = fs.readFileSync(agentYamlPath, 'utf8');
  assert.ok(/^name:\s*forge$/m.test(agentYamlSource), 'Expected agent.yaml to use the forge name');
});
...
test('.opencode/plugins/forge-hooks.ts active plugin banner matches package.json', () => {
  const source = fs.readFileSync(opencodeHooksPluginPath, 'utf8');
  const match = source.match(new RegExp(`## Active Plugin: FORGE v(${semverPattern})`));
  assert.ok(match, 'Expected .opencode/plugins/forge-hooks.ts to declare an active plugin banner');
  assert.strictEqual(match[1], expectedVersion);
});
...
test('claude plugin.json uses short plugin slug', () => {
  assert.strictEqual(claudePlugin.name, 'forge');
});
...
test('claude marketplace.json has plugins array with the published plugin entry', () => {
  ...
  assert.strictEqual(claudeMarketplace.name, 'forge');
  assert.strictEqual(claudeMarketplace.plugins[0].name, 'forge');
});
...
test('codex plugin.json uses short plugin slug', () => {
  assert.strictEqual(codexPlugin.name, 'forge');
});
...
test('codex plugin.json explicitly declares the supported lifecycle hook bundle', () => {
  assert.strictEqual(
    ...
    'Codex supports a top-level hooks path; keep the FORGE hook bundle explicit instead of inventing provider-specific settings'
  );
...
test('marketplace.json uses short marketplace slug', () => {
  assert.strictEqual(marketplace.name, 'forge');
});
...
test('marketplace.json plugin entry uses short plugin slug', () => {
  assert.strictEqual(marketplace.plugins[0].name, 'forge');
});
...
test('marketplace local plugin source is a self-contained native Codex bundle', () => {
  ...
  for (const plugin of marketplace.plugins) {
    ...
    assert.strictEqual(sourceRoot, repoRoot, `FORGE's native Codex bundle must use the self-contained repository root, got: ${plugin.source.path}`);
    ...
    assert.ok(fs.existsSync(path.join(sourceRoot, 'skills', 'configure-forge', 'SKILL.md')), 'Codex configure-forge skill must ship inside the installed source root');
  }
});

// ── plugins/forge marketplace plugin folder ─────────────────────────────────────
...
console.log('\n=== plugins/forge Codex marketplace plugin folder ===\n');

const marketplacePluginManifestPath = path.join(repoRoot, 'plugins', 'forge', '.codex-plugin', 'plugin.json');
const marketplacePluginManifest = loadJsonObject(marketplacePluginManifestPath, 'plugins/forge/.codex-plugin/plugin.json');
const rootCodexManifest = loadJsonObject(path.join(repoRoot, '.codex-plugin', 'plugin.json'), '.codex-plugin/plugin.json');

test('plugins/forge manifest name matches the root Codex manifest', () => {
  assert.strictEqual(marketplacePluginManifest.name, rootCodexManifest.name);
});

test('plugins/forge manifest version matches package.json', () => {
  assert.strictEqual(marketplacePluginManifest.version, expectedVersion);
});

test('plugins/forge manifest version matches the root Codex manifest', () => {
  assert.strictEqual(marketplacePluginManifest.version, rootCodexManifest.version);
});

test('plugins/forge manifest reuses root skills and MCP config without vendoring', () => {
  ...
  assert.ok(!fs.existsSync(path.join(pluginDir, 'skills')), 'plugins/forge must not vendor a second skills/ copy (see #2097 review)');
  assert.ok(!fs.existsSync(path.join(pluginDir, '.mcp.json')), 'plugins/forge must not vendor a second .mcp.json (see #2097 review)');
});

test('plugins/forge manifest interface assets resolve to root assets', () => {
  const pluginDir = path.dirname(path.dirname(marketplacePluginManifestPath));
...
test('plugins/forge README marks the thin folder as a legacy compatibility artifact', () => {
  const readmePath = path.join(repoRoot, 'plugins', 'forge', 'README.md');
  assert.ok(fs.existsSync(readmePath), 'Expected plugins/forge/README.md');
  const source = fs.readFileSync(readmePath, 'utf8');
  assert.ok(source.includes('legacy compatibility artifact'));
  assert.ok(source.includes('repository root'));
  assert.ok(source.includes('check-plugin-cache.js'), 'plugins/forge README must point at the cache health check');
