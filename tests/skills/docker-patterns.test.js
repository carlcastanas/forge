...
test('documents the FORGE plugin setup harness and safe operating modes', () => {
...
test('documents the private named-container lifecycle and terminal boundary', () => {
  assert.match(skill, /FORGE_TMPFS_SIZE/);
  ...
  assert.match(skill, /docker rm.*forge-plugin-shell/is);
...
test('requires the offline smoke to execute the locally packed public bin', () => {
  ...
  assert.match(skill, /package\.json.*bin\.forge/is);
