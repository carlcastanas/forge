...
const bashBinary = process.env.FORGE_TEST_BASH || (process.platform === 'win32' ? null : 'bash');
...
function executeInstallation(block, scenario) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-repo-scan-install-'));
  try {
...
if (bashBinary) {
  ...
  console.log('  Integration coverage skipped on Windows without FORGE_TEST_BASH');
}
