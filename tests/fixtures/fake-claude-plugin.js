 ...
 *
 * Environment:
 * - FORGE_TEST_CLAUDE_STATE: JSON state file (required)
 * - FORGE_TEST_CLAUDE_CALLS: JSONL argv log (optional)
 *
 * State supports:
 * {
...
if (!statePath) {
  process.stderr.write('FORGE_TEST_CLAUDE_STATE is required\n');
  process.exit(2);
}
...
function createProviderReadArtifacts() {
  ...
  if (process.env.FORGE_TEST_CLAUDE_OVERWRITE_READ_ARTIFACTS === '1') {
  ...
  if (
    process.env.FORGE_TEST_CLAUDE_WRITE_XDG_DATA === '1'
...
if (args[0] === 'plugin' && args[1] === 'marketplace' && args[2] === 'add') {
  ...
  const marketplaces = [
    ...(state.marketplaces || []).filter(entry => entry.name !== 'forge'),
    {
      name: 'forge',
      source: 'github',
      repo: 'your-org/FORGE',
...
if (args[0] === 'plugin' && args[1] === 'install' && args[2] === 'forge@forge') {
  const scopeIndex = args.indexOf('--scope');
  const scope = scopeIndex >= 0 ? args[scopeIndex + 1] : 'user';
  const plugins = [
    ...(state.plugins || []).filter(plugin => (
      plugin.id !== 'forge@forge' || plugin.scope !== scope
    )),
    { id: 'forge@forge', scope, enabled: true, version: '2.0.0' },
  ];
  writeState({ ...state, plugins });
  process.exit(0);
}

if (args[0] === 'plugin' && args[1] === 'update' && args[2] === 'forge@forge') {
  const scopeIndex = args.indexOf('--scope');
  const scope = scopeIndex >= 0 ? args[scopeIndex + 1] : 'user';
