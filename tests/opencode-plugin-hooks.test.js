...
async function loadPlugin() {
  ...
  const pluginUrl = pathToFileURL(
    path.join(repoRoot, ".opencode", "dist", "plugins", "forge-hooks.js")
  ).href
...
async function withTempProject(files, fn) {
  const projectDir = fs.mkdtempSync(path.join(os.tmpdir(), "forge-opencode-plugin-"))
  try {
...
async function main() {
  ...
  const { ForgeHooksPlugin } = await loadPlugin()
  const tests = [
    [
      ...
      async () => withTempProject([], async (projectDir) => {
        ...
        try {
          ...
          const disabledWarnings = client.logs.filter(
            (entry) =>
              entry.level === "warn" &&
              entry.message.includes("[FORGE] changed-files tracking disabled") &&
              entry.message.includes("forge repair --target opencode")
          )
          assert.strictEqual(
    ...
    [
      ...
      async () => withTempProject(
        ...
        async (projectDir) => {
          ...
          // Verify FORGE_VERSION is not hardcoded
          assert.ok(env.FORGE_VERSION !== "1.8.0", "FORGE_VERSION should not be hardcoded to 1.8.0")
          assert.ok(env.FORGE_VERSION.match(/^\d+\.\d+\.\d+$/), "FORGE_VERSION should be a valid semver version")
        }
      ),
    ],
    [
      ...
      async () => withTempProject(["CLAUDE.md"], async (projectDir) => {
        ...
        assert.deepStrictEqual($.calls, [], `Unexpected shell probes: ${$.calls.join(", ")}`)
        assert.ok(
          client.logs.some((entry) => entry.message === "[FORGE] Found CLAUDE.md - loading project context"),
