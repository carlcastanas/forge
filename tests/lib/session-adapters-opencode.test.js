...
function writeOpencodeFixture({ title = 'rebuild the basket trader rebalancer', updatedAgoMs = 0 } = {}) {
  const storageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-opencode-store-'));
  const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-opencode-repo-'));
...
test('normalizeOpencodeSession produces a valid forge.session.v1 snapshot', () => {
...
test('adapter reads latest session, extracts model from messages, derives objective from title', () => {
  ...
  const recordingDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-opencode-rec-'));
...
test('auto-title "New session - ..." yields empty objective; stale session is recorded', () => {
  ...
  const recordingDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-opencode-rec2-'));
...
test('registry routes structured opencode target and lists the adapter', () => {
  const { storageDir, repoRoot } = writeOpencodeFixture();
  const recordingDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-opencode-reg-'));
...
test('adapter throws for empty store and unknown id; findLatest on empty => null', () => {
  const storageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-oc-empty-'));
...
test('findSessionInfoById + direct file target + isOpencodeSessionFileTarget', () => {
  const storageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-oc-byid-'));
  const now = Date.now();
...
test('parseOpencodeSession: model from later assistant message, missing-time => recorded', () => {
  const storageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-oc-parse-'));
...
test('resolveGitBranch real path returns null outside a repo', () => {
  const storageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-oc-nogit-'));
