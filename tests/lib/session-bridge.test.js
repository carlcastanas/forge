...
function runTests() {
  ...
  if (
    test('returns path containing forge-metrics-', () => {
      const p = getBridgePath('test-session');
      assert.ok(p.includes('forge-metrics-'), `Expected forge-metrics- in path, got: ${p}`);
    })
  )
    passed++;
  else failed++;
  ...
  if (
    test('resolveSessionId uses FORGE_SESSION_ID env var', () => {
      const original = process.env.FORGE_SESSION_ID;
      try {
        process.env.FORGE_SESSION_ID = 'env-session-42';
        ...
        if (original === undefined) {
          ...
          process.env.FORGE_SESSION_ID = original;
        }
      }
    })
  )
