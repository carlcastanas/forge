...
function runTests() {
  ...
  if (test('getAgentDataHome honors FORGE_AGENT_DATA_HOME', () => {
    const original = process.env.FORGE_AGENT_DATA_HOME;
    const overrideRoot = path.join(utils.getTempDir(), `forge-agent-data-${Date.now()}`);
    try {
      process.env.FORGE_AGENT_DATA_HOME = overrideRoot;
      ...
      if (original === undefined) {
        delete process.env.FORGE_AGENT_DATA_HOME;
      } else {
        process.env.FORGE_AGENT_DATA_HOME = original;
      }
    }
  })) passed++; else failed++;

  if (test('getAgentDataHome defaults to ~/.cursor/forge when CURSOR_VERSION is set', () => {
    const originalVersion = process.env.CURSOR_VERSION;
    const originalHome = process.env.FORGE_AGENT_DATA_HOME;
    try {
      delete process.env.FORGE_AGENT_DATA_HOME;
      ...
      const reloaded = require('../../scripts/lib/utils');
      const expected = path.join(reloaded.getHomeDir(), '.cursor', 'forge');
      ...
      if (originalHome === undefined) {
        ...
        process.env.FORGE_AGENT_DATA_HOME = originalHome;
      }
    }
  })) passed++; else failed++;

  if (test('getAgentDataHome expands tilde in FORGE_AGENT_DATA_HOME', () => {
    const original = process.env.FORGE_AGENT_DATA_HOME;
    try {
      process.env.FORGE_AGENT_DATA_HOME = path.join('~', '.cursor', 'forge-test');
      delete require.cache[require.resolve('../../scripts/lib/utils')];
      const reloaded = require('../../scripts/lib/utils');
      const expected = path.join(reloaded.getHomeDir(), '.cursor', 'forge-test');
      assert.strictEqual(reloaded.getAgentDataHome(), expected);
    } finally {
      delete require.cache[require.resolve('../../scripts/lib/utils')];
      if (original === undefined) {
  ...
  if (test('isGitRepo returns true in a git repo', () => {
    // We're running from within the FORGE repo, so this should be true
  ...
  if (test('findFiles recursive scan skips unreadable subdirectory silently', () => {
    ...
    const tmpDir = path.join(utils.getTempDir(), `forge-findfiles-r71-${Date.now()}`);
  ...
  if (test('findFiles skips broken symlinks that match the pattern', () => {
    ...
    const tmpDir = path.join(utils.getTempDir(), `forge-r84-findfiles-toctou-${Date.now()}`);
  ...
  if (test('replaceInFile with empty string replacement deletes matched text', () => {
    const tmpDir = path.join(utils.getTempDir(), `forge-r88-replace-empty-${Date.now()}`);
    fs.mkdirSync(tmpDir, { recursive: true });
  ...
  if (test('countInFile returns 0 for valid file with no pattern matches', () => {
    const tmpDir = path.join(utils.getTempDir(), `forge-r88-count-zero-${Date.now()}`);
    fs.mkdirSync(tmpDir, { recursive: true });
  ...
  if (test('countInFile with /apple|banana/ (alternation, no g) counts all matches', () => {
    const tmpDir = path.join(utils.getTempDir(), `forge-r95-alternation-${Date.now()}`);
    fs.mkdirSync(tmpDir, { recursive: true });
