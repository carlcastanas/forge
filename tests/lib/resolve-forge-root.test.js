/**
 * Tests for scripts/lib/resolve-forge-root.js
 *
 * Covers the FORGE root resolution fallback chain:
...
function setupStandardInstall(homeDir) {
  ...
  fs.mkdirSync(path.join(claudeDir, FORGE_SKILL_SENTINEL), { recursive: true });
  return claudeDir;
}
...
function setupLegacyPluginInstall(homeDir, segments) {
  ...
  fs.mkdirSync(scriptDir, { recursive: true });
  fs.writeFileSync(path.join(scriptDir, 'utils.js'), '// stub');
  fs.mkdirSync(path.join(legacyDir, FORGE_SKILL_SENTINEL), { recursive: true });
  return legacyDir;
}
function setupPluginCache(homeDir, pluginSlug, orgName, version) {
  ...
  fs.mkdirSync(scriptDir, { recursive: true });
  fs.writeFileSync(path.join(scriptDir, 'utils.js'), '// stub');
  fs.mkdirSync(path.join(cacheDir, FORGE_SKILL_SENTINEL), { recursive: true });
  return cacheDir;
}

function runTests() {
  console.log('\n=== Testing resolve-forge-root.js ===\n');
  ...
  if (test('finds current plugin install at ~/.claude/plugins/forge', () => {
    const homeDir = createTempDir();
    try {
  ...
  if (test('finds current plugin install at ~/.claude/plugins/forge@forge', () => {
    const homeDir = createTempDir();
    try {
  ...
  if (test('finds exact legacy plugin install at ~/.claude/plugins/forge', () => {
    const homeDir = createTempDir();
    try {
      const expected = setupLegacyPluginInstall(homeDir, ['forge']);
      const result = resolveForgeRoot({ envRoot: '', homeDir });
  ...
  if (test('finds exact legacy plugin install at ~/.claude/plugins/forge@forge', () => {
    const homeDir = createTempDir();
    try {
      const expected = setupLegacyPluginInstall(homeDir, ['forge@forge']);
      const result = resolveForgeRoot({ envRoot: '', homeDir });
  ...
  if (test('finds marketplace current plugin install at ~/.claude/plugins/marketplaces/forge', () => {
    const homeDir = createTempDir();
    try {
  ...
  if (test('finds marketplace legacy plugin install at ~/.claude/plugins/marketplaces/forge', () => {
    const homeDir = createTempDir();
    try {
      const expected = setupLegacyPluginInstall(homeDir, ['marketplaces', 'forge']);
      const result = resolveForgeRoot({ envRoot: '', homeDir });
  ...
  if (test('handles multiple versions in plugin cache', () => {
    ...
    try {
      ...
      assert.ok(
        result === expected ||
        result === path.join(homeDir, '.claude', 'plugins', 'cache', 'forge', 'legacy-org', '1.7.0'),
  ...
  if (test('rejects a partial ~/.claude (scripts + non-FORGE skills) and prefers a complete root (#2544)', () => {
    const homeDir = createTempDir();
    try {
      // ~/.claude has FORGE's scripts and a user's OWN skills/ dir, but not FORGE's
  ...
  if (test('rejects a partial exact plugin root (scripts, no FORGE skill) and prefers a complete root (#2544)', () => {
    const homeDir = createTempDir();
    try {
      // An exact plugin root under ~/.claude/plugins/forge ships FORGE's scripts but
      // not FORGE's skills. The stricter predicate must reject it on the
      // exact-plugin branch too, not only for ~/.claude.
      const partialScripts = path.join(homeDir, '.claude', 'plugins', 'forge', 'scripts', 'lib');
  ...
  if (test('rejects a partial plugin-cache root (scripts, no FORGE skill) and falls back to ~/.claude (#2544)', () => {
    const homeDir = createTempDir();
    try {
      // A versioned plugin-cache root ships FORGE's scripts but not FORGE's skills.
  ...
  if (test('custom probe skips a qualifying root that lacks the probed script (auto-update)', () => {
    ...
    // carry full resolver evidence (script tree + sentinel FORGE skill) yet
    ...
    try {
      const claudeDir = setupStandardInstall(homeDir);
      const marketplaceRoot = setupLegacyPluginInstall(homeDir, ['marketplaces', 'forge']);
