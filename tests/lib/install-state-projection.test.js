...
function createTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'forge-install-projection-'));
}

function createState(options = {}) {
  const targetRoot = options.targetRoot;
  const installStatePath = options.installStatePath || path.join(targetRoot, 'forge-install-state.json');
    ...
    try {
      ...
      const result = reconcileInstallStateProjections(store, [
        discoveryRecord(state),
        {
          ...
          targetRoot: absentRoot,
          installStatePath: path.join(absentRoot, 'forge-install-state.json'),
          exists: false,
          state: null,
          error: null,
          legacy: false,
