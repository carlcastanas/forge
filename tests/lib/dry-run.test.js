...
function runTests() {
  ...
  if (test('returns false when FORGE_DRY_RUN is unset', () => {
    const env = { ...process.env };
    delete env.FORGE_DRY_RUN;
  ...
  if (test('returns true when FORGE_DRY_RUN=1', () => {
    const env = { ...process.env, FORGE_DRY_RUN: '1' };
    const result = spawnSync(process.execPath, [
      '-e',
      'const { isDryRun } = require("./scripts/lib/hook-flags"); process.exit(isDryRun() ? 1 : 0)',
    ], { cwd: path.resolve(__dirname, '..', '..'), env });
  ...
  if (test('returns false when FORGE_DRY_RUN=0', () => {
    const env = { ...process.env, FORGE_DRY_RUN: '0' };
    const result = spawnSync(process.execPath, [
      '-e',
      'const { isDryRun } = require("./scripts/lib/hook-flags"); process.exit(isDryRun() ? 1 : 0)',
    ], { cwd: path.resolve(__dirname, '..', '..'), env });
  ...
  if (test('skips hook execution and logs preview when FORGE_DRY_RUN=1', () => {
    ...
    const result = spawnSync(process.execPath, [
      ...
      input,
      encoding: 'utf8',
      env: { ...process.env, FORGE_DRY_RUN: '1' },
  ...
  if (test('executes hook normally when FORGE_DRY_RUN is not set', () => {
    const runWithFlags = path.resolve(__dirname, '..', '..', 'scripts', 'hooks', 'run-with-flags.js');
    const hookScript = 'scripts/hooks/doc-file-warning.js';
  ...
  if (test('--dry-run sets FORGE_DRY_RUN env var for child commands', () => {
    ...
    const result = spawnSync(process.execPath, [forgeJs, '--dry-run', '--help'], {
      encoding: 'utf8',
      env: { ...process.env },
    });
    assert.strictEqual(result.status, 0);
  ...
  if (test('--dry-run is stripped from args so command routing works', () => {
    const forgeJs = path.resolve(__dirname, '..', '..', 'scripts', 'forge.js');
    const result = spawnSync(process.execPath, [forgeJs, '--dry-run', 'doctor'], {
      encoding: 'utf8',
      env: { ...process.env },
    });
    assert.ok(
  ...
  if (test('--dry-run works with implicit install routing', () => {
    const forgeJs = path.resolve(__dirname, '..', '..', 'scripts', 'forge.js');
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-dry-run-home-'));
    try {
      const result = spawnSync(process.execPath, [forgeJs, '--dry-run', '--json', 'typescript'], {
        encoding: 'utf8',
        env: {
          ...process.env,
