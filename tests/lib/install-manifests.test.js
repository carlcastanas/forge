...
function runTests() {
  ...
  if (test('marks unified-memory install surfaces as requiring the separate FORGE runtime', () => {
    const component = getInstallComponent('skill:unified-memory');
    assert.deepStrictEqual(component.moduleIds, ['skill-unified-memory']);
    assert.match(component.description, /forge-universal/i);
    ...
    assert.match(singleSkillModule.description, /forge-universal/i);
    assert.match(singleSkillModule.description, /separate|external/i);
    assert.match(workflowModule.description, /forge-universal/i);
