...
const DESCRIPTORS = [
  ...
  },
  {
    ...
    fn: () => {
      const state = { schemaVersion: 'forge.github.coordination.v1', kind: 'epic', status: 'available' };
      const body = [
        '<!-- forge-coordination:start -->',
        '```json',
        JSON.stringify(state, null, 2),
        '```',
        '<!-- forge-coordination:end -->',
      ].join('\n');
      const result = extractCoordinationState(body);
      assert.ok(result !== null);
      assert.strictEqual(result.status, 'available');
      assert.strictEqual(result.kind, 'epic');
      assert.strictEqual(result.schemaVersion, 'forge.github.coordination.v1');
    },
  },
  {
    group: 'extractCoordinationState',
  ...
  // renderCoordinationState
  {
    ...
    fn: () => {
      const state = {
        schemaVersion: 'forge.github.coordination.v1', kind: 'epic', status: 'available',
      ...
      assert.ok(rendered.includes('<!-- forge-coordination:start -->'), 'Missing start marker');
      assert.ok(rendered.includes('<!-- forge-coordination:end -->'), 'Missing end marker');
  ...
  },
  {
    ...
    fn: () => {
      const state = {
        schemaVersion: 'forge.github.coordination.v1', kind: 'epic', status: 'claimed',
