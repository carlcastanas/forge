...
function runTests() {
  ...
  if (test('creates a valid install-state payload', () => {
    const state = createInstallState({
      ...
      installStatePath: '/repo/.cursor/forge-install-state.json',
      request: {
        profile: 'developer',
    ...
    assert.strictEqual(state.schemaVersion, 'forge.install.v1');
  ...
  if (test('validates managed hook metadata for Claude settings operations', () => {
    const baseOptions = {
      ...
      installStatePath: '/home/test/.claude/forge/install-state.json',
      request: {
        profile: 'core',
  ...
  if (test('writes and reads install-state from disk', () => {
    ...
    const statePath = path.join(testDir, 'forge-install-state.json');
  ...
  if (test('rejects invalid install-state payloads on read', () => {
    ...
    try {
      fs.writeFileSync(statePath, JSON.stringify({ schemaVersion: 'forge.install.v1' }, null, 2));
      assert.throws(
