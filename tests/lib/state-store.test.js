/**
 * Tests for the SQLite-backed FORGE state store and CLI commands.
 */
...
const FORGE_SCRIPT = path.join(__dirname, '..', '..', 'scripts', 'forge.js');
...
const mode = process.env.FORGE_FAKE_GH_MODE || 'open';
...
async function seedStore(dbPath) {
  ...
  store.upsertSession({
    ...
    harness: 'claude',
    state: 'active',
    repoRoot: '/tmp/forge-repo',
    startedAt: '2026-03-15T08:00:00.000Z',
    endedAt: null,
    snapshot: {
      schemaVersion: 'forge.session.v1',
      adapterId: 'dmux-tmux',
      session: {
        id: 'session-active',
        kind: 'orchestrated',
      ...
      workers: [
        {
          ...
          worktree: '/tmp/forge-repo/.worktrees/worker-1',
        },
        {
          id: 'worker-2',
          label: 'Worker 2',
          state: 'idle',
          branch: 'feat/state-store',
          worktree: '/tmp/forge-repo/.worktrees/worker-2',
        },
      ],
      aggregates: {
        workerCount: 2,
  ...
  store.insertSkillRun({
    ...
    skillVersion: '1.0.0',
    sessionId: 'session-recorded',
    taskDescription: 'Outline FORGE 2.0 work',
    outcome: 'unknown',
    failureReason: null,
    tokensUsed: 300,
    durationMs: 500,
  ...
  store.insertDecision({
    ...
    rationale: 'Need queryable local state for FORGE control plane',
...
async function runTests() {
  ...
  if (await test('creates the default state.db path and applies migrations idempotently', async () => {
    const homeDir = createTempDir('forge-state-home-');

    try {
      const expectedPath = path.join(homeDir, '.claude', 'forge', 'state.db');
  ...
  if (await test('preserves SQLite special database names like :memory:', async () => {
    const tempDir = createTempDir('forge-state-memory-');
  ...
  if (await test('creates private state-store directories and atomically persists a private database file', async () => {
    const testDir = createTempDir('forge-state-private-');
    const privateParent = path.join(testDir, 'new-parent', 'forge');
  ...
  if (await test('refuses a final state database symlink without changing its target', async () => {
    const testDir = createTempDir('forge-state-final-link-');
  ...
  if (await test('refuses an intermediate state database symlink without writing outside the requested tree', async () => {
    const testDir = createTempDir('forge-state-parent-link-');
    const outsideDir = path.join(testDir, 'outside');
    const linkedParent = path.join(testDir, 'linked-parent');
    const dbPath = path.join(linkedParent, 'forge', 'state.db');
    ...
    try {
      ...
      assert.strictEqual(fs.existsSync(path.join(outsideDir, 'forge', 'state.db')), false);
    } finally {
      cleanupTempDir(testDir);
    }
  })) passed += 1; else failed += 1;
  ...
  if (await test('stores sessions and returns detailed session views with workers, skill runs, and decisions', async () => {
    const testDir = createTempDir('forge-state-db-');
    const dbPath = path.join(testDir, 'state.db');
  ...
  if (await test('builds an empty status snapshot with null rates and missing install health', async () => {
    const testDir = createTempDir('forge-state-empty-');
    const dbPath = path.join(testDir, 'state.db');
  ...
  if (await test('tracks linked work items for Linear, GitHub, and handoff progress', async () => {
    const testDir = createTempDir('forge-state-work-items-');
    const dbPath = path.join(testDir, 'state.db');
    ...
    try {
      ...
      const linearItem = store.upsertWorkItem({
        id: 'linear-forge-20-control-plane',
        source: 'linear',
        sourceId: 'FORGE-20',
        title: 'Define harness-neutral session/worktree contract',
        status: 'in-progress',
        priority: 'high',
        url: 'https://linear.app/forgetools/issue/FORGE-20',
        owner: 'control-plane',
        repoRoot: '/tmp/forge-repo',
        sessionId: 'session-active',
        metadata: {
          project: 'FORGE 2.0: Control Plane',
        },
      ...
      store.upsertWorkItem({
        ...
        source: 'handoff',
        sourceId: 'forge-rc1-release-decision-20260511.md',
      ...
      assert.strictEqual(linearItem.id, 'linear-forge-20-control-plane');
      assert.strictEqual(linearItem.metadata.project, 'FORGE 2.0: Control Plane');
  ...
  if (await test('normalizes default optional fields and reports warning install health', async () => {
    const testDir = createTempDir('forge-state-defaults-');
    const dbPath = path.join(testDir, 'state.db');
  ...
  if (await test('rejects invalid limits and unserializable JSON payloads', async () => {
    const testDir = createTempDir('forge-state-errors-');
    const dbPath = path.join(testDir, 'state.db');
  ...
  if (await test('status CLI supports human-readable and --json output', async () => {
    const testDir = createTempDir('forge-state-cli-');
    const dbPath = path.join(testDir, 'state.db');
  ...
  if (await test('status CLI --exit-code reports attention without suppressing output', async () => {
    const attentionDir = createTempDir('forge-state-attention-');
    const okDir = createTempDir('forge-state-ok-');
  ...
  if (await test('status CLI can emit and write markdown operator snapshots', async () => {
    ...
    try {
      ...
      assert.match(written, /^# FORGE Status/m);
  ...
  if (await test('work-items CLI supports upsert, list, show, and close', async () => {
    const testDir = createTempDir('forge-work-items-cli-');
    const dbPath = path.join(testDir, 'state.db');
    ...
    try {
      const upsertResult = runNode(WORK_ITEMS_SCRIPT, [
        ...
        'high',
        '--url',
        'https://linear.app/example/issue/FORGE-99',
        '--owner',
        'control-plane',
        '--metadata-json',
        '{"project":"FORGE 2.0"}',
      ...
      assert.strictEqual(listPayload.items[0].id, 'linear-forge-99');

      const showResult = runNode(WORK_ITEMS_SCRIPT, ['show', 'linear-forge-99', '--db', dbPath]);
      assert.strictEqual(showResult.status, 0, showResult.stderr);
      assert.match(showResult.stdout, /linear\/#FORGE-99 in-progress: Ship work item CLI/);

      const closeResult = runNode(WORK_ITEMS_SCRIPT, ['close', 'linear-forge-99', '--db', dbPath, '--json']);
  ...
  if (await test('work-items CLI syncs GitHub PRs and issues into readiness', async () => {
    ...
    try {
      const env = {
        FORGE_GH_SHIM: createGhShim(binDir),
      };
  ...
  if (await test('forge CLI delegates the new status, sessions, and work-items subcommands', async () => {
    const testDir = createTempDir('forge-state-cli-');
    const dbPath = path.join(testDir, 'state.db');
    ...
    try {
      ...
      const statusResult = runNode(FORGE_SCRIPT, ['status', '--db', dbPath, '--json']);
      ...
      const sessionsResult = runNode(FORGE_SCRIPT, ['sessions', 'session-active', '--db', dbPath, '--json']);
      ...
      const workItemResult = runNode(FORGE_SCRIPT, [
        'work-items',
        'upsert',
        'handoff-roadmap',
        '--db',
      ...
      const delegatedStatusResult = runNode(FORGE_SCRIPT, ['status', '--db', dbPath, '--json']);
