/**
 * Focused coverage for safely managing FORGE hook entries in Claude settings.
 */
...
function assertAtomicParentReplacementRejected(stage) {
  ...
  try {
    ...
    fs.closeSync = function(descriptor) {
      ...
      // can now replace the parent, exercising FORGE's identity check too.
...
function runTests() {
  ...
  if (test('materializes hook roots and rejects unresolved environment references', () => {
    ...
    const materialized = materializeManagedHooks(source, '/opt/forge');
    ...
    assert.strictEqual(Buffer.from(encodedRoot, 'base64').toString('utf8'), '/opt/forge');
    assert.throws(() => materializeManagedHooks({}, '/opt/forge'), /hooks object/);
    assert.throws(() => materializeManagedHooks(source, ''), /target root/);
    assert.throws(
      () => materializeManagedHooks({
        hooks: {
          Stop: [entry('forge:stop', 'node -e "const e=process.env.CLAUDE_PLUGIN_ROOT"')],
        },
      }, '/opt/forge'),
  ...
  if (test('replaces every plugin-root placeholder recursively and immutably', () => {
    ...
    const resolved = replacePluginRootPlaceholders(source, '/opt/forge');

    assert.deepStrictEqual(source, before);
    assert.deepStrictEqual(resolved, {
      SessionStart: [{
        id: 'session:start',
        command: '/opt/forge/start.js:/opt/forge',
        nested: ['/opt/forge/nested.js', 3, null],
      }],
    });
  })) passed++; else failed++;
  ...
  if (test('atomic settings updates recover a stale invalid lock after its lease', () => {
    ...
    const settingsPath = path.join(tempDir, 'settings.json');
    const lockPath = `${settingsPath}.forge.lock`;
    try {
  ...
  if (test('atomic settings updates serialize nested FORGE writers and release the lock', () => {
    ...
    try {
      updateSettingsAtomic(settingsPath, settings => {
        assert.throws(
          ...
          /Another FORGE process is updating Claude settings/
        );
  ...
  if (test('atomic settings updates refuse a symlinked destination', () => {
    ...
    try {
      ...
      assert.throws(
        ...
        error => error.code === 'ELOOP' || error.code === 'FORGE_SETTINGS_CHANGED'
      );
  ...
  if (test('fresh merge appends managed entries while preserving unrelated settings and hooks', () => {
    ...
    const managed = {
      SessionStart: [entry('forge:start', 'node /opt/forge/start.js')],
      Stop: [entry('forge:stop', 'node /opt/forge/stop.js')],
    };
    ...
    assert.deepStrictEqual(result.added, [
      { event: 'SessionStart', id: 'forge:start' },
      { event: 'Stop', id: 'forge:stop' },
    ]);
  ...
  if (test('fresh merge treats a different entry with the same event and id as a conflict', () => {
    const settings = {
      hooks: {
        Stop: [entry('forge:stop', 'user-modified')],
      },
    };
    const managed = {
      Stop: [entry('forge:stop', 'managed')],
    };

    assert.throws(
      () => mergeManagedHooks(settings, managed),
      /Refusing to overwrite.*Stop.*forge:stop/
    );
    assert.deepStrictEqual(settings.hooks.Stop[0], entry('forge:stop', 'user-modified'));
  })) passed++; else failed++;

  if (test('fresh merge adopts an identical existing event and id without duplicating it', () => {
    const managed = { Stop: [entry('forge:stop', 'managed')] };
    const result = mergeManagedHooks({ hooks: clone(managed) }, managed);

    assert.deepStrictEqual(result.settings.hooks.Stop, managed.Stop);
    assert.deepStrictEqual(result.unchanged, [{ event: 'Stop', id: 'forge:stop' }]);
  })) passed++; else failed++;

  if (test('upgrade replaces an entry only while it still equals previous managed content', () => {
    const previousManagedHooks = { Stop: [entry('forge:stop', 'version-1')] };
    const managedHooks = { Stop: [entry('forge:stop', 'version-2')] };
    const result = mergeManagedHooks(
      { hooks: { Stop: [entry('forge:stop', 'version-1')] } },
      managedHooks,
      { previousManagedHooks }
    );

    assert.deepStrictEqual(result.settings.hooks.Stop, managedHooks.Stop);
    assert.deepStrictEqual(result.updated, [{ event: 'Stop', id: 'forge:stop' }]);
  })) passed++; else failed++;

  if (test('upgrade fails closed when previous managed content has drifted', () => {
    const settings = { hooks: { Stop: [entry('forge:stop', 'customer-edit')] } };
    const before = clone(settings);

    assert.throws(
      () => mergeManagedHooks(
        settings,
        { Stop: [entry('forge:stop', 'version-2')] },
        { previousManagedHooks: { Stop: [entry('forge:stop', 'version-1')] } }
      ),
  ...
  if (test('upgrade is idempotent when the desired entry is already installed', () => {
    const desired = { Stop: [entry('forge:stop', 'version-2')] };
    const result = mergeManagedHooks(
      { hooks: clone(desired) },
      desired,
      { previousManagedHooks: { Stop: [entry('forge:stop', 'version-1')] } }
  ...
  if (test('upgrade removes unchanged entries that are no longer managed', () => {
    const previousManagedHooks = {
      Stop: [
        entry('forge:keep', 'version-1'),
        entry('forge:removed', 'old-command'),
      ],
    };
    const desired = { Stop: [entry('forge:keep', 'version-2')] };
    ...
    assert.deepStrictEqual(result.removed, [{ event: 'Stop', id: 'forge:removed' }]);
  })) passed++; else failed++;

  if (test('upgrade removes multiple retired hooks without deleting their neighbor', () => {
    const previousManagedHooks = {
      Stop: [entry('forge:a', 'a'), entry('forge:b', 'b')],
    };
    const userEntry = entry('user:c', 'keep-user');
    const result = mergeManagedHooks({
      hooks: { Stop: [...previousManagedHooks.Stop, userEntry] },
    }, { SessionStart: [entry('forge:start', 'start')] }, { previousManagedHooks });

    assert.deepStrictEqual(result.settings.hooks.Stop, [userEntry]);
    assert.deepStrictEqual(result.removed, [
      { event: 'Stop', id: 'forge:a' },
      { event: 'Stop', id: 'forge:b' },
    ]);
  })) passed++; else failed++;

  if (test('upgrade refuses to remove a retired entry after user drift', () => {
    const previousManagedHooks = { Stop: [entry('forge:removed', 'old-command')] };
    const settings = { hooks: { Stop: [entry('forge:removed', 'user-edited')] } };

    assert.throws(
      () => mergeManagedHooks(settings, { SessionStart: [entry('forge:start', 'start')] }, {
        previousManagedHooks,
      }),
      /Refusing to remove.*forge:removed.*drifted/
    );
  })) passed++; else failed++;
      ...
      () => mergeManagedHooks(
        {
          hooks: {
            Stop: [
              entry('forge:stop', 'version-1'),
              entry('forge:stop', 'another-copy'),
            ],
          },
        },
        { Stop: [entry('forge:stop', 'version-2')] },
  ...
  if (test('repair mode overwrites a drifted same-event managed id and preserves neighbors', () => {
    ...
    const result = repairManagedHooks(
      { hooks: { Stop: [userEntry, entry('forge:stop', 'drifted')] } },
      { Stop: [entry('forge:stop', 'repaired')] }
    );

    assert.deepStrictEqual(result.settings.hooks.Stop, [
      userEntry,
      entry('forge:stop', 'repaired'),
    ]);
    assert.deepStrictEqual(result.updated, [{ event: 'Stop', id: 'forge:stop' }]);
  })) passed++; else failed++;
  ...
  if (test('inspect reports exact, missing, and drifted managed entries plus the actual subset', () => {
    const expected = {
      SessionStart: [entry('forge:start', 'start')],
      Stop: [
        entry('forge:stop', 'expected'),
        entry('forge:missing', 'missing'),
      ],
    };
    const actualStart = entry('forge:start', 'start');
    const actualDrift = entry('forge:stop', 'changed');
    ...
    assert.deepStrictEqual(result.matched, [{ event: 'SessionStart', id: 'forge:start' }]);
    assert.deepStrictEqual(result.missing, [{ event: 'Stop', id: 'forge:missing' }]);
  ...
  if (test('inspect fails closed on duplicate matching ids in one settings event', () => {
    assert.throws(
      () => inspectManagedHooks(
        { hooks: { Stop: [entry('forge:stop', 'a'), entry('forge:stop', 'b')] } },
        { Stop: [entry('forge:stop', 'expected')] }
      ),
      /multiple.*forge:stop/i
    );
  })) passed++; else failed++;
  ...
  if (test('inspect and uninstall key ownership by event plus id', () => {
    const recorded = { Stop: [entry('forge:stop', 'managed')] };
    const moved = { hooks: { SessionStart: [entry('forge:stop', 'managed')] } };

    const inspection = inspectManagedHooks(moved, recorded);
    assert.strictEqual(inspection.status, 'missing');
    assert.deepStrictEqual(inspection.missing, [{ event: 'Stop', id: 'forge:stop' }]);

    const uninstall = uninstallManagedHooks(moved, recorded);
    assert.deepStrictEqual(uninstall.settings, moved);
    assert.deepStrictEqual(uninstall.missing, [{ event: 'Stop', id: 'forge:stop' }]);
  })) passed++; else failed++;

  if (test('uninstall removes exact recorded entries, retains drift, and cleans empty events', () => {
    const recorded = {
      SessionStart: [entry('forge:start', 'start')],
      Stop: [entry('forge:stop', 'recorded')],
      Notification: [entry('forge:notify', 'notify')],
    };
    const userEntry = { matcher: 'Bash', hooks: [{ type: 'command', command: 'user' }] };
    const driftedStop = entry('forge:stop', 'customer-edit');
    const settings = {
      theme: 'dark',
      hooks: {
    ...
    assert.deepStrictEqual(result.removed, [
      { event: 'SessionStart', id: 'forge:start' },
      { event: 'Notification', id: 'forge:notify' },
    ]);
  ...
  if (test('uninstall removes consecutive managed hooks without deleting a user neighbor', () => {
    const recorded = { Stop: [entry('forge:a', 'a'), entry('forge:b', 'b')] };
    const userEntry = entry('user:c', 'keep-user');
  ...
  if (test('uninstall removes hooks entirely after the final managed event is emptied', () => {
    const recorded = { Stop: [entry('forge:stop', 'recorded')] };
    ...
    assert.deepStrictEqual(result.removed, [{ event: 'Stop', id: 'forge:stop' }]);
    assert.deepStrictEqual(result.retained, []);
  })) passed++; else failed++;
  ...
  if (test('uninstall accepts structurally valid hooks from an older runtime contract', () => {
    ...
    assert.deepStrictEqual(result.settings, {});
    assert.deepStrictEqual(result.removed, [{ event: 'LegacyEvent', id: 'forge:legacy' }]);
  })) passed++; else failed++;

  if (test('all settings transforms reject non-array hook events before changing data', () => {
    const settings = { hooks: { Stop: 'invalid' } };
    const managed = { Stop: [entry('forge:stop', 'expected')] };
