/**
 * Tests for allowlisted FORGE2 control-pane actions.
 */
...
function runTests() {
  ...
  if (test('builds copyable and executable allowlisted FORGE2 actions', () => {
    ...
    assert.strictEqual(sync.cwd, path.join(repoRoot, 'forge2'));
  ...
  if (test('rejects unknown action identifiers', () => {
    assert.throws(
      () => buildControlPaneAction('rm -rf', { repoRoot: '/repo/forge' }),
