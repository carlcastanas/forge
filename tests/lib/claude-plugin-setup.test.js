...
function createFixture(initialState = {}) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'forge plugin setup '));
...
function writeManagedState(fixture, selectedModules, operations = []) {
  const statePath = path.join(fixture.configDir, 'forge', 'install-state.json');
...
test('Windows command-line fallback preserves spaced paths and JSON arguments', () => {
  assert.strictEqual(
    buildWindowsCommandLine(
      ...
      ['plugin', 'install', 'forge@forge', '--config', '{"hooks_enabled":false}']
    ),
    '"C:\\Program Files\\Claude\\claude.cmd" plugin install forge@forge --config "{""hooks_enabled"":false}"'
  );
  assert.throws(
...
test('provider runner times out a hung Claude command with structured context', () => {
  ...
  const spawn = (command, args, options) => {
    ...
    assert.deepStrictEqual(args, ['plugin', 'marketplace', 'update', 'forge']);
...
test('same-scope repeat setup updates FORGE and changes durable user hook preferences', () => {
  withFixture({
    ...
    assert.deepStrictEqual(settings.pluginConfigs['forge@forge'].futureKey, { keep: true });
    assert.strictEqual(settings.pluginConfigs['forge@forge'].options.unknown, 'keep');
    assert.strictEqual(settings.pluginConfigs['forge@forge'].options.hooks_enabled, false);
    assert.strictEqual(settings.pluginConfigs['forge@forge'].options.hook_profile, 'standard');
...
test('setup preserves an explicit attribution opt-in', () => {
  withFixture({
    ...
    // `attribution` wins over `includeCoAuthoredBy` in Claude Code, so FORGE must not
...
test('skills-directory FORGE plugins fail closed before marketplace or plugin mutation', () => {
  withFixture({
    plugins: [{
      id: 'forge@skills-dir',
      scope: 'user',
    ...
    assertThrowsContaining(
      () => setupClaudePlugin(setupOptions(fixture, { scope: 'user' })),
      ['forge@skills-dir', 'duplicate', 'uninstall']
    );
    assert.deepStrictEqual(mutationCalls(readCalls(fixture)), []);
  });
});
...
test('manual plugin layouts fail closed before provider mutation', () => {
  withFixture({}, fixture => {
    ...
    fs.writeFileSync(manualManifest, JSON.stringify({ name: 'forge' }));
    assertThrowsContaining(
      () => setupClaudePlugin(setupOptions(fixture, { scope: 'user' })),
      ['manual', 'forge']
    );
...
test('duplicate FORGE plugin scopes fail closed before mutation', () => {
  withFixture({
...
test('malformed plugin JSON and malformed plugin entries fail closed', () => {
  for (const pluginListResponses of [['{not-json'], [[{ id: 'forge@forge', enabled: true }]]]) {
...
test('malformed marketplace JSON and marketplace name collisions fail closed', () => {
  const cases = [
    ...
    },
    {
      initial: {
        marketplaces: [{
          name: 'forge',
          source: 'git',
          url: 'https://github.com/example/not-forge.git',
          scope: 'user',
        }],
      },
    ...
    {
      initial: { marketplaces: [{ name: 'forge' }] },
...
test('dry-run snapshots local-scope inventory into isolated Claude and project roots', () => {
  withFixture({}, fixture => {
    ...
    fs.writeFileSync(projectSettingsPath, '{"enabledPlugins":{"forge@forge":true}}\n');
    ...
    const runClaude = (args, runOptions) => {
      ...
      assert.strictEqual(
        shadowInstalled.plugins['forge@forge'][0].projectPath,
        runOptions.cwd
      );
      assert.ok(
        shadowInstalled.plugins['forge@forge'][0].installPath
