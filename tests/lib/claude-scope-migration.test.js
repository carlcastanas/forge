...
function createFixture(initialState = {}) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'forge scope migration '));
...
function uninstallArgv(scope) {
  return ['plugin', 'uninstall', 'forge@forge', '--scope', scope, '--keep-data'];
}
...
test('zero installs, ambiguous non-destination scopes, and invalid inventories fail closed', () => {
  const cases = [
    ...
    {
      state: { pluginListResponses: [[{ id: 'forge@forge', enabled: true }]] },
      scope: 'project',
      code: 'INVALID_PLUGIN_INVENTORY',
    },
    {
  ...
  const cases = [
    {
      state: {
        ...
        marketplaces: [marketplace('user')],
        failures: [{
          argv: ['plugin', 'marketplace', 'update', 'forge'],
          status: 7,
...
test('migration preserves hook preferences unless --hooks is explicit', () => {
  ...
  withFixture({
    ...
    assert.strictEqual(settings.pluginConfigs['forge@forge'].futureKey, true);
