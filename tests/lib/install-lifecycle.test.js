...
function createCursorStateOptions(projectRoot, overrides = {}) {
  ...
  const installStatePath = overrides.installStatePath || path.join(targetRoot, 'forge-install-state.json');
...
function writeClaudeState(homeDir, overrides = {}) {
  ...
  const installStatePath = overrides.installStatePath
    || path.join(targetRoot, 'forge', 'install-state.json');
  const options = {
...
function runTests() {
  ...
  if (test('discovers installed states for multiple targets in the current context', () => {
    ...
    try {
      const claudeStatePath = path.join(homeDir, '.claude', 'forge', 'install-state.json');
      const cursorStatePath = path.join(projectRoot, '.cursor', 'forge-install-state.json');
  ...
  if (test('discovers missing and invalid install-state records', () => {
    ...
    try {
      ...
      const statePath = path.join(targetRoot, 'forge-install-state.json');
  ...
  if (test('OpenCode discovery, doctor, and uninstall honor the explicit config root', () => {
    ...
    const installStatePath = path.join(targetRoot, 'forge-install-state.json');
  ...
  if (test('doctor reports target mismatches, missing sources, unverified operations, and version drift', () => {
    ...
    try {
      ...
      const actualStatePath = path.join(actualTargetRoot, 'forge-install-state.json');
  ...
  if (test('doctor reports invalid install-state files as errors', () => {
    ...
    try {
      const statePath = path.join(projectRoot, '.cursor', 'forge-install-state.json');
  ...
  if (test('Claude repair migration derives roots from the adapter and removes only the managed legacy file', () => {
    ...
    try {
      const targetRoot = path.join(homeDir, '.claude');
      const adapterStatePath = path.join(targetRoot, 'forge', 'install-state.json');
  ...
  if (test('repair reports invalid states, missing sources, unsupported operations, and no-op refreshes', () => {
    ...
    try {
      const invalidStatePath = path.join(invalidProjectRoot, '.cursor', 'forge-install-state.json');
  ...
  if (test('repair surfaces OpenCode build failures without blocking other targets', () => {
    ...
    try {
      withTemporarilyMovedPath(path.join(REPO_ROOT, '.opencode', 'dist'), () => {
        ...
        const cursorStatePath = path.join(cursorTargetRoot, 'forge-install-state.json');
    ...
    try {
      ...
      fs.renameSync = function renameSyncWithAncestorSwap(sourcePath, targetPath) {
        if (
          ...
          && path.basename(path.dirname(targetPath)).startsWith('.forge-remove-')
        ) {
  ...
  if (test('repair removes retired managed hooks using the recorded ownership snapshot', () => {
    ...
    try {
      ...
      const retiredHook = managedHookEntry('forge:retired', 'node retired.js');
      const recordedHooks = {
        ...currentHooks,
      ...
      assert.ok(!repaired.hooks.Stop.some(entry => entry.id === 'forge:retired'));
      assert.deepStrictEqual(repaired.hooks, currentHooks);
      const state = readInstallState(path.join(targetRoot, 'forge', 'install-state.json'));
  ...
  if (test('uninstall removes only unchanged managed Claude hooks and reports drift as partial', () => {
    ...
    try {
      ...
      const settingsPath = path.join(targetRoot, 'settings.json');
      const managedHooks = {
        SessionStart: [managedHookEntry('forge:start', 'node managed-start.js')],
        Stop: [managedHookEntry('forge:stop', 'node managed-stop.js')],
      };
      const userHook = {
        id: 'user:stop',
        matcher: 'Bash',
      ...
      const driftedHook = managedHookEntry('forge:stop', 'node user-edited-stop.js');
      fs.mkdirSync(targetRoot, { recursive: true });
      fs.writeFileSync(settingsPath, formatJson({
        theme: 'dark',
        hooks: {
