...
test('animator advances frames and exits when its parent disconnects', () => {
  ...
  runAnimator('Applying FORGE setup...', {
  ...
  assert.deepStrictEqual(writes, [
    `\r${FRAMES[1]} Applying FORGE setup...`,
    `\r${FRAMES[2]} Applying FORGE setup...`,
  ]);
  disconnect();
...
test('spinner renders immediately and clears again after animator termination', () => {
  ...
  const spinner = startTerminalSpinner('Applying FORGE setup...', {
...
test('real animator advances independently and cannot write after cleanup', () => {
  ...
  assert.match(result.stdout, new RegExp(`${FRAMES[0]} Applying FORGE setup`));
  assert.match(result.stdout, new RegExp(`${FRAMES[1]} Applying FORGE setup`));
...
test('real animator exits when its parent process disappears', () => {
  const source = `
    const { startTerminalSpinner } = require(${JSON.stringify(spinnerModule)});
    startTerminalSpinner('Applying FORGE setup...');
    process.exit(0);
  `;
  const result = spawnSync(process.execPath, ['-e', source], {
    encoding: 'utf8',
