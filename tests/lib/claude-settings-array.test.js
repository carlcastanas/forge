...
function config(command) {
  return {
    hooks: {
      Stop: [{ id: 'forge:array', hooks: [{ type: 'command', command }] }],
    },
  };
}

function materialize(command, root = '/opt/forge') {
...
const tests = [
  ['materializes every array command without changing the source', () => {
    ...
    const command = materializeManagedHooks(source, '/opt/forge').Stop[0].hooks[0].command;
    ...
    assert.deepStrictEqual(command.slice(2), ['/opt/forge/scripts/start.js', '--unchanged']);
  }],
  ['array argv commands execute with the exact root in a clean process', () => {
    const root = '/tmp/FORGE space/\'"$`\\路径';
