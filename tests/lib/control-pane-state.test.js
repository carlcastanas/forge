/**
 * Tests for the local FORGE2 control-pane state projection.
 */
...
async function writeSampleForge2Database(dbPath) {
  ...
  insertSession.run([
    'lead-hermes',
    'Coordinate Hermes desktop and FORGE release work',
    'FORGE',
    '2.0-control-pane',
    'claude',
    'claude',
    ...
    'running',
    4242,
    '/tmp/forge-worktrees/hermes',
    'forge/hermes-control-pane',
    'main',
    1200,
    800,
    2000,
  ...
  insertEntity.run([
    ...
    '/notes/hermes.md',
    'How the FORGE maintainers routes Hermes Desktop, Zellij panes, Devin-style delegation, and FORGE release control work.',
  ...
  db.run('INSERT INTO context_graph_observations (session_id, entity_id, observation_type, priority, pinned, summary, details_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [
    ...
    3,
    1,
    'Hermes Desktop and FORGE should share recall before dispatching work.',
...
async function writeSampleWorkItemsDatabase(dbPath) {
  ...
  insertWorkItem.run([
    ...
    'github-pr',
    '2131',
    'Merge FORGE control pane',
    'done',
    'normal',
    'https://github.com/carlcastanas/forge/pull/2131',
    'operator',
    '/repo/forge',
    null,
    JSON.stringify({
      branch: 'product/forge2-knowledge-control-pane',
  ...
  insertWorkItem.run([
    ...
    'open',
    'high',
    'https://github.com/carlcastanas/forge/issues/2290',
    null,
    '/repo/forge',
    null,
    JSON.stringify({}),
...
async function runTests() {
  ...
  if (
    await test('builds an operator snapshot from FORGE2 SQLite and configured connectors', async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-control-pane-state-'));
      ...
      try {
        ...
        assert.strictEqual(snapshot.schemaVersion, 'forge.control-pane.snapshot.v1');
  ...
  if (
    await test('projects state-store work items into agent Kanban summary', async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-control-pane-work-items-'));
      const dbPath = path.join(tempDir, 'forge2.db');
  ...
  if (
    await test('treats an unreadable optional state-store database as empty work items', async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-control-pane-corrupt-work-items-'));
      const dbPath = path.join(tempDir, 'forge2.db');
  ...
  if (
    await test('resolves config from explicit db path and TOML connector file', async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-control-pane-config-'));
      const dbPath = path.join(tempDir, 'state.db');
      const configPath = path.join(tempDir, 'forge2.toml');
  ...
  if (
    await test('prefers the operator home config over stale app-support config', async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-control-pane-precedence-'));
      const homeDir = path.join(tempDir, 'home');
      const homeConfigDir = path.join(homeDir, '.claude');
      const appConfigDir = path.join(homeDir, 'Library', 'Application Support', 'forge2');
      ...
      try {
        ...
        fs.writeFileSync(path.join(homeConfigDir, 'forge2.toml'), `db_path = "${homeDbPath.replace(/\\/g, '\\\\')}"\n`, 'utf8');
  ...
  if (
    await test('shows configured connectors even when the SQLite database is missing', async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-control-pane-missing-db-'));
  ...
  if (
    await test('handles an existing SQLite database before FORGE2 tables are created', async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-control-pane-empty-db-'));
  ...
  if (
    await test('handles malformed JSON rows and all session state counters', async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-control-pane-edge-db-'));
      const dbPath = path.join(tempDir, 'forge2.db');
