...
test('sessions command uses shared inline resolver in all node scripts', () => {
  ...
  assert.strictEqual((sessionsDoc.match(/scripts','lib','resolve-forge-root/g) || []).length, 6);
});
...
test('skill-health command uses shared inline resolver in all shell snippets', () => {
  ...
  assert.strictEqual((skillHealthDoc.match(/scripts','lib','resolve-forge-root/g) || []).length, 3);
});
...
test('instinct-status command uses shared inline resolver (no stale legacy fallback) (#2037)', () => {
  ...
  assert.strictEqual((instinctStatusDoc.match(/scripts','lib','resolve-forge-root/g) || []).length, 1);
...
test('auto-update command probes for the script it runs, not just scripts/lib', () => {
  ...
  // sentinel FORGE skill, per #2544/#2577) yet still lack scripts/auto-update.js.
  ...
  for (const docPath of autoUpdateDocs) {
    ...
    assert.strictEqual(
      (doc.match(/scripts','lib','resolve-forge-root/g) || []).length, 1,
...
test('resolveEccRoot module covers current and legacy marketplace plugin roots', () => {
  ...
  try {
    const legacyRoot = path.join(legacyHomeDir, '.claude', 'plugins', 'marketplaces', 'forge');
  ...
  assert.ok(INLINE_RESOLVE.includes("scripts','lib','resolve-forge-root"));
});
