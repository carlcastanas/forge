...
Common issues and solutions for FORGE plugin.
...
**Solutions:**
```bash
# First inspect what FORGE still knows about this machine
...
# Claude Code → Extensions → FORGE → Uninstall
# Then reinstall from marketplace

# If the issue is marketplace/account access, use FORGE Tools billing/account recovery separately; do not use reinstall as a proxy for account recovery
...
`[FORGE] changed-files tracking disabled` warning appears in the OpenCode
...
- The `~/.opencode` install is missing or incomplete for this machine —
  ...
  and the `forge-hooks` plugin depend on `plugins/lib/changed-files-store.js`;
  since `forge-hooks.ts` is OpenCode's plugin entry point (loaded once at
...
**Solutions:**
```bash
# From the FORGE repo, check for and repair missing/incomplete managed files
forge doctor --target opencode
forge repair --target opencode

# If that reports no drift but plugins/ is still missing on the device,
# re-run the FORGE installer for the opencode target
```
...
plugin, not FORGE — FORGE never writes to `~/.config/opencode/`. Fix the model prefix
...
cd ~/.claude/plugins/cache/forge
npm install

# Or for manual install
cd ~/.claude/plugins/forge
npm install
```
