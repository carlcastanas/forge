...
pub async fn run(db: StateStore, cfg: Config) -> Result<()> {
    tracing::info!("FORGE daemon started");
...
mod tests {
    ...
    fn temp_db_path() -> PathBuf {
        std::env::temp_dir().join(format!("forge2-daemon-test-{}.db", uuid::Uuid::new_v4()))
    }
            ...
            Ok(manager::WorktreeBulkMergeOutcome {
                merged: vec![
                    manager::WorktreeMergeOutcome {
                        session_id: "worker-a".to_string(),
                        branch: "forge/worker-a".to_string(),
                    ...
                    manager::WorktreeMergeOutcome {
                        session_id: "worker-b".to_string(),
                        branch: "forge/worker-b".to_string(),
                        base_branch: "main".to_string(),
                ...
                rebased: vec![manager::WorktreeRebaseOutcome {
                    ...
                    branch: "forge/worker-r".to_string(),
                    base_branch: "main".to_string(),
                    already_up_to_date: false,
                }],
