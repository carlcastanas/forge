...
These lifecycle hook definitions document FORGE's memory persistence contract for Claude Code plugin and manual installs.
...
- `session-activity-tracker.js` records tool usage and file activity for FORGE2 status and observability.
...
| `PostToolUse` | `post:session-activity-tracker` | Record tool and file activity for FORGE2 metrics | no |
...
- Bound context loaded at session start with `FORGE_SESSION_START_MAX_CHARS`.
- Allow opt-out with `FORGE_SESSION_START_CONTEXT=off`.
- Keep lifecycle hooks profile-gated through `FORGE_HOOK_PROFILE` and `FORGE_DISABLED_HOOKS`.
