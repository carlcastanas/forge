export const meta = {
  name: 'orch-review',
  description:
    'FORGE Review phase as a native Claude Code workflow: multi-dimension review (quality + language + conditional security) then adversarial verification of every CRITICAL/HIGH finding. Returns blocking + advisory findings for Gate 2.',
  phases: [
...
// Language → FORGE reviewer agent. Mirrors the agents present in agents/.
const LANGUAGE_REVIEWER = {
  typescript: 'forge:typescript-reviewer',
  javascript: 'forge:typescript-reviewer',
  python: 'forge:python-reviewer',
  go: 'forge:go-reviewer',
  rust: 'forge:rust-reviewer',
  java: 'forge:java-reviewer',
  kotlin: 'forge:kotlin-reviewer',
  swift: 'forge:swift-reviewer',
  php: 'forge:php-reviewer',
  csharp: 'forge:csharp-reviewer',
  fsharp: 'forge:fsharp-reviewer',
  react: 'forge:react-reviewer',
  vue: 'forge:vue-reviewer',
  flutter: 'forge:flutter-reviewer',
  dart: 'forge:flutter-reviewer',
  django: 'forge:django-reviewer',
  fastapi: 'forge:fastapi-reviewer',
  cpp: 'forge:cpp-reviewer'
};
...
const dimensions = [
  { key: 'quality', label: 'correctness & quality', agentType: 'forge:code-reviewer' },
  ...(langReviewer ? [{ key: `lang:${input.language}`, label: `${input.language} idioms & pitfalls`, agentType: langReviewer }] : []),
  ...(securityNeeded ? [{ key: 'security', label: 'security (OWASP, secrets, injection)', agentType: 'forge:security-reviewer' }] : [])
];
if (securityNeeded) {
