---
description: Detect a project's stack and produce a dry-run FORGE onboarding plan using the repository's install manifests and stack mappings.
---

# /project-init

Create a safe, reviewable FORGE onboarding plan for the current project. This command should start in dry-run mode and only write files after explicit user approval.
...
/project-init --config forge-install.json
```
...
3. Use FORGE's installer and manifest tooling. Do not hand-copy files or clone arbitrary remotes as an install shortcut.
...
- FORGE config: `forge-install.json`
- optional stack map: `config/project-stack-mappings.json` in the FORGE repo

When the FORGE checkout is available, use `config/project-stack-mappings.json` as the stack-to-rules/skills reference. If the file is unavailable, fall back to the installed FORGE manifests and explicit user choices.
...
3. Resolve the smallest useful FORGE plan:
   - project has an `forge-install.json`: `node scripts/install-plan.js --config forge-install.json --json`
...
- `/forge-guide` for interactive feature discovery before installing
