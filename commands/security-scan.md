---
description: Run Forge Shield against agent, hook, MCP, permission, and secret surfaces.
agent: forge:security-reviewer
subtask: true
---

# Security Scan Command

Run Forge Shield against the current project or a target path, then turn the findings into a prioritized remediation plan.
...
- `--fix`: applies only Forge Shield fixes explicitly marked as safe and auto-fixable.
...
```bash
npx forge-shield scan --path "${TARGET_PATH:-.}" --format text
```

For local Forge Shield development, run from the Forge Shield checkout:

```bash
npm run scan -- --path "${TARGET_PATH:-.}" --format text
```

Do not invent findings. Use Forge Shield output as the source of truth and separate scanner facts from follow-up judgment.
