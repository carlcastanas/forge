...
- `forge loop-status --json` emits machine-readable status for recent local
  Claude transcripts.
- `forge loop-status --home <dir>` scans a different home directory when
  inspecting another local profile or mounted workspace.
- `forge loop-status --transcript <session.jsonl>` inspects one transcript
  directly.
- `forge loop-status --bash-timeout-seconds 1800` adjusts the stale Bash
  threshold.
- `forge loop-status --exit-code` exits `2` when stale loop or tool signals are
...
- `forge loop-status --watch` refreshes status until interrupted.
- `forge loop-status --watch --watch-count 3 --exit-code` refreshes a bounded
  number of times, then exits with the highest status seen.
- `forge loop-status --watch --watch-count 3` emits a bounded watch stream for
  scripts and handoffs.
- `forge loop-status --watch --write-dir ~/.claude/loops` maintains
