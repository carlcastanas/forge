# .codex-plugin — Codex Native Plugin for FORGE

This directory contains the **Codex plugin manifest** for FORGE.
...
Codex 0.146.0 and newer use `plugin add`, not `plugin install`. Add FORGE's
...
registration. To fetch a newer marketplace snapshot before applying a new FORGE
release, run:

```bash
codex plugin marketplace upgrade forge
codex plugin add forge@forge
```
...
FORGE's marketplace entry points at the repository root. Codex copies the selected
...
then review and trust the FORGE hook definition before enabling it.
...
Once the cached skills are available, invoke `$configure-forge` inside Codex for
FORGE's guided configuration. Installing the plugin again is idempotent and does
...
(`bash scripts/sync-forge-to-codex.sh`) is a separate compatibility
...
layer explicitly with `forge uninstall --legacy-codex-sync --dry-run`, followed
by `forge uninstall --legacy-codex-sync`. Cleanup never targets conversation
...
After install, `codex plugin list` is only a registration check. From an FORGE
...
The installed plugin registers under the short slug `forge` so tool and command names
...
- FORGE is moving to a skills-first workflow surface. Legacy `commands/` remain for
