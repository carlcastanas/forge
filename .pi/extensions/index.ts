/**
 * FORGE adapter for the Pi coding agent.
 *
 * This is the ONLY adapter logic FORGE ships for Pi. FORGE's canonical assets stay
 ...
 *   - Pi lifecycle events -> FORGE's existing hook runner (`run-with-flags.js`),
 *     so FORGE hook profiles and disable flags keep working under Pi.
 *   - FORGE's SessionStart `additionalContext` payload -> Pi's system prompt.
 *   - A `/forge-doctor` command for install diagnostics.
 *
...
/**
 * FORGE package root. This file lives at `<root>/.pi/extensions/index.ts`, so the
...
/** FORGE's universal hook runner. It applies hook-profile and disable flags. */
const HOOK_RUNNER = path.join(FORGE_ROOT, "scripts", "hooks", "run-with-flags.js")

const HOOK_TIMEOUT_MS = 30_000
const MAX_HOOK_OUTPUT_BYTES = 1024 * 1024

/**
 * FORGE rules injected into Pi's system prompt, read from the canonical
...
/** Values FORGE treats as "off" across its existing environment switches. */
const DISABLED_VALUES = new Set(["0", "false", "off", "none", "disabled"])

/**
 * Optional Pi companion packages. FORGE works without every one of these; they
 * are reported by `/forge-doctor` so users can see which extras are available.
 */
...
interface HookSpec {
  /** FORGE hook id, used for profile gating and disable flags. */
  id: string
  /** Hook script path relative to the FORGE package root. */
  script: string
...
/**
 * Run an FORGE hook through FORGE's own runner.
 *
 ...
 * FORGE package root if Pi reports a directory that no longer exists, so a stale
...
/**
 * Build the environment FORGE hooks expect.
 *
 * `CLAUDE_PLUGIN_ROOT` / `FORGE_PLUGIN_ROOT` are how every FORGE hook locates the
 * package; setting them from `FORGE_ROOT` is what makes a global install resolve
 * correctly instead of probing the user's project. The `CLAUDE_*` session vars
 * are the names FORGE's shared hook scripts already read across harnesses.
 */
function buildHookEnv(ctx: ExtensionContext): NodeJS.ProcessEnv {
  const env: NodeJS.ProcessEnv = {
    ...process.env,
    CLAUDE_PLUGIN_ROOT: FORGE_ROOT,
    FORGE_PLUGIN_ROOT: FORGE_ROOT,
...
/**
 * Map Pi's session reason onto the `source` values FORGE's SessionStart hook
 ...
 *
 * FORGE hooks emit a JSON envelope, but the runner passes stdin straight through
 ...
 * allowlist length would overstate a partial install in `/forge-doctor`, which is
 * the one place a user looks to find exactly that.
 */
let cachedRuleFileCount = 0

/**
 * FORGE's portable engineering rules, concatenated from the canonical
 * `rules/common/` directory of the installed package.
 *
 * Returns null when disabled via `FORGE_PI_RULES` or when no rule file could be
...
function loadPortableRules(): string | null {
  ...
  if (isDisabledByEnv(process.env.FORGE_PI_RULES)) {
    cachedRules = null
  ...
  for (const file of PORTABLE_RULE_FILES) {
    let text: string
    try {
      text = fs.readFileSync(path.join(FORGE_ROOT, "rules", "common", file), "utf8").trim()
    } catch {
      continue
    }
...
function describeRulesStatus(): string {
  if (isDisabledByEnv(process.env.FORGE_PI_RULES)) {
    return "disabled via FORGE_PI_RULES"
  }

  const rules = loadPortableRules()
  if (!rules) {
    return `NOT FOUND (${path.join(FORGE_ROOT, "rules", "common")})`
  }
...
function buildDoctorReport(ctx: ExtensionContext): string {
  const skillsDir = path.join(FORGE_ROOT, "skills")
  const commandsDir = path.join(FORGE_ROOT, "commands")
  ...
  const lines = [
    ...
    `  profile:       ${process.env.FORGE_HOOK_PROFILE || "standard (default)"}`,
    `  disabled:      ${process.env.FORGE_DISABLED_HOOKS || "none"}`,
    "",
  ...
  lines.push(
    "",
    "Companion packages are optional; FORGE skills, commands, and session hooks",
...
export default function (pi: ExtensionAPI): void {
  /**
   * FORGE's SessionStart hook returns context for the model, but Pi has no
  ...
  pi.on("session_start", async (event, ctx) => {
    ...
    if (result.failure) {
      ctx.ui.notify(`FORGE session-start hook skipped (${result.failure})`, "warning")
      return
    }
  ...
  pi.on("before_agent_start", event => {
    ...
    const rules = loadPortableRules()
    if (rules) {
      additions.push(`<forge-engineering-rules>\n${rules}\n</forge-engineering-rules>`)
    }

    if (pendingContext) {
      additions.push(`<forge-session-context>\n${pendingContext}\n</forge-session-context>`)
      pendingContext = undefined
    }
  ...
  pi.on("session_shutdown", async (event, ctx) => {
    ...
    if (result.failure) {
      ctx.ui.notify(`FORGE session-end hook skipped (${result.failure})`, "warning")
    }
  })

  pi.registerCommand("forge-doctor", {
    description: "Report FORGE adapter status: package root, canonical resources, hooks, companions",
    handler: async (_args, ctx) => {
      pi.sendMessage(
        {
          customType: "forge-doctor",
