...
This directory contains the **Pi adapter** for FORGE — a thin extension that connects the
[@earendil-works/pi-coding-agent](https://github.com/earendil-works/pi-coding-agent)
terminal coding agent to FORGE's canonical skills, prompts, and lifecycle hooks.

## Design Principle

FORGE's canonical assets—skills, agents, commands, and hooks—**remain the single source of truth**.
This adapter contains **only the integration logic**. No copies, no duplication.

## What This Provides

- **FORGE's skills** from `./skills/` — available in Pi as `/skill:<name>`
- **FORGE's commands** from `./commands/` — available in Pi as `/<name>`
- **FORGE's engineering rules** from `./rules/common/` — injected into Pi's system
...
- **Session lifecycle hooks** — FORGE's SessionStart and SessionEnd hooks, run through FORGE's own
  `run-with-flags.js`, so `FORGE_HOOK_PROFILE` and `FORGE_DISABLED_HOOKS` keep working under Pi
- **Session context injection** — whatever FORGE's SessionStart hook returns as
  `additionalContext` is folded into Pi's system prompt for the next turn
- **`/forge-doctor`** — diagnostic command to verify the integration
...
pi install -l /path/to/FORGE

# Verify
pi list
```

Then inside Pi, run `/forge-doctor` to confirm skills, commands, and hooks are available.
...
If you already have FORGE installed for Claude Code, point Pi at the same canonical directories
...
or `/forge-doctor` — use Option 1 for the full integration.
...
1. **Skill and command mounting** — Pi reads `./skills` and `./commands` directly via the
   `pi` key in `package.json`. No transformation is needed: FORGE's `SKILL.md` files already
   follow the Agent Skills standard Pi implements, and FORGE's command frontmatter
   (`description`, `argument-hint`) is already Pi's prompt-template format
2. **Lifecycle hooks** — Maps Pi's `session_start` to FORGE's `session:start` hook
   (`scripts/hooks/session-start.js`) and Pi's `session_shutdown` to FORGE's `session:end:marker`
   hook (`scripts/hooks/session-end-marker.js`), both invoked through
   `scripts/hooks/run-with-flags.js` so FORGE's profile and disable flags are honored
3. **Rule injection** — Reads FORGE's portable engineering rules from the canonical
   `rules/common/` directory at runtime and appends them to the system prompt inside an
   `<forge-engineering-rules>` block on every turn. Nothing is copied into `.pi/`.
   ...
   first adapter. Set `FORGE_PI_RULES` to `0`, `false`, `off`, `none`, or `disabled` to turn
   injection off; `/forge-doctor` reports the current state and the injected size
4. **Context injection** — Parses `hookSpecificOutput.additionalContext` from the SessionStart
   ...
   `<forge-session-context>` block. Non-JSON hook output is tolerated, not treated as an error
...
recursively launching the OMP binary as a hook runner. Set `FORGE_HOOK_NODE` to
...
FORGE works in Pi without any of these. Skills and commands are fully available today.

These capabilities are provided by existing community Pi packages rather than by
anything FORGE would need to write. This adapter deliberately does not bundle or
auto-install them: bundling would ship third-party code that executes with full
user permissions in every FORGE install, and would make optional capabilities
mandatory. Install whichever you want yourself — `/forge-doctor` reports which are
...
`~/.config/mcp/mcp.json` — which is exactly the format FORGE already uses in
`.mcp.json` and `mcp-configs/mcp-servers.json`.

Verified against `pi-mcp-adapter` 2.21.2: copying FORGE's `mcp-configs/mcp-servers.json`
to a project's `.mcp.json` registers Pi's `mcp` tool and `/mcp` command with all
35 FORGE servers discovered, alongside this adapter's own `/forge-doctor`. No
translation layer is needed and no FORGE change is required.
...
FORGE neither installs nor depends on that package. Two caveats: the adapter's
...
**Fix:** run `pi config` and confirm the FORGE package's skills and prompts are enabled
(<kbd>Tab</kbd> switches between user and project scope). Then confirm the package itself is
registered with `pi list`.

### `/forge-doctor` not found or reports missing package root

**Cause:** Extension not loaded or package installed incorrectly.

**Fix:**
1. Run `pi list` to confirm FORGE is registered
2. Restart Pi: exit and reopen the session
3. Run `/forge-doctor` again

`/forge-doctor` prints the resolved package root, the skill and command counts it found, the
...
**Cause:** the extension is not loaded, or the hooks are gated off by an FORGE hook profile.

**Fix:**
1. Confirm `pi list` shows FORGE and that `/forge-doctor` reports the hook runner as found
2. Check `FORGE_HOOK_PROFILE` and `FORGE_DISABLED_HOOKS` — `/forge-doctor` prints both. A hook
   listed in `FORGE_DISABLED_HOOKS` is skipped by design
...
- MCP is not bundled, but FORGE's MCP configs load in Pi through the community `pi-mcp-adapter` — see [MCP](#mcp) above
