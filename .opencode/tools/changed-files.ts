    ...
    changedFilesStorePromise = import("../plugins/lib/changed-files-store.js").catch(() => {
      ...
      throw new Error(
        "changed-files tool: could not load the changed-files store. " +
          ...
          "(an interrupted or partial FORGE install can leave tools/ populated without plugins/). " +
          "Run `node scripts/repair.js --target opencode` (or `forge repair --target opencode`) " +
          "from the FORGE repo to restore the missing files."
      )
    })
  }
