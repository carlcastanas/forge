# OpenCode FORGE Plugin

> WARNING: This README is specific to OpenCode usage.
> If you installed FORGE via npm (e.g. `npm install opencode-forge`), refer to the root README instead.

FORGE plugin for OpenCode - agents, commands, hooks, and skills.

## Installation

## Installation Overview

There are two ways to use FORGE:

1. **npm package (recommended for most users)**
   Install via npm/bun/yarn and use the `forge-install` CLI to set up rules and agents.
...
This loads the FORGE OpenCode plugin module from npm:
- hook/event integrations
- bundled custom tools exported by the plugin

It does **not** auto-register the full FORGE command/agent/instruction catalog in your project config. For the full OpenCode setup, either:
...
After installation, the `forge-install` CLI is also available:
...
If you also want to apply the FORGE home install
...
- `FORGE_HOOK_PROFILE`: `minimal`, `standard` (default), `strict`
- `FORGE_DISABLED_HOOKS`: comma-separated hook IDs to disable

## Skills

The default OpenCode config loads 11 curated FORGE skills via the `instructions` array:
...
provider and select a model in OpenCode; FORGE's primary agent uses that global
