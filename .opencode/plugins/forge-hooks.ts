/**
 * FORGE Plugin Hooks for OpenCode
 *
...
type ForgeHooksPluginFn = (input: PluginInput) => Promise<Record<string, unknown>>

export const ForgeHooksPlugin: ForgeHooksPluginFn = async ({
  client,
  $,
  directory,
  worktree,
  ...
  const log = (level: "debug" | "info" | "warn" | "error", message: string) =>
    client.app.log({ body: { service: "forge", level, message } })
  ...
  // interrupted or partial FORGE install on Termux/Android) only disables
  ...
  try {
    ...
    Promise.resolve()
      .then(() =>
        log(
          "warn",
          "[FORGE] changed-files tracking disabled: could not load the changed-files store. " +
            "Run `forge repair --target opencode` to restore the missing files. Other FORGE hooks are unaffected."
        )
      )
      .catch(() => {})
  }
  ...
  const currentProfile = normalizeProfile(process.env.FORGE_HOOK_PROFILE)
  const disabledHooks = new Set(
    (process.env.FORGE_DISABLED_HOOKS || "")
      .split(",")
      .map((item) => item.trim())
      .filter(Boolean)
  )
  ...
  return {
    ...
    "file.edited": async (event: { path: string }) => {
      ...
      if (hookEnabled("post:edit:format", ["strict"]) && event.path.match(/\.(ts|tsx|js|jsx)$/)) {
        try {
          ...
          log("info", `[FORGE] Formatted: ${event.path}`)
        } catch (error: unknown) {
          // Prettier not installed or failed - log but continue
          const errorMessage = error instanceof Error ? error.message : String(error)
          log("debug", `[FORGE] Prettier formatting failed for ${event.path}: ${errorMessage}`)
        }
      }
      ...
      if (hookEnabled("post:edit:console-warn", ["standard", "strict"]) && event.path.match(/\.(ts|tsx|js|jsx)$/)) {
        try {
          ...
          if (result.trim()) {
            ...
            log(
              "warn",
              `[FORGE] console.log found in ${event.path} (${lines} occurrence${lines > 1 ? "s" : ""})`
            )
          }
        } catch {
    ...
    "tool.execute.after": async (
      ...
      if (
        ...
        try {
          ...
          log("info", "[FORGE] TypeScript check passed")
        } catch (error: unknown) {
          const err = error as { stdout?: string }
          log("warn", "[FORGE] TypeScript errors detected:")
          if (err.stdout) {
      ...
      if (
        ...
        log("info", "[FORGE] PR created - check GitHub Actions status")
      }
    },
    ...
    "tool.execute.before": async (
      ...
      if (
        ...
        log(
          "info",
          "[FORGE] Remember to review changes before pushing: git diff origin/main...HEAD"
        )
      }
      ...
      if (
        ...
        if (
          ...
          log(
            "warn",
            `[FORGE] Creating ${filePath} - consider if this documentation is necessary`
          )
        }
      }
      ...
      if (hookEnabled("pre:bash:tmux-reminder", "strict") && input.tool === "bash") {
        ...
        if (
          ...
          log(
            "info",
            "[FORGE] Long-running command detected - consider using background execution"
          )
        }
      }
    },
    ...
    "session.created": async () => {
      ...
      log("info", `[FORGE] Session started - profile=${currentProfile}`)

      // Check for project-specific context files
      if (hasProjectFile("CLAUDE.md")) {
        log("info", "[FORGE] Found CLAUDE.md - loading project context")
      }
    },
    ...
    "session.idle": async () => {
      ...
      log("info", "[FORGE] Session idle - running console.log audit")
      ...
      if (totalConsoleLogCount > 0) {
        log(
          "warn",
          `[FORGE] Audit: ${totalConsoleLogCount} console.log statement(s) in ${filesWithConsoleLogs.length} file(s)`
        )
        filesWithConsoleLogs.forEach((f) =>
          log("warn", `  - ${f}`)
        )
        log("warn", "[FORGE] Remove console.log statements before committing")
      } else {
        log("info", "[FORGE] Audit passed: No console.log statements found")
      }
      ...
      try {
        if (process.platform === "darwin") {
          // macOS
          await $`osascript -e 'display notification "Task completed!" with title "OpenCode FORGE"' 2>/dev/null`
        } else if (process.platform === "win32") {
          // Windows - PowerShell notification
          await $`powershell -Command "Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show('Task completed!', 'OpenCode FORGE', 'OK', 'Information')" 2>/dev/null`
        } else if (process.platform === "linux") {
          // Linux - notify-send (requires libnotify)
          await $`notify-send "OpenCode FORGE" "Task completed!" 2>/dev/null`
        }
      } catch (error: unknown) {
        // Notification not supported or failed - log but continue
        const errorMessage = error instanceof Error ? error.message : String(error)
        log("debug", `[FORGE] Desktop notification failed: ${errorMessage}`)
      }
    ...
    "session.deleted": async () => {
      ...
      log("info", "[FORGE] Session ended - cleaning up")
      editedFiles.clear()
    ...
    "todo.updated": async (event: { todos: Array<{ text: string; done: boolean }> }) => {
      ...
      if (total > 0) {
        log("info", `[FORGE] Progress: ${completed}/${total} tasks completed`)
      }
    },
     ...
     * Action: Sets PROJECT_ROOT, PACKAGE_MANAGER, DETECTED_LANGUAGES, FORGE_VERSION
     */
    "shell.env": async () => {
     ...
     * Action: Push FORGE context block and custom compaction prompt
     */
    "experimental.session.compacting": async () => {
      const contextBlock = [
        "# FORGE Context (preserve across compaction)",
        "",
        "## Active Plugin: FORGE v1.0.0",
    ...
    "permission.ask": async (event: PermissionEvent) => {
      log("info", `[FORGE] Permission requested for: ${event.tool}`)
      ...
      try {
        ...
        if (["read", "glob", "grep", "search", "list"].includes(event.tool)) {
          log("debug", `[FORGE] Auto-approved read-only tool: ${event.tool}`)
        ...
        if (event.tool === "bash" && /^(npx )?(@biomejs\/biome|prettier|black|gofmt|rustfmt|swift-format)/.test(cmd)) {
          log("debug", `[FORGE] Auto-approved formatter: ${cmd}`)
        ...
        if (event.tool === "bash" && /^(npm test|npx vitest|npx jest|pytest|go test|cargo test)/.test(cmd)) {
          log("debug", `[FORGE] Auto-approved test execution: ${cmd}`)
          return { approved: true, reason: "Test execution" }
        }

        // Everything else: let user decide
        log("debug", `[FORGE] Permission requires user approval: ${event.tool}`)
        return { approved: undefined }
      } catch (error: unknown) {
        // Error in permission handling - log and deny for safety
        const errorMessage = error instanceof Error ? error.message : String(error)
        log("error", `[FORGE] Permission handling error for ${event.tool}: ${errorMessage}`)
...
export default ForgeHooksPlugin
