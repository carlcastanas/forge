...
This guide helps you migrate from Claude Code to OpenCode while using the FORGE configuration.
...
### FORGE Plugin Hooks Included

The FORGE OpenCode configuration includes translated hooks:
...
### 2. Use the FORGE OpenCode Configuration
...
│   ├── forge-hooks.ts           # All FORGE hooks as plugins
...
### Option 1: Use FORGE Configuration Directly
...
This only loads the published FORGE OpenCode plugin module (hooks/events and exported plugin tools).
It does **not** automatically inject FORGE's full `agent`, `command`, or `instructions` config into your project.

If you want the full FORGE OpenCode workflow surface, use the repository's bundled `.opencode/opencode.json` as your base config or copy these pieces into your project:
- `.opencode/commands/`
- `.opencode/prompts/`
...
4. If you installed only `plugin: ["forge-universal"]`, note that npm plugin install does not auto-add FORGE commands or agents to your project config
