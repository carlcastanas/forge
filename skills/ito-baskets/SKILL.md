...
- Never use the compute bridge for basket data: `forge ito find` submits an
  authenticated RFQ and `forge ito status` reads RFQ/procurement status, not
...
This skill never uses device authorization or `forge ito login`; those belong to
