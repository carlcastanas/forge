---
name: hermes-imports
description: Convert local Hermes operator workflows into sanitized FORGE skills and release-pack artifacts. Use when preparing a Hermes workflow for public FORGE reuse without leaking private workspace state, credentials, or local-only paths.
metadata:
  origin: FORGE
---
...
Use this skill when turning a repeated Hermes workflow into something safe to ship in FORGE.

Hermes is the operator shell. FORGE is the reusable workflow layer. Imports should move stable patterns from Hermes into FORGE without moving private state.

## When To Use

- A Hermes workflow has repeated enough times to become reusable.
- A local operator prompt should become a public FORGE skill.
