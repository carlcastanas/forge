...
description: Guide FORGE installation, update, or reconfiguration from inside Claude Code, Codex, or Kimi while respecting each harness's real plugin, scope, and hook capabilities.
metadata:
  origin: FORGE
---
...
and show the welcome only after success. Never clone FORGE into a temporary
directory or copy plugin components by hand.

For a human-operated terminal, the canonical entry points are `forge setup` and
`npx forge-universal setup`. Inside a harness, use the explicit non-interactive
...
- In Codex, use Codex's native plugin lifecycle. Do not offer Claude scopes or
  map FORGE's four Claude hook profiles onto Codex.
- In Kimi, install the project surface under `./.kimi-code`. Kimi does not
  provide FORGE's Claude lifecycle-hook profiles.
...
Run both commands and summarize the installed FORGE scope, enabled state, and
marketplace source:
...
Treat a single existing `forge@forge` installation as a reconfiguration. Do not
interpret Claude's provider-owned "Open home page" control as installation
evidence. Stop and report the recovery returned by setup for multiple FORGE
...
- `off` keeps skills and commands but disables FORGE hook automation.
...
```bash
npx --yes --package forge-universal forge setup --mode claude-plugin \
  --scope <scope> --hooks <hooks> [--move-scope] --dry-run --json
```
...
Ask one yes/no question. Do not run a bare interactive `forge setup` through a
...
Continue only when exactly one enabled `forge@forge` entry exists at the selected
...
`FORGE_VERSION_PATTERN` from `scripts/lib/terminal-welcome.js`. Reject unexpected
...
Continue only when the JSON reports FORGE installed and provides its
...
`FORGE_VERSION_PATTERN`. Invoke `node` directly with this argument array; this is
...
`./.kimi-code`; `hooks=unsupported` for FORGE lifecycle hooks. Do not ask the
Claude scope or hook-mode questions. Preview first:

```bash
npx --yes --package forge-universal forge install --profile core --target kimi --dry-run
```
...
```bash
npx --yes --package forge-universal forge doctor --target kimi
```
...
```bash
npx --yes --package forge-universal forge welcome --action configured
```

Do not claim that Kimi installed or configured FORGE lifecycle hooks.
