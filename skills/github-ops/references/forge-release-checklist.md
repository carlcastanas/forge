...
- Milestone: `M0` in the FORGE 2.2 release train.
- Contract: ship one exact, verified artifact, keep FORGE authority over release
...
- Keep external-compute and Nasiko wording bounded to shipped behavior only.
...
- Do **not** use `scripts/release.sh` as-is for `FORGE-031`.
...
git rev-parse HEAD
git rev-parse origin/main
git tag -s "v${VERSION}" -m "FORGE ${VERSION}" HEAD
git tag -v "v${VERSION}"
...
### 9. Record Evidence And Close Out FORGE-031
...
- Mark `FORGE-031` resolved only after the signed patch release is public and
