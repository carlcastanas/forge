...
description: Evidence-first automation inventory and overlap audit workflow for FORGE. Use when the user wants to know which jobs, hooks, connectors, MCP servers, or wrappers are live, broken, redundant, or missing before fixing anything.
metadata:
  origin: FORGE
---
...
- `forge-tools-cost-audit` when the real problem is webhook fanout, queued jobs, or billing burn in the sibling app repo
...
- the user wants to know what was ported from another agent system and what still needs to be rebuilt inside FORGE
...
The value is in collapsing noisy automation into one canonical FORGE lane, not in preserving every historical path.
