...
`ito-training` is the canonical FORGE skill for training on Itô compute. FORGE
...
Managed training is unavailable today. The FORGE bridge exposes only `login`,
...
backend remain scaffolds. The locally enforceable guarantee is that FORGE rejects
