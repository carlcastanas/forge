...
**Opt-in (FORGE):** The `codescene` block in `mcp-configs/mcp-servers.json` is a template only. FORGE plugin installs do not auto-enable bundled MCP servers. Copy the entry into your config only if you want it. You can exclude it during FORGE install/sync with `FORGE_DISABLED_MCPS=codescene,...`.
...
Same triggers as **When to Use** above — this heading is what FORGE uses for skill auto-activation.
...
| FORGE skill / flow | Code Health MCP role |
|------------------|----------------------|
...
**Context tip:** FORGE recommends keeping MCP count low. Enable `codescene` when doing substantive edits; disable when not needed.
