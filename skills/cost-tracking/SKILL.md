---
name: cost-tracking
description: Track and report Claude Code token usage, spending, and budgets from the local FORGE cost-tracker metrics log. Use when the user asks about costs, spending, usage, tokens, budgets, or cost breakdowns by model, session, or date.
metadata:
  origin: community
---
...
that FORGE's `stop:cost-tracker` hook writes.
