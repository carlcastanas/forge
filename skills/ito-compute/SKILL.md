...
Use the canonical Itô compute CLI or MCP server. FORGE does not implement a
parallel client, local simulation, reservation, workload runner, or inference
server. FORGE itself does no browser automation.
...
Set `FORGE_ITO_CLI_EXECUTABLE` to the explicit absolute built entry:

```text
/absolute/path/to/ito-cloud-runtime/cli/ito-compute-cli/dist/bin/ito.js
```

FORGE never discovers this credential-bearing client through `PATH`.
`forge ito login` performs device authorization and never inherits `ITO_API_KEY`.
...
1. Run `forge ito login` before the first operation. FORGE delegates this to the
   ...
   `forge ito login --no-browser` to suppress the page handoff. FORGE itself does no
   ...
   return to the originating task and continue with `forge ito auth`.
...
2. Run `forge ito auth` to validate existing credentials; it never starts login
   and rejects `--no-browser`.
3. Before `forge ito find`, obtain explicit buyer authority to submit an RFQ.
...
5. Run `forge ito status` to inspect RFQs and procurement orders.
...
6. When a quote is ready and the buyer explicitly approves, accept it:
   ...
   ```sh
   forge ito accept rfq_<ticket-id>
   ```
...
7. Run `forge ito logout` when the user explicitly asks to revoke this device.
...
`forge ito evals` exposes the canonical CLI's narrow live adapter to a separately
...
recover, repair, reset, purchase, or order resources. FORGE does not forward
...
order. The passive dashboard link in FORGE help is a separate user-operated web
