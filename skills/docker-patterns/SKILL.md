...
- Keep npm and npx's executable cache at `NPM_CONFIG_CACHE=/tmp/npm-cache` on
  ...
  with `FORGE_TMPFS_SIZE`; `FORGE_WORKSPACE_SIZE` separately controls the private
  workspace mount.
...
### Exercise the FORGE Plugin Setup Harness
...
```bash
docker compose -p forge-plugin-debian-test \
  -f docker/plugin-setup/compose.yaml \
  run --rm -T real-cli dry-run

docker compose -p forge-plugin-ubuntu-test \
  -f docker/plugin-setup/compose.yaml \
...
```bash
forge install --profile core --target claude-project --dry-run --json
```
...
self-created tarball under `/tmp`, validates the `forge-universal` package name,
required install manifests, and the confined `package.json` `bin.forge` mapping,
then invokes the extracted `forge` executable. The runtime stays on
...
```bash
docker compose -p forge-plugin-session \
  -f docker/plugin-setup/compose.yaml \
  run --detach --name forge-plugin-shell real-cli shell
```
...
```bash
docker inspect --format '{{.State.Running}}' forge-plugin-shell
...
```bash
docker exec -it -w /workspace/project forge-plugin-shell bash
```
...
```bash
docker rm --force forge-plugin-shell
docker compose -p forge-plugin-session \
  -f docker/plugin-setup/compose.yaml \
  down --remove-orphans
```
...
```bash
docker image inspect forge-plugin-setup:debian forge-plugin-setup:ubuntu
```
