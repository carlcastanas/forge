...
| Concern | FORGE | Plankton |
|---------|-----|----------|
| Code quality enforcement | PostToolUse hooks (Prettier, tsc) | PostToolUse hooks (20+ linters + subprocess fixes) |
| Security scanning | Forge Shield, security-reviewer agent | Bandit (Python), Semgrep (TypeScript) |
...
1. Install FORGE as your plugin (agents, skills, commands, rules)
2. Add Plankton hooks for write-time quality enforcement
3. Use Forge Shield for security audits
4. Use FORGE's verification-loop as a final gate before PRs

### Avoiding Hook Conflicts

If running both FORGE and Plankton hooks:
- FORGE's Prettier hook and Plankton's biome formatter may conflict on JS/TS files
- Resolution: disable FORGE's Prettier PostToolUse hook when using Plankton (Plankton's biome is more comprehensive)
- Both can coexist on different file types (FORGE handles what Plankton doesn't cover)
...
```bash
export FORGE_HOOK_PROFILE=strict
export FORGE_QUALITY_GATE_FIX=true
export FORGE_QUALITY_GATE_STRICT=true
```
