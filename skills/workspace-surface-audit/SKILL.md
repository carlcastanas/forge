...
description: Audit the active repo, MCP servers, plugins, connectors, env surfaces, and harness setup, then recommend the highest-value FORGE-native skills, hooks, agents, and operator workflows. Use when the user wants help setting up Claude Code or understanding what capabilities are actually available in their environment.
metadata:
  origin: FORGE
---
...
This is the FORGE-native answer to setup-audit plugins. It does not modify files unless the user explicitly asks for follow-up implementation.
...
- Comparing official marketplace plugins against FORGE-native coverage
...
- Prefer FORGE-native workflows over generic "install another plugin" advice when FORGE can reasonably own the surface.
...
- Separate three things clearly:
  ...
  - available but not wrapped well in FORGE
...
2. Environment surface
   - `.env*` files in the active repo and obvious adjacent FORGE workspaces
...
- existing FORGE skills already relevant to the workspace

If a surface exists only as a primitive, call that out. Example:

- "Stripe is available via connected app, but FORGE lacks a billing-operator skill"
- "Google Drive is connected, but there is no FORGE-native Google Workspace operator workflow"
...
2. whether FORGE already has parity
3. whether FORGE only has primitives
4. whether FORGE is missing the workflow entirely

### Phase 3: Turn Gaps Into FORGE Decisions

For every real gap, recommend the correct FORGE-native shape:

| Gap Type | Preferred FORGE Shape |
|----------|---------------------|
...
2. **Parity**
   - where FORGE already matches or exceeds the benchmark
3. **Primitive-only gaps**
   - tools exist, but FORGE lacks a clean operator skill
4. **Missing integrations**
   - capability not available yet
5. **Top 3-5 next moves**
   - concrete FORGE-native additions, ordered by impact
...
- If FORGE already has a strong primitive, propose a wrapper skill instead of inventing a brand-new subsystem.

## Good Outcomes

- The user can immediately see what is connected, what is missing, and what FORGE should own next.
