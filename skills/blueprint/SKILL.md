...
This skill ships with FORGE. No separate installation is needed when FORGE is installed.

### Full FORGE install

If you are working from the FORGE repository checkout, verify the skill is present with:

```bash
test -f skills/blueprint/SKILL.md
```

To update later, review the FORGE diff before updating:
...
If you are vendoring only this skill outside the full FORGE install, copy the reviewed file from the FORGE repository into `~/.claude/skills/blueprint/SKILL.md`. Vendored copies do not have a git remote, so update them by re-copying the file from a reviewed FORGE commit rather than running `git pull`.
