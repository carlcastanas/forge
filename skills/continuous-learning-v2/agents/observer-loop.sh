...
ANALYSIS_COOLDOWN="${FORGE_OBSERVER_ANALYSIS_COOLDOWN:-60}"
IDLE_TIMEOUT_SECONDS="${FORGE_OBSERVER_IDLE_TIMEOUT_SECONDS:-1800}"
...
analyze_observations() {
  ...
  if [ "${CLV2_IS_WINDOWS:-false}" = "true" ] && [ "${FORGE_OBSERVER_ALLOW_WINDOWS:-false}" != "true" ]; then
    echo "[$(date)] Skipping claude analysis on Windows due to known non-interactive hang issue (#295). Set FORGE_OBSERVER_ALLOW_WINDOWS=true to override." >> "$LOG_FILE"
    return
  fi
  ...
  MAX_ANALYSIS_LINES="${FORGE_OBSERVER_MAX_ANALYSIS_LINES:-500}"
  ...
  analysis_file="$(mktemp "${observer_tmp_dir}/forge-observer-analysis.jsonl.XXXXXX")"
  ...
  prompt_file="$(mktemp "${observer_tmp_dir}/forge-observer-prompt.XXXXXX")"
...
PROMPT
  ...
  timeout_seconds="${FORGE_OBSERVER_TIMEOUT_SECONDS:-120}"
  ...
  if [ -n "${FORGE_OBSERVER_MAX_TURNS:-}" ]; then
    max_turns="${FORGE_OBSERVER_MAX_TURNS}"
  else
  ...
  # but an explicit FORGE_OBSERVER_MAX_TURNS override may be non-numeric, empty, or too
  ...
  analysis_result_file="$(mktemp "${observer_tmp_dir}/forge-observer-result.XXXXXX")"
  ...
  # Model is configurable via FORGE_OBSERVER_MODEL (defaults to haiku for cost efficiency);
  # e.g. FORGE_OBSERVER_MODEL=opus for higher-quality instinct extraction. Heavier models are
  # slower — consider raising FORGE_OBSERVER_TIMEOUT_SECONDS (default 120s) so the watchdog
  ...
  set -m
  FORGE_SKIP_OBSERVE=1 FORGE_HOOK_PROFILE=minimal claude --model "${FORGE_OBSERVER_MODEL:-haiku}" --max-turns "$max_turns" --print \
