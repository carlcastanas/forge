...
#   - FORGE observing its own Haiku observer sessions (self-loop)
#   - FORGE observing other tools' automated sessions
...
# (FORGE_HOOK_PROFILE=minimal, FORGE_SKIP_OBSERVE=1, agent_id, path exclusions).
...
[ "${FORGE_HOOK_PROFILE:-standard}" = "minimal" ] && exit 0

# Layer 3: cooperative skip env var for automated sessions.
[ "${FORGE_SKIP_OBSERVE:-0}" = "1" ] && exit 0

# Layer 4: subagent sessions are automated by definition.
_FORGE_AGENT_ID=$(echo "$INPUT_JSON" | "$PYTHON_CMD" -c "import json,sys; print(json.load(sys.stdin).get('agent_id',''))" 2>/dev/null || true)
[ -n "$_FORGE_AGENT_ID" ] && exit 0

# Layer 5: known observer-session path exclusions.
_FORGE_SKIP_PATHS="${FORGE_OBSERVE_SKIP_PATHS:-observer-sessions,.claude-mem}"
if [ -n "$STDIN_CWD" ]; then
  IFS=',' read -ra _FORGE_SKIP_ARRAY <<< "$_FORGE_SKIP_PATHS"
  for _pattern in "${_FORGE_SKIP_ARRAY[@]}"; do
...
_NOTE_OBSERVER_NOSURVIVE() {
  ...
  local warn_after="${FORGE_OBSERVER_NOSURVIVE_WARN_AFTER:-3}"
  local streak
  ...
  if printf '%s\n' "$streak" > "$streak_file" 2>/dev/null &&
    ...
    printf -v message '%s\n%s\n%s\n%s' \
      ...
      "$platform_hint" \
      "[observe] Set FORGE_OBSERVER_NOSURVIVE_WARN_AFTER to change this threshold (currently ${warn_after})."
...
SIGNAL_EVERY_N="${FORGE_OBSERVER_SIGNAL_EVERY_N:-20}"
