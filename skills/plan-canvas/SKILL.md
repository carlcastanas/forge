...
FORGE-native around the `/plan` confirmation gate, with zero dependencies.
...
Invoke the CLI as `forge-plan-canvas` — the bin shipped by the `forge-universal`
...
Codex — or just run the `forge-plan-canvas` commands directly.

```bash
# 1. Open the artifact in the user's browser (returns immediately)
forge-plan-canvas open .claude/plans/feature.plan.md

# 2. Block until the human responds. Leave running; re-run if interrupted:
#    queued feedback is never lost.
forge-plan-canvas await .claude/plans/feature.plan.md
```
...
- `forge-plan-canvas pending` lists feedback queued with no listener. Check it
...
```bash
forge-plan-canvas await <file> --reply "Split Phase 2 as requested. Take a look."
```
...
forge-plan-canvas typing <file> --state thinking
# switch to "agent is typing..." just before a reply lands
forge-plan-canvas typing <file> --state typing
```
...
**4. End** when review concludes: `forge-plan-canvas end <file>`.
...
Diagrams render in the FORGE dark theme with the accent palette. Mermaid loads in
...
mirror at `FORGE_PLAN_CANVAS_MERMAID_URL` for air-gapped use.

## Rules

- Markdown artifacts render in FORGE's plan template (including Mermaid blocks);
...
- The server is loopback-only and exits after 30 idle minutes
  (`FORGE_PLAN_CANVAS_IDLE_MS`); `stop` shuts it down explicitly. State lives
  in `~/.claude/plan-canvas/` (`FORGE_PLAN_CANVAS_STATE_DIR`).
...
```bash
forge-plan-canvas open .claude/plans/notifications.plan.md
forge-plan-canvas await .claude/plans/notifications.plan.md
# → {"status":"feedback","items":[{"kind":"verdict","verdict":"approve"}]}
forge-plan-canvas end .claude/plans/notifications.plan.md
...
forge-plan-canvas await <file> --reply "Reworked the risk table."
