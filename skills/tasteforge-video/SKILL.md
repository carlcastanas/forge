...
`tasteforge` package in the tasteforge-video repository; FORGE orchestrates and
...
This boundary is the core of the skill. Everything FORGE can actually run is
**local, deterministic, and offline**:

| Operation | Deterministic? | FORGE may run |
|---|---|---|
...
**Provider generation must fail closed in FORGE.** Any live Fal (or other
...
separate lane with its own review. FORGE never calls Fal, never reads any API
...
FORGE's job is to route here, run the local deterministic commands, and
