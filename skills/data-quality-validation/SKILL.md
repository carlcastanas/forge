...
gets acted on. Two things determine whether a pipeline is trustworthy: where the checks
sit relative to the write, and whether a failing check actually stops something. Done
means every producer-consumer boundary has a declared contract, violations are caught
before publication, failing rows are held somewhere inspectable rather than discarded,
and the alerts still get read after a month.

## When to activate

- Building or reviewing an ingestion pipeline, ELT model, or reverse-ETL sync
- A downstream consumer reports numbers that do not reconcile, or an upstream provider
  changed a field, a type, or a nullability without notice
- Choosing where to put tests: source, staging, mart, or serving layer
- Designing alerting for data pipelines, or cleaning up after a bad load
...
- Schema evolution and rollout mechanics — use
  [database-migrations](../database-migrations/SKILL.md)
- Query performance and index design — [postgres-patterns](../postgres-patterns/SKILL.md)
- Deciding whether a metric change is real — use
  [ab-testing-experimentation](../ab-testing-experimentation/SKILL.md)
- Interface agreements for service APIs — use
  [contract-first](../contract-first/SKILL.md)

## Prerequisites

- A named owner for each source table and each published dataset
- A framework that can run assertions (dbt, SQLMesh, Great Expectations, Soda, or plain
  SQL in an orchestrator), and the ability to fail a step
- A quarantine location with the target's schema, plus a rejection reason column

## Process

### 1. Write the contract before writing the checks

A contract states what a consumer may rely on. Without one, every check is an engineer's
guess and the post-incident argument has no reference point.
...
The `semantics` block prevents the most expensive class of data bug: two teams reading
the same correct table and getting different answers because they assumed different
grain.
...
| Completeness | Is anything missing? | Not-null on required fields; row count in an expected band |
| Validity | Are values in range? | Accepted-value sets, numeric bounds, identifier regex, no future timestamps |
| Uniqueness / integrity | Is the grain right and are keys resolvable? | Unique on the grain key; foreign keys resolve |
| Freshness / volume | Did it arrive, and is the amount plausible? | Max timestamp within SLA; count deviation from trailing median |
...
models:
  - name: fct_orders
    ...
    columns:
      ...
      - name: status
        tests: [{ accepted_values: { values: [pending, paid, shipped, refunded] } }]
      - name: total_cents
        tests: [{ dbt_utils.accepted_range: { min_value: 0, max_value: 100000000 } }]
```

Volume checks are the most often skipped and the most often first to reveal an upstream
break. A row count that collapses passes every column-level test ever written.
...
with daily as (
  ...
  from {{ ref('fct_orders') }}
  where placed_at >= current_date - interval '15 days' group by 1
), baseline as (
...
where d = current_date - interval '1 day' and (n < med * 0.6 or n > med * 1.8)
```
...
range downstream code assumes.

**Monitor** — records and alerts, does not block. Use when a violation is informational,
or when blocking harms more than the bad data does: a slight freshness miss, a
distribution shift, a new category in a low-cardinality field.
...
Then treat quarantine as a queue with an owner, not an archive. Report rejected counts
beside accepted counts in every run summary so partial ingestion is visible; give
quarantine rows a retention policy, because an unbounded reject table becomes its own
storage and privacy problem; and make replay of a fixed batch a single command.
...
- Alert on the aggregate rejection rate crossing a threshold, not on each rejected row,
  and when quarantine stays non-empty beyond a stated period.
...
- Route by severity: gate failures page, monitor breaches open a ticket, drift lands in
  a weekly digest.
- Deduplicate by dataset and check; suppress downstream alerts when an upstream dataset
  already failed. One broken source should produce one page, not forty.
...
7. Notify consumers with the affected window and which metrics moved, and replay the
   quarantine batch for that window.
8. Complete only when the reconciliation query returns zero variance.
```
...
- [ ] Backfill runbook covers snapshot, reconciliation, and consumer notice; each
      incident moves at least one check earlier in the pipeline
...
- dbt tests plus the `dbt_utils` and `dbt_expectations` packages
